...
- **seedgo: the docs page standard — one shape for every `docs/*.md` page, scored in audits** (DPLAN-0351 / FPLAN-0628; seedgo building, devpulse landing). The owner, 2026-09-19 09:48: "expecting same structure throughout with guides experience for the agents… its a lot of docs so ill be only random checking." Two independent surveys of the fleet's 179 pages (seedgo's and a devpulse sub-agent's, compared in DPLAN-0351 Notes) agreed on what the corpus already does and refuted the generic-slot skeleton devpulse had drafted: 418 of 437 section headings are single-use topic names and the best pages use them, so the shape is a bare README back-link at the top, one `# Title`, one present-tense purpose paragraph, free topic sections at depth ≤3 with commands by reference, and fixed names only for the optional `## Why it is this way`, `## What is not verified` and `## Related` (the corpus's one shared heading, 56 pages). No history, no cured entries, no defect paragraphs. New in `aipass_standards/`: `docs_page.md`, `docs_page_content.py`, `docs_page_check.py` — five scored checks over `<branch>/docs/*.md` one level deep (one H1 first, a purpose paragraph, heading depth, relative links and images resolve, size at the cap), plus advisory nominate-only lines in `check_branch_info` for the back-link above the purpose, story arms at 7/8 precision or better (`used to`, `previously`, cure verb beside a date), and defect prose (a line linking to a known_issues page is a pointer, excluded). Pages named `known_issues`/`tech_debt` are exempt by name, scored and advisory, until the owner rules on defect registers. The 20k cap did not exist as a key: `context_standards/pack.json` gains `caps["docs/*.md"].max_chars = 20000`, read at call time through `startup_budget_check.docs_page_cap()`, and a test pins that the checker holds no copy. Measured before landing: 160 pages scored, 16 exempt, 15 of 18 branches at 100, 0 dead links, 0 over size; 3 pages red for a missing purpose paragraph (ai_mail S84, api SECURITY, backup tests), each given its one line in this landing so every branch reads Docs_Page 100. Advisory baseline for wave 0: back-link missing on 119 pages in 17 branches, story 69 lines in 15, defect prose 11 lines in 8. 28 cases + 1 content test, 28/28 mutants killed, suite 4357. `EXPECTED_STANDARDS` 48 → 49. Seedgo's own `docs/v5_vs_haiku_shadow_diff.md` moved to its `docs.local/research/`. The scaffold template contract went to spawn; the render-time read waits on spawn. README consistency, measured with no rule added: all 18 READMEs carry Quick Start, Commands and Architecture; the de facto order (Quick Start · What It Does · Live Inventory · How To Reach Me · Commands · Architecture · Documentation · Integration Points) is exact on 6, and an ordered 8-section check would flag 11 of 18 today, the owner's call.
- **seedgo: the README standard names its eight sections, and the docs page back-link is scored** (DPLAN-0351 phase 3, `35943ae9`; seedgo building, devpulse landing). The owner, 2026-09-19 09:48: "id like to get the readme to some sort of consistency and be seedgo checking in audits." `readme_check` gains the advisory `readme sections` line over `README_SECTIONS`, the eight exact ordered names above, nominating a missing or misplaced section by name; `readme.md` 1.3 carries the canonical order, the AUTO-marker sections retire, and the nominations for a lower-case H1 and a `**Last Updated:**` line off the foot are advisory too. `docs_page_check` folds the back-link into check 1 (a page opens with the README back-link, then one H1: red, not a nomination) and gains check 6, "not a register": a page named `known_issues` or `tech_debt` is red, the exemption gone with the owner's ruling. Spawn's half: the docs page skeleton at `spawn/templates/docs_page.md` BESIDE the citizen tree, never stamped, read through `docs_page_template()` which raises on a missing file and holds no fallback, and `templates/citizen/docs/README.md` in the index shape (`fbe74f5d`).
- **seedgo: the name ratchet — the owner's first name and a person's home path cannot regrow in the tree** (DPLAN-0350, the last step; seedgo building, devpulse landing). New `context_standards/name_ratchet.py` + `name_ratchet_baseline.json`, wired as gate 2 in `.github/scripts/seedgo_audit.py` between the startup ratchet and the audit, advisory (`NAME_RATCHET_GATES = False`: it prints its red and the job carries on; flipping it is one line). The name is a case-insensitive substring of any line in any `git ls-files` file and of the path itself; the home-path rule covers `/home/<x>`, `/Users/<x>` and `C:\Users\<x>` in `.md` and `*_content.py`, trailing slash optional (the slash-required shape missed 2 of the 5 telegram table cells), placeholders pass (`user`, `me`, `someone`, `<…>`, `{…}`, `$…`, `runner`). The baseline holds a count of lines per file per rule — paths and numbers only, never a line or a hash, so the file cannot carry the name; a red is only above the baseline, a cure shows as LOOSE with the number to write. The name is assembled from two pieces in the source and every reported line is masked. A tree git cannot list, an unreadable baseline or an unopenable file is red, never clean. Exemptions are exactly DPLAN-0350's list (the culture doc, the CHANGELOG, plan files, the hook run logs, `.claude/settings.json`, the fixtures that must carry the name, the suspend tool), each covering that one file, not its family, named in the docstring and pinned by a test. Measured: HEAD 0 new (20 name lines in 11 files and 5 home-path lines in 1 baselined); the pre-sweep tree 722d8db2 would have read 549 new name lines in 307 files. 18 cases in batch10's `TestRatchet*` classes and 1 CI-script case; none spells the name, they read it off the module, with batch10's own exempt fixtures as the external oracle after a misspelt name survived the first mutant run (26/26 killed after).
- **memory: a runtime fence on every write path — memory never writes outside AIPass, and the leak is purged** (DPLAN-0349; memory owning, devpulse landing). The owner's ruling A, 2026-09-18: an AIPass agent must have no way to write another project's files; reading is allowed, writing never. What happened: one `drone @memory` call from a Vera Studio cwd on 09-17 07:47 persisted that project's registry into `memory_json/known_registries.json`, and every rollover walk after it (six runs, 09-17 07:59 → 09-18 18:57, from writer's and verify's PreCompacts and two auto_process passes) wrote four Vera Studio branches' `.trinity` files, created `.backup/` and `.chroma/` directories in their trees (47 files, 13 dirs) and ingested their entries into the AIPass global Chroma. The fence, new `apps/handlers/write_fence.py`: `outside_root(target, root)` is the pure verdict on RESOLVED paths by containment, not `startswith` (so `/x/aipass_other` is outside `/x/aipass`); `fence_write(target, lane=)` is the one wrapper — an ERROR naming path and root, a `write_fence_refused` op-log row, nothing written. ROOT is the repository root walked from memory's own source file up to `AIPASS_REGISTRY.json`, never cwd; no allow-list, no env override; it fails SHUT when the root resolves to the filesystem root (repo_root's last resort on a wheel with no registry, where `/` would admit every write). Guarded: `write_memory_file` (the gate for extractor, trinity_push, tab_renderer, normalizer and todo_roll), rollover backup and restore, `_recreate_trinity_file`, `normalize_memory_file`, the backlog parent, the branch-local Chroma path, `spawn_pusher._write_json`, `receipt._write`; unfenced on purpose because they write memory's own state inside its own tree: config, op logs, `.ai_central`, `AIPASS_ROOTS.json`, push report, template bump, the processors, the indexer, the lock and the global `.chroma`. No cwd persistence: `persist_registry` and `load_known_registries` are gone, `_find_caller_registries` only walks and drops any out-of-root registry, the watcher now shares the detector's scope (one definition; +3 residents its own walk had missed). `known_registries.json` is retired to `memory_json/.archive/` (17 entries: 7 real, 10 pytest temp paths the old code had appended). Rollover scope 26 → 22, the difference exactly Vera Studio's four branches, the same from both cwds. Purged from the global Chroma: 285 records in 7 `<branch>_<type>` collections (vera 114, verify 121, writer 36, research 14; 09-17: 213, 09-18: 37, and 35 from April in `vera_local` — same species, same certainty), every id verified before deletion, every text present word for word in that branch's own local store, the collections dropped; search returns 47 collections, 0 Vera-branch. NOT removed, other owners' lanes and the owner's call: 204 + 179 Vera sender/recipient rows in ai_mail's sent and deleted collections, 121 `flow_plans` chunks of @vera-owned plans, and @backup's 08-15 snapshots of the store. The meta line "archived to @memory" is stamped only through the fence. The `.backup/` dirs, `rollover_backup_*.json` files and `.chroma/` stores memory created inside Vera Studio are still there for Vera to judge; the full write table with a log line per row is in memory's `docs.local/sub_agent_drops/vera_writes_2026-09-18.md`. Also: `test_declared_roots` tripwire narrowed to the claim that matters (an external citizen declaring core or resident) after Vera's honest `residency: external` stamp fired it. Red first: 22 pins failed before the wiring, 5 mutants red; 16 net new tests inside existing classes. From the devpulse seat: 1895 passed / 2 skipped; the scope pin `test_the_rollover_lane_never_reaches_into_another_repository` green plainly and with a Vera Studio caller cwd; `audit aipass @memory` 100. Open on memory's pad: two hooks `rollover FAILED` lines at the last seconds of two Vera runs, undiagnosed.

- **hooks: the edit gate reads who may edit whose files from the registries — the owner's boundary ruling, encoded** (DPLAN-0349, hooks FPLAN-0627; hooks owning, devpulse landing). The ruling, 2026-09-18: an agent is sandboxed to its own files; devpulse alone edits anywhere; seedgo and spawn edit anything inside AIPass and never beyond it; a manager edits her own project's agents' files and nothing outside her project; reading another's files is allowed, writing never. Measured BEFORE the cure with `edit_gate.handle` on the live tree, three lanes each (Edit, `sed -i`, `python -c`): trust ran downward, so any AIPass seat could write any project nested under it (`projects/baud` allowed for seedgo and for hooks); a manager had no standing on the tool lane (Vera refused on Edit into her own writer agent) and nobody inside a project was fenced on the shell lane (hooks `sed -i` into memory allowed); project-level files (the repo root's `.aipass/`, `.claude/`, README) were anybody's; the three trusted names counted in ANY project — a Vera branch named seedgo was trusted inside Vera; and the registry contents were never read at all, the project root being a `*_REGISTRY.json` filename glob and the branch a `src/<pkg>/<branch>` path shape. AFTER: new `apps/modules/write_ownership.py` 1.0.0 reads the boundary — the caller's project is the nearest ancestor of the session cwd holding a `*_REGISTRY.json` (the same marker as the mail fence, GH #733), a branch is the deepest registry row whose path holds the file, a manager is the row with `owner: true` (spawn's sealed field, never the passport), and seedgo/spawn are trusted only when the caller's registry is `AIPASS_REGISTRY.json`; the path shape survives only as a fallback where a project has no branches table. Crossings are refused in BOTH directions unless the caller is the verified admin seat (the downward-trust pin REVERSED from allow to refuse; the verified-admin twin still passes). Inside a project the shell lane convicts on write grammar only (redirect, `tee`, `sed -i`, `cp`/`mv` destination, `dd of=`) and never on an interpreter's held paths, because a read cannot be told from a write and the ruling allows reading — the residual (python writing another branch of its own project) is stated in `docs/edit_gate.md` rather than hidden. Project-level files now belong to the manager, which includes hooks' own `.aipass/hooks.json`, `.aipass/project_hooks.json` and `.claude/provider_manifest.json` — @hooks no longer edits them and routes them through @devpulse; per-file ownership of project-level paths would need a registry field nobody invented. A `*_REGISTRY.json` that parses as a non-empty table with no `branches` key (flow's `PLAN_REGISTRY.json`) no longer marks a project, so @flow's plan files do not read as a foreign project to @flow. The gate's outer "unexpected error (allowing)" now logs at WARNING, because the fence is a call-time import and a crash turns every fence off for that call. `docs/edit_gate.md` opens with what the gate cannot see: a write a service makes from Python — a drone verb, @memory's rollover, any `python -m` door — never passes through it, so every service that writes files must carry its own fence (memory's is the first, landing separately). Also: `tests/conftest.py` 2.2.0 isolates the fleet-shared `src/aipass/.diagnostics_state.json` (autouse), because every live seat's auto_fix writes it and a test Editing a `.py` picked up another seat's open import error and went red — very likely hooks' long-standing todo 49 flake. `edit_gate.py` 1.16.0 (1,495 → 1,464 lines), `bash_writes.py` 1.6.0, three test files. 19 new cases plus 1 reversed, all inside existing classes, 11 red before the cure; 12 mutants on a `/tmp` copy of the package, all killed. Measured from this seat: 2118 passed / 2 skipped, `audit aipass @hooks` 100. The gate reads source, so this was live fleet-wide from the working tree the moment it landed — expect first refusals from citizens that wrote project-level files or used `sed` into another branch.

- **hooks: grounding fails loud and small, every injection is on a ledger, a mail body is data, and two proposed cures refused on numbers** (DPLAN-0347 / FPLAN-0593 Phase 5, hooks FPLAN-0620; hooks owning, devpulse landing). **Degraded fail mode** (`cadence.py` 2.6.0): when turns cannot be counted, cadence used to force turn 0, which fires every loader (21,383 chars a prompt); now only `DEGRADED_LOADERS` fire (the kernel plus the alert and mail channels, 3,750 chars) and navmap, branch and identity are withheld, not replaced, with one WARNING per degraded turn. Grounding a seat was promised and did not get (a stamped tree with no kernel, a branch with no prompt, a loader that raises) opens the kernel block and part 1 of the post-compact re-ground with a `[GROUNDING DEGRADED …]` banner naming what was carried, what is missing and why; a tree that promises nothing stays silent (`grounding_content.py` 1.3.0, `tier0_kernel.py` 1.2.0, `post_compact_regrounding.py` 2.1.0). **Injection ledger** (`apps/modules/injection_ledger.py` + `apps/handlers/config/ledger_store.py`, both 1.0.0; `engine.py` 1.7.0): one JSONL line per dispatch that reached the model, per hook chars (UTF-16) and a short sha, grouped on cadence's turn token because parallel siblings can read a lagging turn number. Warn-only; it warns on an injection over the 10,000 persist line and on a record it could not write. `drone @hooks ledger` reads the caller's own session: the first bare read printed a neighbour's turns, because every citizen runs a live session. **Mail-body gate** (`git_gate.py` 1.1.0, `bash_writes.py` 1.5.0): the gate scans `code_text()`, which blanks a heredoc body whose consumer only reads it and hands back a shell's own program text tokenized, so prose quoting a write-shaped line no longer refuses a reply and `bash -c "<write>"` is no longer read as an empty quoted argument. A first version appended every interpreter's program and refused a read-only `awk` filter on a real turn; it is now shells only (bash, sh, zsh), and awk, perl and python programs read as they did before. **Template and cap message** (feedback c273274e): `.aipass/project_hooks.json` runs `pre_edit_gate` on Bash too, so new projects get the scripted-write fence and the memory shell refusal. That refusal names `drone @memory` and measured caps only where @memory imports (`edit_gate.py` 1.15.0). **Memory file writes:** an Edit or Write that would leave a `.trinity` file unparseable is refused (it was allowed through a catch-all), and an edit that repairs a broken file is allowed with a WARNING. The tripwire charges only a file that does not measure clean. A clean change, even to the seat's own file, is reported as a question and logged at INFO, because another seat's compaction rolls this seat's `local.json` mid-call. **MagicMock leak:** `test_log_write_failure_does_not_crash` handed a bare Mock path to `append_jsonl`, whose mkdir left `MagicMock/LOG_FILE/` in the branch root from 2026-07-10. The test now uses a tmp path, and a tree-level test fails if any mock-named directory appears in the branch root. The leaked directory went through `drone rm`. **Refused, with the numbers:** *cadence-by-default*, fenced as a restatement: DPLAN-0347 named 4 opt-outs, but this repo's 13 UserPromptSubmit entries need 9. A default lives in the engine every stamped project runs, and two projects carry `temporal` and `context_gauge` with no opt-out key, so those two would drop from every turn to every fifth. An 11-turn trace per handler is identical before and after this phase's other rows. *Tripwire grace-then-re-stat*: every memory writer is atomic, and the retained logs (09-12 → 09-16) held 2 real breakages, open 9 s and 8 s. A grace long enough to hide the false alarms hides those too, so the cure landed at the source instead (the unparseable-edit refusal above). `edit_gate.py` is at 1,495 lines, under the 1,500 split line only after its docstrings moved to `docs/`, and needs a real split. Measured: 2080 passed / 2 skipped from both the branch and the repo root, `audit aipass @hooks` 100, checklist 34/34 on every changed source, branch prompt 8,824 of 9,000.

- **memory: the todo backlog trims by moving deeper, never by deleting** (DPLAN-0347 / FPLAN-0593 Phase 5; memory owning, devpulse landing). The per-agent backlog grew forever and nothing trimmed it — seedgo's was already at 72 records. The trim is now a move: records past `rollover.backlog.max_records` (100, in `memory.config.json`) go to a sibling `backlog.archive.json` under the same order-of-operations contract the roll already uses — append to the archive, atomic replace, read back and compare every record, and only then shorten the backlog, so a record may sit in both files for an instant and never in neither. Nothing is dropped, because the backlog is the only copy of a rolled todo and is never vectorised: a trim that deleted would delete the last copy of open work. It hangs off `append_to_backlog`, the one choke point both the roll and the push cross, so it fires when the file grows rather than on a sweep nobody runs, and a failed trim never fails the append. No usable ceiling means NO trim, never a trim to zero — the failure mode is "the file stays long", never "the last copy is gone". `high_water` is not lowered, so no number is re-issued. Two consequences stated rather than discovered later: an archived record is not reachable by `todo restore`, which reads the backlog only, and the archive grows forever by design. Also landed: `entry_limits.draft_percent` is a config key (the module constant is gone and a pin keeps it gone), and `apps/modules/limits.py` is the modules gateway that was missing — @hooks and @seedgo both reached into the handler by dotted path because there was no door, and seedgo's mirrored copy can now retire.

- **seedgo: the per-file startup ratchet, and `host_leak` — the greeting can no longer grow back** (DPLAN-0347 / FPLAN-0593 Phase 5; seedgo owning, devpulse landing). `startup_ratchet.py` 1.0.0 in the context pack, wired into the existing `seedgo-audit` CI job ahead of the audit loop (`.github/scripts/seedgo_audit.py`, no new step, no `ci.yml` change): CI goes red when a branch's `README.md` or `.aipass/aipass_local_prompt.md` grows past its cap. Chars as `wc -m` counts them, never lines or tokens; a file AT its cap passes, over is `>`. Caps are READ from their owner at run time and no cap number exists in the gate — pinned by an AST walk asserting no `{10000, 9000, 6000, 15000, 25000, 600}` literal in either file: README 10,000 from seedgo's own `context_standards/pack.json`, prompt 9,000 from hooks' `BRANCH_CHAR_BUDGET`. Every failure line names four things — file, measured, cap, owning branch (`… measured 10,316 chars, cap 10,000 chars (OVER by 316), owner @seedgo (context_standards/pack.json → caps["README.md"].max_chars)`) — because a red that does not say who owns the file costs a seat a round trip; an unreadable owner cap fails loudly naming the owner and the exception, never falling back to a remembered number. Scope is deliberately today's measured pair: `.trinity` and the dashboard are machine-local and gitignored so CI cannot see them, and `docs/` is measured but not gated while the fleet still holds pages that predate the 20,000 rule. Proven both ways before landing: green across 18 branches / 36 gated files on head (tightest hooks README 9,116 and aipass prompt 8,968), and red against a copied fixture tree (grown hooks README 10,316 and aipass prompt 9,068 both failing, a README set to exactly 10,000 passing), the fixture removed and the live tree never grown. 28 pins in two existing test files, 28/28 mutants killed with sha256 restore proof. Also `host_leak` 1.0.0, the v5 pack's 16th rule, advisory and nominate-only (`SCORED = False`): a test that fakes a platform AND weaves a host path into composed text beside a literal — the Windows CI red where a test faked macOS, built its fake `lsof` line out of `tmp_path`, and the POSIX-only parser could not read the line the test itself wrote. The broad arms were measured and REFUSED with numbers: "fakes a platform + any host value" is 23 rows fleet-wide, all 23 hand-read, 0 real; "host path into composed text" alone is 223 rows; the shipped pair-shape is 0 rows on the fleet today, catches the pre-cure unit, and leaves its cured sibling clean. The other CI red's shape (a fixture depending on module-level state another test warms) was measured and refused too: 393 production module-level containers, 160 unseeded reads, 8 with a sibling seeder, 8 of 8 false, 0 recall on the defect it was built for — the refusal and its numbers are written into `host_leak.md` so nobody re-derives it. 22/22 mutants killed; 4 of them found real defects in the rule's own code first.

- **memory: the closed field shape, `lint fields` and the keep-count ceiling** (DPLAN-0347 / FPLAN-0593 Phase 1, thread 16; memory owning, devpulse landing). Every field of a `.trinity` entry is now capped, not just one: `memory.config.json` publishes `entry_limits.entry_types.<type>.fields` as `{type, required, max_chars, max_items}` — sessions `status` 40, key_learnings `key` 80, `tags` 10 items / 120 joined, todos `priority` 10, `date` 10 everywhere (the one cap the room did not name: a closed shape with an uncapped string is not closed; measured max is 10 on all 803 entries across 22 branches); `summary` / `value` / `task` / `note` unchanged — plus `file_budgets` (local 25,000, observations 15,000, passport 6,000 file / 600 per string, SIZE only: spawn owns the passport schema). Why: `todos.status` was the uncapped hole that grew devpulse's local.json to 64,657 chars (S464); the cap on one field per type left every other string free to become the next one. `changed_entries` returns two new reasons in the published six-key shape plus `field` and `units`: `unknown_field` (the field may not exist) and `field_over_cap` (chars or items); `check_file_budget` adds `file_over_budget` for the passport row; `missing_field` and `unmeasurable` reused. Field violations use a stricter identity than the canonical cap — an entry is carried only when the whole dict is in `before`, because a write that edits only `status` leaves the summary byte-identical and the text test would wave through exactly the edit this shape exists to catch. `drone @memory lint fields [@branch]` lists every string field by chars (max / p95 / n against its cap, passport row included) and flags what is outside the shape; fleet run 22 branches, 2,938 measurements, 5 outside — devpulse `status` 917 and 154, `key` 83, aipass `identity.purpose` 869, devpulse `identity.class_extension` 674 — all legal until touched. The keep-count ceiling: `config set` and `set-default` refuse, and load clamps, a count whose worst-case file (every entry at every cap, serialized) busts its budget — sessions 16, key_learnings 17, todos 14, observations 19 at today's co-tenants, the refusal naming the budget and the other two tenants; no shipped default clamps (15 ≤ 16, 15 ≤ 17, 10 ≤ 14, 15 ≤ 19), config md5 identical before and after a refused set. `trinity_push` derives its entry shape from the config — `ENTRY_RULES` is gone, seedgo's `trinity_groups` mirror is the last copy and goes in Phase 2. `todo restore` of a non-canonical backlog entry is now a refused write that leaves both files as found (the backlog contract forbids reshaping); the text stays readable via `todo backlog`. Files: `memory.config.json`; `entry_limits` 1.11.0, `config_loader` 1.5.0, `budget` 1.0.0 (new), `lint_handler` 1.3.0, `memory_files` 1.4.0, `trinity_push` 1.4.0, `tab_renderer` 1.6.0, `lint` 1.2.0, `rollover` 0.8.0 (1,498 lines against seedgo's 1,500 — the next addition needs a split). ~70 pins in six existing test files, mutants run and restored on seven handlers; 1808 passed in CI shape (`-n auto --dist loadscope`) and at `-n 3`; seedgo audit @memory 100. Found while building: the non-entry structure figures in DPLAN-0347 (local.json 3,254 / observations 2,068) were memory's own measurement and inflated by the indent method; correct is ~1,629 / ~1,306; the conservative 3,500 / 2,500 allowance stands, which is why sessions has one slot of headroom (16 against 15).

- **ai_mail: a dispatch wake refreshes the recipient's dashboard before the session spawns** (DPLAN-0347 / FPLAN-0593 Phase 3, ai_mail FPLAN-0599; ai_mail owning, devpulse landing). `wake.py` 3.2.0 calls `refresh_recipient_dashboard` (new handler `wake_dashboard.py` 1.0.0, which calls prax's `refresh_single_dashboard`) for the RECIPIENT branch at the one point past every gate and above all three spawn lanes (tmux manager, systemd scope, detached Popen), so the agent's first status glance is written before the process that reads it exists. Why: `DASHBOARD.local.json` is the single glance the startup protocol reads and nothing refreshed it on the way in — a woken agent read whatever its own last session left behind, "0 unread" while the dispatch that woke it sat in the inbox (canary's card was 3 h 29 m stale at the moment of the probe). One branch, never the fleet: `refresh_all_dashboards` walks 18 and the recipient is one of them. FAIL-OPEN, and that is the one exemption from failing honestly in this file — an import that cannot resolve, a raise, an error status prax RETURNED rather than raised, or a refresh still running at `DASHBOARD_REFRESH_TIMEOUT` (5.0 s against a measured 0.85–0.98 s) each record a prax WARNING naming the branch and a `warn` step, and the spawn goes ahead: a stale dashboard costs the woken agent one refresh, a dead wake costs the dispatch. The bound needs a thread (`signal.alarm` is POSIX-only and main-thread-only; this runs under the daemon as often as under a terminal), `daemon=True` so an overrun cannot hold the process open either. The import is local and through prax's module door (`aipass.prax.apps.modules.dashboard`), never the handler — a module-level import would make a broken prax dashboard an ImportError at ai_mail import time, which is exactly the trade the fail-open exists to refuse. Measured live (probe wake at @canary, 2026-09-15 17:28, haiku): refresh 0.33 s, `last_updated` 13:59:18 → 17:28:56.087, the register row opened 58 ms later at 17:28:56.145, the agent's first turn ~1.5 s after that (run completed 17:29:14 after 16.5 s) — the card is written before the agent exists, zero agent tokens. Files: `apps/handlers/dispatch/wake_dashboard.py` 1.0.0 (new — the helper went to its own file because `wake.py` measured 1,554 lines with it inline, over seedgo's 1,500; `wake.py` imports it under its old private alias, so every pin and stub keeps the name it had), `apps/handlers/dispatch/wake.py` 3.2.0 (1,464 lines), `test_wake.py` +6 pins in the existing file (red first: 6/6 red on the missing helper before the build). 6/6 mutants red, including "refresh below the tmux lane" and "fleet refresh instead"; 1498 passed both rootdirs; seedgo 34/34 on `wake.py` and 23/23 on the test file — it caught two of my own rows first, a silent catch in the worker (now logged in the thread that sees it, so the record exists even when the join times out) and a `/tmp` literal in a pre-existing darwin test.

- **drone: `drone @git commit` refuses an essay in the subject line** (DPLAN-0347 / FPLAN-0593 Phase 3, drone FPLAN-0600; drone owning, devpulse landing). `commit_handler.py` 1.2.0: a commit whose first line exceeds prax's `SUBJECT_CAP` (120) is refused with exit 1 — nothing staged, nothing committed — and the refusal text is the rule itself: subject under ~80 chars in `type(scope): what` form, blank line, the why in the body at any length; the refusal shows the offending length and the cap. Why: eight recent commits carried 1.3–4 KB single-line subjects with no body, and `git log --oneline`, the dashboard's `last_commit_msg` and the PR list all render the subject, so every glance carried the whole paragraph; prax capped the display this morning, but the door is where the shape belongs. The number is imported from `aipass.prax.apps.modules.dashboard`, never copied — a commit subject and a plan subject are the same glance — and the pin reads the import line out of the source, because `is`-identity on a small int passes even when 120 is hand-copied (that mutant survived until the pin changed). The gate runs first in `commit_changes()`: above repo resolution, above staging, above `--all`'s ruff and pytest lane (pinned: zero `subprocess.run` calls on a refusal), and the `--repo` external door inherits it. Second door found and capped: `pr_handler.py` 1.1.0's `create_pr` composes `feat(<branch>): <description>` and commits it, so it measures before `acquire_lock` — a refusal holding the repo-wide lock would block every citizen while it teaches. Leading blank lines are stripped before line 1 is measured, as git's own cleanup does. `git_module.py` 1.4.0 names the cap in `git commit --help` and rejoins `repo_door` to the handler import block (seedgo's dead_code 1.1.0 landed). Proven live from the devpulse seat: a 183-char subject refused, exit 1, HEAD unchanged. 11 tests in two existing files, 9/9 mutants red; drone suite 1370 passed both rootdirs; seedgo audit @drone 100. README: new section on the cap, Known Issues bullet closed.
- **aipass: the provider manifest carries a settings scalar, and doctor names the gap** (DPLAN-0347 / FPLAN-0593 Phase 3, aipass FPLAN-0601; aipass owning, devpulse landing). `.claude/provider_manifest.json` 1.1.0 gains `cli.claude.settings`, a map of TOP-LEVEL personal-settings keys and the values AIPass wants — first entry `includeGitInstructions: false`; the manifest is the single source and no default for the key exists in code. `provider_wire.py` 1.1.0 merges it through the verb that already writes hooks (`auto_wire_provider`, same dated backup, same atomic write): a key the file does not have is set, a key the user set to a DIFFERENT value is reported and left alone ("Kept your includeGitInstructions=true (manifest wants false) — your value wins, nothing overwritten"), and no other key in the file is read or written; scalars only (a nested block typed into the slot is dropped with a WARNING), with `false` and `0` kept distinct so a 0 cannot read as a satisfied `false`. `_doctor_wire.py` 1.1.0 / `doctor.py` 1.1.0 give `aipass doctor` one diff row per manifest key — `manifest wants false, provider has MISSING` / `… provider has true` — each carrying the exact cure (`Run: aipass doctor --fix`; for a conflict, the line that says the human value stands and names what to drop first), and the interactive/`--fix` gate now reads the settable gaps instead of the glyphs, so a value the user set never makes doctor offer to wire over it. Why: `includeGitInstructions` is effective ONLY in the personal settings file (the project copy is ignored, S470; personal proven effective, S471), Claude Code injects a ~4k-char gitStatus block every session without it, and no installer wrote the key while the manifest had no slot for a scalar — doctor could not even see the gap, so a fresh clone regressed ~4k tokens silently and DPLAN-0346's DONE bullet on this was wrong. Found while building: the interactive run calls the manifest check twice and the replace filter stripped only the fixed labels, so a settings row — labelled with the manifest KEY — printed twice; `merge_manifest_rows` now replaces every label the fresh pass carries. Proven on a scratch HOME end to end (MISSING named with the cure → the wire verb sets it → doctor reads it as wanted → a hand-set `true` reported with zero writes, the seeded `model` key untouched); the live `~/.claude/settings.json` was never opened for write. 13 pins in two existing test files, 6/6 mutants red, 1114 passed; seedgo 34/34 on all three sources and audit @aipass 100 — it caught a module doing `json.loads` on the settings file first (now `json_handler.read_json`). Also closed: `test_doctor.py` stubs the sandbox checker so the suite no longer runs real `npm` (3 logs per run in `~/.npm/_logs` → 0). Both wire doors now merge the slot: `refresh_provider_hooks` at install (setup.sh's only wire door, `provider_wire.py` 1.2.0) and `auto_wire_provider` in-process, so a fresh clone gets `includeGitInstructions` at install rather than at the first `aipass doctor --fix` — proven on a scratch HOME through setup.sh's heredoc verbatim, including an upgrade re-run that reports a hand-set `true` and leaves it.
- **aipass: the test-write policy resolves the project root, not bare cwd** (DPLAN-0347 / FPLAN-0593 Phase 3 follow-up). `init_flow.py` 1.3.0: `_get_test_write_policy_path` resolves `find_registry()` and stamps at `<registry root>/.aipass/test_write_policy.json`, falling back to cwd only when no registry exists (a fresh install, where cwd IS the project root). Why: bare `Path.cwd()` meant any suite run from a branch directory created a SECOND policy — `src/aipass/aipass/.aipass/test_write_policy.json`, one byte off the root file (json_handler writes no trailing newline) — and @hooks' gate walks UP, so the nearest copy silently outranks the fleet's live ruling. Second occurrence of the same door (DPLAN-0337 R5 removed the first). Reproduced from an empty tmp cwd, cured, re-measured: the same run now leaves the directory empty. `test_init_flow.py` 1.1.0 adds an autouse floor pointing the policy into tmp_path for the whole file plus three pins; 2/2 mutants red. The stray copy could not be `drone rm`'d (`.aipass/` is a hard carve-out with no override); on Patrick's word it was moved to aipass's untracked `.archive/` with a dated stray suffix.
- **aipass: the structure scanner reads project-registry residency** (DPLAN-0347 / FPLAN-0593 Phase 3 follow-up). `structure_scanner.py` 1.1.0 adds `resident_root()`: the first ancestor holding a `*_REGISTRY.json` that LISTS the agent (by name, case-insensitive, or by a path row resolving to its directory) is that agent's home, and `check_placement` measures each agent — and reads package names — against that home, falling back to the scan root. Why: a citizen of a project under `projects/` legitimately lives outside the framework's `src/`, and measuring it against the framework root produced one WARNING per scan forever — @trigger escalated `captured_structure_scanner` four times in an hour, 28 lifetime since 2026-08-28, and the real count was 4 per scan (aipass_site, baud, earmark, finch). Measured: 4 WARNING lines on the last pre-fix scan, zero on the two after, all four resolving to their own project roots. Residency is a listing, not adjacency: a registry listing someone else is no cover, and an unregistered citizen outside `src/` is still flagged (both pinned). 5 pins in `test_structure_scan.py` 1.1.0, 2/2 mutants red. aipass suite 1124 passed, audit 100.
- **spawn: the birth contract — a newborn starts inside every owner's cap, and `update` reports `.md` drift instead of falling through it in silence** (DPLAN-0347 / FPLAN-0593 Phase 3, spawn FPLAN-0602; spawn owning, devpulse landing). Template split: `templates/citizen/README.md` is the face (purpose, `drone @self` for the live inventory, a `docs/` index; the command list and the Three-Layer block are gone), `.aipass/aipass_local_prompt.md` is breadcrumbs under a contract comment naming hooks' `BRANCH_CHAR_BUDGET` as the cap's owner, and the `docs/README.md` stub carries the per-module convention (one file per module or handler group, under 20,000 chars, indexed from the README). Newborn startup footprint 11,277 → 9,369 chars (README 2,089 → 1,711, prompt 4,153 → 2,376); `drone @seedgo audit context` on a scratch newborn shows no column over cap. `update_ops.py` 2.2.0 adds `_md_state()`: every existing `.md` is compared against the rendered template and answered as matches / differs / unreadable with a line per file in the preview (`update.py` 1.2.0) — never a write, because the README diet is owner judgment (DPLAN-0199); measured on canary as 7 differ, 16 match. `test_birth_receipt.py` 1.1.0 pins the newborn under every cap, the passport per-string cap, the dashboard rendered at birth from the template (no call to flow or prax), and caps-never-copied — every cap is read from its owner at assert time (seedgo `pack.json`, hooks `BRANCH_CHAR_BUDGET`, memory `file_budgets`, prax `DASHBOARD_CHAR_BUDGET`) and a hand-copied literal fails the suite. `test_update.py` 1.2.0 pins the `.md` arm as read-only. Found on the way: the hooks edit gate already refuses a 643-char passport principle in the template itself — the per-string defect cannot be authored through the Edit door at all.
- **seedgo: the `startup_budget` pack — the greeting is a measured number, not a percentage somebody notices** (DPLAN-0347 / FPLAN-0593 Phase 2, seedgo FPLAN-0598; seedgo owning, devpulse landing). `drone @seedgo audit context [@branch]` renders the fleet table: one row per branch, README / prompt / local / observations / passport / dashboard in chars (`wc -m`, never bytes or tokens) against each layer's cap, over-cap marked. Branch-level, ADVISORY (gates nothing; the per-file CI ratchet is Phase 5). Caps are read from their owner at call time, never copied: README 10,000 from `context_standards/pack.json` (Patrick 16:02 — the one number seedgo owns, in JSON so the ratchet can read it without importing Python), prompt 9,000 from hooks' `BRANCH_CHAR_BUDGET`, dashboard 6,000 from prax's `DASHBOARD_CHAR_BUDGET`, `.trinity` 25k / 15k / 6k + 600 from `memory.config.json entry_limits.file_budgets`; an unreadable owner is an ERROR row, never a default; `external_inputs()` declares memory's config so the incremental cache cannot serve a stale row after a cap moves. The pins are behavioural, not identity — monkeypatch hooks' or prax's constant and the row must move, so a hand-copied literal goes red (CPython interns small ints, so `is` cannot tell an import from a copy; drone hit the same today). Measured on the fleet (final run): 17 of 18 READMEs over 10,000 — memory 95,704, api 92,978, ai_mail 85,110, prax 77,953, drone 69,040, hooks 68,126, daemon 62,147, trigger 57,729, flow 46,285, spawn 36,300, seedgo 35,464, skills 30,490, backup 25,700, aipass 23,660, commons 22,849, cli 15,493, canary 13,370; devpulse 4,742 the only one under — every prompt under 9,000 (hooks 7,960 highest), every `.trinity` file under budget, two passports over the 600-char per-string cap (aipass `identity.purpose` 869, devpulse `identity.class_extension` 674). Startup_Budget fleet average 71%, every branch still Overall 100: advisory working. Files: `context_standards/{pack.json, startup_budget_check.py 1.0.0, startup_budget_content.py 1.0.0, startup_budget.md}`; `audit_display.py` 1.2.0 gained one generic fleet-table arm (returns early when no checker publishes a `fleet_row`, so other packs' output is byte-identical); one name-scoped bypass for `measure_corpus` (called only through pack.json's `measured_by` string, which `unused_function` cannot see). 22 new test functions in existing files, 23/23 mutants red; seedgo suite 4188 passed from the repo root. Deviation, ruled acceptable: seedgo reads memory's config key directly rather than importing memory's handler (a cross-branch handler import fails its own encapsulation standard and memory publishes no modules gateway for these) — a one-line memory gateway is a Phase 5 row.

### Changed

- **the fleet's docs pages take the standard's shape: a back-link on every page, the move stamps gone, the defect registers retired** (DPLAN-0351 wave 0; devpulse landing, eight sub-agents). Two rulings from the owner this morning. On the 16 `docs/known_issues.md` / `docs/tech_debt.md` pages (born in the 09-15 README diet, defect paragraphs by definition, cured entries still standing on 13 of them): "retire them. unless, tech depth and maintenance nightmare is what they are move to docs.local." So each moved, unchanged, into its branch's untracked `docs.local/`, and the 59 links to them in 52 tracked pages went: index rows deleted, Related bullets deleted, prose pointers trimmed to the claim they carried (drone's `testing.md` lost a Related heading that had held only that bullet). Open defects live on pads and in plans from here; a stranger sees the CHANGELOG and a module page's "What is not verified". Then, on "can haiku subagents add the back link? all files": 110 pages in 16 branches got `[<- Back to the README](../README.md)` as their first line, measured against the checker's own rule (a README link above the purpose paragraph), the 67 "Moved out of README.md on 2026-09-15 (DPLAN-0347…)" provenance stamps went as history the CHANGELOG already carries, and daemon's ten below-the-purpose links moved up. After the wave the back-link advisory reads 8 of 160 pages: spawn's six (spawn awake on its scaffold build, its own pass follows) and canary's two live edits. Audit 100 on all 15 branches touched, Docs_Page and Readme rows included, every `docs/*.md` still reachable from its README. Seedgo's follow-through on its next wake: the `known_issues`/`tech_debt` exemption in `docs_page_check` becomes a red (the register must not come back), and the back-link lane goes from advisory to scored once spawn's six are in.
- **every branch README is on the eight-section shape, and every newborn starts on it** (DPLAN-0351 README wave; devpulse landing, three sub-agents on 16 READMEs, devpulse's own by hand; spawn owning its template). The owner, 2026-09-19: "expecting same structure throughout." Seventeen READMEs move to Quick Start · What It Does · Live Inventory · How To Reach Me · Commands · Architecture · Documentation · Integration Points: sections moved whole and verbatim, headings renamed to the exact names (`What I Do`, `What It Is` → `What It Does`; `Integration` → `Integration Points`), the H1 in capitals, `**Last Updated:**` moved from the head to the foot between two rules above the back-link, and a fleet-boilerplate `Live Inventory` (self-map and `--help`, generated, never copied) or `How To Reach Me` (mail, dispatch, what to report) added where a page had none. No "What It Does" was written by anyone but its owner: cli's is missing and stays nominated in its audit, the cli owner's content. Devpulse's README was rewritten onto the shape by hand, the watchdog paragraph corrected to the one-shot background wire. Spawn's `templates/citizen/README.md` takes the same eight sections with the foot date rendered from the mint placeholder; two pins in `test_template_hygiene.py` parse the section list out of seedgo's `readme_check.py` at assert time so the pin moves when the shape moves, and every relative link in a newborn README is checked where the citizen lands. The template carries no foot back-link on purpose: a citizen minted at any depth in any project has no fixed path to its project README, and spawn proved the hard-coded fleet link dangles for a newborn elsewhere. Measured after: 18 of 18 branches read Overall 100 with Readme 100, one `readme sections` nomination (cli); spawn's suite 993 passed.
- **aipass's two research memos leave `docs/`** (DPLAN-0351, the owner's ruling 2026-09-19 09:48: "move the research paper to docs.local"). `test_quality_tooling_research.md` (128k chars, 6.4× the docs page cap) and `test_suite_governance_research.md` (37k) were studies written for a design session, not the branch's depth, and they were the two worst pages in both DPLAN-0351 surveys of the fleet's 179 `docs/*.md` pages. They now sit untracked in `src/aipass/aipass/docs.local/sub_agent_drops/`, the compass #12 home for research output; the two index rows and the "predate the page cap" apology left `docs/README.md` and the branch README. Audit 100, readme docs index 11/11 linked. Seedgo's own `docs/v5_vs_haiku_shadow_diff.md` (15.6k) is the third and moves under seedgo's FPLAN.
- **the owner's name is out of every string a seat is shown — prompts, config, rendered standards, refusals, skip reasons** (DPLAN-0350 wave 1; devpulse landing across nine branches). The rule, the owner 2026-09-15 and again 2026-09-19: the public tree never names him, because a seat that reads his name in a prompt or a config note on another user's machine is told who its user is and gets it wrong for everyone. The culture doc `.claude/CLAUDE.md` is history and stays; the CHANGELOG and plan files are history and stay. Swept in this wave, every place the text ships at runtime rather than sits in a comment: devpulse's and seedgo's branch prompts (5 lines), devpulse's `passport.seed.json` (3), the root `.aipass/project_hooks.json` template comment and `.aipass/test_write_policy.json` note, `aipass init`'s preview line and its source comment, hooks' testwrite refusal, testgate policy explanation and release notice, devpulse's release-notify compose text and the admin grant's `granted_by` default (`"owner"`; the live untracked birth certificate keeps its minted value and no verifier reads the field), api's one-door refusal, spawn's registry-privilege message, trigger's log line, memory's PARKED import error and its pinned skip reason, and seedgo's rendered standards (gateway boundary, host state, and the hardcoded-path example paths now read `/home/someone/…`). The paired test literals moved in the same commit (`test_init_flow`, `test_admin_grant`, `test_testwrite_gate`, `test_post_compact_regrounding`, `test_symbolic_extras`). Attribution reads "the owner"; text addressed to whoever is at the keyboard reads "the user". Edit tool only, 24 files, 34 lines. Measured: hooks 2118, devpulse 1721 (with aipass), api 1711, spawn 982, trigger 1086, memory 1895, seedgo 4309 passed; the five failures across those runs were watchdog, host-attach and reload-sentinel timing tests under parallel load, each green alone. Audit 100 on aipass, devpulse, hooks, seedgo. **Waves 2–5, the same night:** the comment, docstring and doc-prose attributions across every branch, swept by haiku sub-agents one branch each (twelve agents over four rounds) from exact `git grep` line lists, Edit tool only, every diff read from this seat before commit, every branch suite run (one at a time after the first parallel round stalled the disk: iowait 55%, api's git-status lane timed out, trigger escalated it). 289 files, ~560 lines, in `d77416dd`, `30f25cb5`, `165c7746`, `1488e7c9`. Fixture values made neutral only where the test controls both sides (a token label, a Windows cwd, `granted_by`, a shell test's typed name); flow's plan templates and seedgo's rendered standards included; absolute home paths in docs became `<owner-repo>` or "the repo root". Refused or reverted on the way: the api settings-conformance corpus notes line (byte-pinned by `manifest.json` and mirrored in baud, api's call), 11 hooks fixtures whose real home paths feed a gate that may walk the filesystem, 16 seedgo hardcoded-path checker fixtures (the test data the checker must catch), the suspend tool's polkit, sudoers and sleep hook keyed to the real Linux account, and four test identifiers. Fleet count 682 hits in 316 files → 134 in 20, of which 83 are the tracked hook run logs and 6 are `.claude/settings.json`'s personal deny rules — both the owner's call, listed in DPLAN-0350 Notes with the suspend files. The ratchet (a seedgo advisory or CI grep so the count cannot regrow) is briefed to seedgo and not built.

- **kernel: the cross-branch rule says what it is about — an AI editing another AI's files — and names services as the framework running** (DPLAN-0349; the owner's words 2026-09-18 23:46, added on his go at 23:57). The kernel's line was `No cross-branch file edits. Issue in another agent's code → mail the owner.` and nothing anywhere in the injected prompts distinguished an agent's edit from a service's write. So when a log showed @memory's rollover code writing another project's `.trinity` files, the only rule a seat had matched it and the seat called it a breach — which sent a night into a boundary program over service doors that the owner then pulled back: AIPass agents offer services (mail, prax logs and dashboards, the daemon's scheduled wakes, rollover) and projects import those services to use them; a service writing into the project it serves is how AIPass runs, not an agent crossing a line. The line now carries that distinction in one sentence each way. Kernel measured 3,034 → 3,251 chars with its comment block (the 2,000 target is the open step-5 ruling in FPLAN-0622).
- **cli 2.3.0: `escape` is published on the display surface, and the surface is measured as split** (cli owning, devpulse landing; night rounds 09-17). `from aipass.cli import escape` (also on `aipass.cli.apps.modules`) is Rich's own function, bound, not wrapped — twelve tests went red on the missing export first. Measured before publishing: `header`, `success`, `section` and `operation_start/complete` parse markup in their values, so a bare `[count]` is eaten and `escape()` fixes it; `error`, `warning` and `fatal` build `Text` and print literally, so `escape()` there puts a visible backslash on screen. Both halves pinned (`TestEscapeExport`, 11 cases; two mutants each failed exactly their own cases). `docs/rich_markup.md` carries the rule — escape the data, do not disarm the line — and the split table; cli's own `--help` had the case (its example lost its `[bold]` tags) and is hand-escaped. Logged in `docs/known_issues.md`, not decided: whether to unify the split, which would change what existing fleet calls render — a fleet ruling. 190 passed, `audit aipass @cli` 100.

- **seedgo: the last mirrored number retires — the draft percent is READ from `memory.config.json`, never carried** (DPLAN-0347 / FPLAN-0593 Phase 5 follow-up; seedgo owning, devpulse landing). `trinity_groups.py` 1.2.0 → 1.3.0. `_DRAFT_PERCENT = 80` existed only because @memory published the percent as a module constant with no key to read; it publishes `entry_limits.draft_percent` now, so the number comes out of the same config dict as the caps and the entry shape — no import, no second source, nothing new for the audit cache to watch. **Never assumed:** a config that publishes no usable percent (absent, not an int, outside 1–100) gets no guessed one — the tab renders `draft to <entry_limits.draft_percent unpublished>` and the Meta lines group refuses whole, naming the key and its owner. @memory's own loader narrows to its regeneration seed instead, which is its call about what it writes, but a checker that quietly agreed with a seed would be scoring files against a number nobody published. The retired guard (`test_the_draft_percent_is_memorys_own_number`, a pin that only existed to keep the mirror honest) becomes a parity pin reached through @memory's new `apps/modules/limits.py` door rather than the handler import seedgo's own `check_handler_independence` sends cross-branch callers away from — the module is held, never the symbol, so a signature change surfaces as an `AttributeError` at the call site. 14 new pins: 5 that the tab moves when the published percent moves (80/50/100/1/33), 7 that an unusable value renders the marker and never a number, 1 that the group refusal names the key, 1 AST walk proving 80 is not a literal in the module. Test file 1.3.0 → 1.4.0; `trinity.md` Live v1.1 → v1.2 points at the key instead of restating 80%. Measured: 5/5 mutants killed with sha256 restore proof, suite 4,270 → 4,284 passed / 53 skipped, `audit aipass @seedgo` 100 overall with Trinity 100 and no type errors, checklist 34/34 and 23/23.

- **memory: `rollover.py` split at 1,498 lines, two under the hard fail, and `todo restore` now names the field it refuses** (DPLAN-0347 / FPLAN-0593 Phase 5; memory owning, devpulse landing). The module sat two lines under seedgo's 1,500-line hard fail, so the next real edit would have failed the audit — the split came before the next feature rather than after it. 1,498 → 899: the config verbs to `apps/modules/rollover_config.py`, the shared JSON emitter to `apps/modules/rollover_json.py`, moved code being the code that was there, with `rollover.py` re-exporting every name so `rollover.<name>` still resolves. They went to `modules/`, not `handlers/`, because the cli standard hard-fails a handler that prints and the config verbs are almost nothing but printing. The restore refusal stays a refusal; its text changed. A pre-write check now measures the whole entry against the closed shape before anything is written and names every broken rule in one sentence, so a seat fixes it in one pass instead of bisecting — the sentences come from the gate's own renderer, so the pre-flight cannot disagree with the gate it predicts. It reports and never repairs, pinned as such, so a later restore cannot quietly pre-answer the open ruling on legacy pads. **Found while splitting, and the same species as both of tonight's CI reds:** each rollover test harness imported the real module one line BELOW its cli stand-in, so the module's console bound to a mock and stayed cached past teardown, and 191 tests then printed nothing while still passing. Serial order only — `loadscope` puts the two files on different workers, so no CI run has ever seen it. Cured in both harnesses with the reason written above the line. **Left at 99% here, and that was wrong — cured in the commit below.** `rollover_config.py` was parked with a named finding rather than resolved, on the reasoning that a bypass to restore a number was worse than an honest 99%. That reasoning never checked the gate: the `seedgo-audit` job has hard-failed any branch under 100% since 2026-06-05, so the decision did not park the finding, it turned CI red. The follow-up bullet has the cure and the corrected diagnosis.

- **seedgo: Rich_Markup follows the literal to the print site** (DPLAN-0347 / FPLAN-0593 Phase 5; seedgo owning, devpulse landing). `rich_markup_check.py` 2.0.0 (+ `rich_markup_content.py` 2.0.0 and a `rich_markup.md` that did not exist) reads two hops the old checker could not: a module-level literal printed by name elsewhere in the file, and a function whose returned literals are printed at a call site in the same file, concatenation read through at both levels. Why: the checker read the PRINT SITE only, so a help page assembled as a returned literal in one module and printed by a caller scored 100 while eating its own placeholders — reproduced red-first by running the transcribed 1.0.0 scan against the uncured file (0 violations, score 100) and the new one (score 0, 29 eaten tokens: `[args]`, `[global]`, `[count]`, `[path]`, `[owner]`, …), with the cured file scoring 100 under both. Second family, from the fleet rule the cli branch published tonight (escape the data, do not disarm the line): a `markup=False` call whose literal carries a tag Rich would have styled is now flagged, because it prints the tags, as is a hand-escape stranded under `markup=False`. The limits are stated rather than papered over, each with its own silence pin: one file, one hop, no chaining, no locals/`join`/`%`/indexed access, no Rich renderable wrappers, no names bound twice — measured as 264 of 5,570 print-site arguments (4.7%) staying unreadable, and a deliberately over-generous cross-file probe finding 0 eaten tokens fleet-wide, which is why call-following was refused. Hand-escaping at a normal print site is NOT flagged: for a styled page that must also show a literal placeholder there is no other spelling, so the demotion lives in the guidance, not in a red. Fleet before/after: 17 branches unmoved at 100, one real finding on one file in one branch (a module-level help constant whose `[opts]` the terminal eats, verified through a capture console), 0 false positives; not cured here — the file is not this branch's. 23 new pins asserted against rendered bytes from a real Console, 84 passing in the existing test file, 24/24 mutants killed with sha256 restore proof.

- **seedgo: the README diet, the fleet's test case — the face is 5,346 chars, was 35,464** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, seedgo FPLAN-0603; seedgo owning, devpulse landing). Under the 10,000 Patrick ruled at 16:02 and scored 100 by seedgo's own `startup_budget` pack (Startup_Budget 70 → 100). README 2.0.2 → 2.1.0. Nothing deleted: the 47-standards table and how a rule is added → `docs/aipass_standards.md` (9,960), the v5 pack → `docs/pytest_quality.md` (3,447), the caps and their owners → `docs/context_standards.md` (3,440), discovery / cache / info channel / bypass / `.seedgoignore` → `docs/audit_engine.md` (7,191), the per-file lane and hook roster → `docs/checklist_and_hooks.md` (3,310), proof / test_map / inventory / shadow cycle and the test counts → `docs/proof_and_coverage.md` (5,414), Known Issues → `docs/tech_debt.md` (8,202); `docs/README.md` 391 → 1,620 is the index, all nine docs linked. The command list is replaced by `drone @seedgo --help`, the two dated snapshots by `drone @seedgo audit aipass @seedgo` and `audit context @seedgo`, and the directory tree by the branch prompt, now its only copy (prompt 5,913 → 6,327; three stale counts and a wrong `config/` line corrected). Advisory docs lane on seedgo: 2 unlinked / 1 absent path / 36 count claims / 2 dated headings / 33 invocations → 0/0/0/0/0. The transferable recipe — the scored README checks and the advisory lane pull in opposite directions and both can be satisfied — is written into the README and `docs/audit_engine.md` for the sixteen branches behind it. `trinity_content.py` 1.1.0 → 1.2.0 and `trinity.md` (Live v1.1) stop restating @memory's entry shapes and point at `entry_limits.entry_types.<type>.fields`. Verified: `audit aipass @seedgo` 100 on all 48 consulted entries, suite 4188 passed / 53 skipped from the repo root, ruff clean.
- **memory: the README diet — the face is 7,703 chars, was 95,704, and `--help` was wrong** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, memory FPLAN-0604; memory owning, devpulse landing). Depth moved to `docs/`: eleven files, one per module or handler group, each under 20,000 and each linked from the README (`rollover_pipeline`, `trinity_standard`, `trinity_push`, `todos_and_backlog`, `config_verbs`, `templates_and_tabs`, `quality_and_proof`, `known_issues`, `cli_surface`, `vector_search`; `todos_v2_shape_contract` existed and is now indexed); `docs/README.md` is the index. The directory tree and the gotchas moved to the branch prompt (1,643 → 6,763, under the 9,000 cap). The hand-typed command list is replaced by `drone @memory` and `drone @memory --help` — and `--help` was fixed first: it named neither `lint fields` nor four live verbs (`health`, `roots`, `fleet`, `governance`) that only the README listed, so the README's list had been masking the generated surface's rot. `apps/memory.py` 1.1.0 → 1.2.0 (the command table and the footer). Three dated Last Updated blocks and every current-state count claim are gone; dated past measurements stay as dated facts. Every string leaving the README was grepped across tests first — none pinned it. Startup_Budget 70 → 100, audit aipass 100 in every category, checklist 39/39 on `memory.py`, 1808 passed / 2 skipped from the repo root.
- **ai_mail: the README diet — the face is 7,196 chars, was 85,110** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, ai_mail FPLAN-0606; ai_mail owning, devpulse landing). Third through the diet after seedgo and memory. `README.md` 2.0.0 is purpose, Quick Start (one fenced block), What It Does, Live Inventory (bare `drone @ai_mail` and `--help`, generated from the code that runs them), How To Reach Me, a Commands section that is a pointer with zero invocation lines, an Architecture paragraph naming the modules and handler groups, the `docs/` index and Integration Points. Depth moved to seven new pages, each under 20,000 and each linked: `docs/identity_and_boundaries.md` 17,673 (sender identity, the verified-caller rail, cross-project mail and wakes, the import guard, the case-exact registry walk), `docs/dispatch_register_and_feed.md` 13,842, `docs/wake_pipeline.md` 13,665 — which also documents the Phase 3 recipient-dashboard refresh, shipped that morning and never written down — `docs/wake_lanes.md` 11,044, `docs/sending_and_delivery.md` 9,796, `docs/known_issues.md` 6,532 (open defects, all registered in APLAN-0006, plus the bypass registry), `docs/cli_contract.md` 5,645; `docs/README.md` 1,005 → 1,195 is the directory's own contract. The branch prompt takes the tree and the gotchas, 4,267 → 6,496 under the 9,000 cap, and the tree was re-derived from the real one: it had been missing `register.py`, `report.py`, `session_pointer.py`, `wake_dashboard.py`, `verified_caller.py`, `help_flags.py`, `contacts.py` and `email_send.py`, and still described `notify.py` as desktop notifications. Deleted rather than moved, because each was stale the day after it was written and each is answered live by a command: the test-count essay (two counting methods, an unverified fresh-checkout skip claim, a retired "Battle Tested" badge), the per-test-file inventory, the bypass-registry arithmetic, and the Status/Seedgo/Tests header line. The `--help` diff memory's pass made mandatory found two live surfaces the hand-typed list had been hiding: `--from`, the explicit sender override, was absent from `email --help` FLAGS, and `view latest` was absent from its USAGE line — both added to the generated surface (`apps/modules/email.py` 3.4.0 → 3.5.0) rather than kept on the page. Measured: Startup_Budget 70 → 100 with every startup file in budget; audit aipass @ai_mail 100 with Readme and Readme_Quality 100; checklist 34/34 on the changed source; the advisory docs lane reads all nine `docs/*.md` linked, with no named-path, count-claim, dated-heading or invocation line; no test pinned any string leaving the README (grepped first); 1498 passed from both rootdirs.
- **api: the README diet — the face is 6,255 chars, was 92,978, and `--help` had never named the host surface** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, api FPLAN-0605; api owning, devpulse landing). Depth moved to nine new `docs/` files, each under 20,000 and each linked from the index: `decisions.md` 14,108 (the branch's incident narrative, moved verbatim), `host_surfaces.md` 17,668, `host_api.md` 16,236, `internals.md` 8,450, `host_autostart.md` 8,158, `git_surface.md` 7,955, `git_remote.md` 6,010, `tech_debt.md` 5,950, `clients.md` 1,521; `docs/SECURITY.md` is now linked too. The 25,170-char remote-lane section was one section with two subjects — that is why it was over the cap — and was split by subject into `git_remote.md` and `host_surfaces.md` rather than squeezed; the sub-agent asked to squeeze it reached 23,325 by wording alone, refused to delete claims to go further, and was right. The directory tree and the gotchas moved into the branch prompt (2,243 → 5,097). Deleted, not moved: the badges and counts — the old README claimed 1499 test functions / 1602 cases against a suite that runs 1708, and a different stale pair two paragraphs later. `drone @api --help` named neither `host-api` nor `integrations`, so the whole host surface was reachable only through the README table about to be deleted: `apps/api.py` 1.0.0 → 1.1.0 adds both rows, a host-api example block and the trailing command line. Measured: Startup_Budget 100, audit aipass 100 with Readme and Readme_Quality 100, checklist 39/39 on `api.py`, docs lane 11/11 linked with no advisory line, 1708 passed in CI shape.
- **prax: the README diet — the face is 7,392 chars, was 77,953** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, prax FPLAN-0607; prax owning, devpulse landing). Nothing rewritten from memory: the depth was sliced out by line range, so every claim in `docs/` is the text that was in the README, and the only edits after the move were the cross-references that pointed at sections which had moved. Nine new `docs/` files, one per module or handler group, each under 13,300 chars so one Read holds it: `logging_api.md`, `json_service.md`, `discovery.md`, `monitor.md`, `architecture.md`, `log_audit.md`, `dashboard.md`, `tests.md` (case counts replaced by the run that produces them; the per-file coverage column stays because it does not rot), `known_issues.md`, beside the existing `dashboard_caps.md`; `docs/README.md` is the index. The directory tree left the README for the branch prompt (3,725 → 6,491) and was re-derived from the real tree — the old one listed five command modules where there are seven and never mentioned `apps/plugins` or `apps/integrations`; the gotchas that lived only in prose (the refresh drops top-level keys, `sections.flow` has two writers, the instance lock guards the relay only, the registry never prunes) are the prompt's Gotchas section now. The dated Status block is gone, replaced by the commands that produce it. The `--help` diff before deleting the command list was empty — the first branch where no verb lived only on the page, and the generated surface carries more than the README ever did — so no source changed and the entry point keeps its version. Measured: Startup_Budget 100, audit aipass 100 with Readme and Readme_Quality 100, docs lane 11/11 linked with no advisory line, 1555 passed.
- **drone: the README diet — the face is 6,780 chars, was 69,040, and `drone @git <verb> --help` had been eating its own placeholders** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, drone FPLAN-0609; drone owning, devpulse landing). Ten new `docs/` files, one per module or handler group, each under 11,000 and every one linked from the README: `routing_and_resolution.md`, `caller_identity.md`, `subprocess_timeouts.md`, `git_access.md`, `git_interface.md`, `external_repo_door.md`, `rm_and_the_record.md`, `broker.md`, `testing.md`, `known_issues.md`. The directory tree and the gotchas moved to the branch prompt (4,186 → 7,252) and the tree was re-derived with `find`: the old one claimed an `integrations/` directory that does not exist, a `hook_sounds (DISABLED)` plugin whose files are gone, and never listed `templates/`, which does. Every dated snapshot number is replaced by the command that regenerates it. Two fixes came out of deleting the hand-typed list: `drone --help` now names `rm --stale` (`apps/drone.py` 1.2.2), and `drone @git <verb> --help` no longer renders through Rich markup, which had been reading `[count]` and `[path]` as style tags and deleting them — "log [count]" reached the terminal as "log" while `drone @git --help` rendered the same page correctly (`apps/modules/git_module.py` 1.4.1, `markup=False`). Seedgo's Rich_Markup standard scored the branch 100 with the page wrong, because it reads the print site and the literal is returned by `get_help()` in the same module — mailed to seedgo. Measured: Startup_Budget 70 → 100, audit aipass 100 on all 48 standards, checklist 39/39 and 34/34 on the two changed sources, docs lane 11/11 linked with no advisory line, 1370 passed in CI shape.
- **hooks: the README diet — the face is 9,116 chars, was 68,126, and the `engine` verb the README was the only witness to is in `--help`** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, hooks FPLAN-0608; hooks owning, devpulse landing). Thirteen new `docs/` pages sliced by line range, each under 8,300 and all sixteen `docs/*.md` indexed: `wiring.md`, `engine.md`, `diagnostics.md`, `project_config.md`, `git_gate.md`, `edit_gate.md`, `testwrite_gate.md`, `bash_writes.md`, `trinity_memory_gate.md`, `prompt_injection.md`, `sandbox.md`, `boot_shim.md`, `known_issues.md`. The scripted-lane section was one section with two subjects and was split, not squeezed: the shell reader and both Windows path spellings in `bash_writes.md`, the memory refusal and the tripwire beside the `.trinity` caps they share a handler with in `trinity_memory_gate.md`. The directory tree is re-derived and lives only in the branch prompt (7,960 → 8,735 of 9,000 — the honest tree is bigger than the abbreviated one it replaced); it gained `sound.py`, `release_notice.py` on both layers, the full modules list and `docs/`, and lost two count claims. Deleted, not moved: the "Start here" block that pointed a stranger at the branch's private `.trinity/`, four struck-through closed rows whose record is in their plans, and the `engine missing from --help` row — because the `--help` diff found `engine` live and unlisted, and it went into `HELP_COMMANDS` (`apps/modules/engine.py` 1.6.0, `apps/hooks.py` 1.3.0) rather than staying on the page. One real test pin moved with the text: `test_live_config_timeouts.py` asserted the project template's `_comment` pointed at the README's "template ruling" section; the pointer in the repo-root `.aipass/project_hooks.json` and the assertion now name `docs/project_config.md`. Measured: Startup_Budget 100 with no red, audit aipass 100 in every category, checklist 34/34 and 39/39 on the two changed sources, docs lane 16/16 linked with no advisory line, 2047 passed / 2 skipped.
- **daemon: the README diet — the face is 6,655 chars, was 62,147** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, daemon FPLAN-0610; daemon owning, devpulse landing). Eleven new `docs/` pages moved by line range, each under 9,600 and all twelve `docs/*.md` indexed: `architecture.md` (the per-file annotations as a table), `scheduler_tick.md`, `schedule_contract.md`, `command_jobs.md`, `recovery_and_catch_up.md`, `rounds.md`, `inbox_sweep.md`, `monitoring.md`, `cli_and_arguments.md`, `testing.md` (opens by saying its numbers are dated measurements and naming the two commands that print the live count — the old README carried two different, never-reconciled test counts), `known_issues.md`. The directory tree lives only in the branch prompt now, re-derived from the real tree (2,744 → 6,302). Deleted, not moved: an Overview that said "framework citizen, full 3-layer architecture" (true of every branch, says nothing about this one), an Identity block the passport owns, a Citizen Class line the passport owns. The `--help` diff found no missing verb but one missing flag: `--version` always answered and was never advertised, and its printed string still said 1.0.0 — `apps/daemon.py` 1.1.0 lists it and the string agrees with the header. Measured: Startup_Budget 100 with no red, audit aipass 100 in every category, checklist 39/39, docs lane 12/12 linked with no advisory line, 703 passed from both rootdirs; no test named any string leaving the README.
- **trigger: the README diet — the face is 6,541 chars, was 57,729, and the event that never existed is out of `--help`** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, trigger FPLAN-0611; trigger owning, devpulse landing). `README.md` 2.7.0 → 3.0.0 is purpose, Quick Start (one fenced block), What It Does, Live Inventory (bare `drone @trigger`, `--help`, and `drone @trigger list` for the live event table), How To Reach Me, a Commands section that is a pointer with zero invocation lines, an Architecture paragraph naming all six modules and the handler groups, the `docs/` index and Integration Points. Depth was sliced out by line range into ten new pages, each far under 20,000 and every one linked: `state_and_durability.md` 9,891, `events.md` 8,265, `escalation.md` 6,480, `known_issues.md` 5,518, `log_watching.md` 5,158, `error_catchup.md` 4,840, `medic.md` 4,814, `event_bus.md` 3,739, `service_and_reload.md` 3,248, `error_registry.md` 1,341; `docs/README.md` 54 → 1,583 is the index. The one section with two subjects — watching and the daemon that runs it — was split rather than squeezed, and the catch-up cursor got its own page because it is its own subject. The directory tree left the README for the branch prompt (2,876 → 6,074 of 9,000), re-derived with `find`: the old prompt claimed fourteen events and fourteen handlers against a live seven, an eight-gate pipeline that has seven gates and an either/or, twelve event handlers, no `escalation` module at all, and it named `apps/extensions` and `apps/plugins` as the unused scaffolding while `apps/integrations` sat beside them unmentioned. The `--help` diff before deleting the command list found three live surfaces the page had been covering for: the bus verbs (`fire`, `list`, `status`) are typed directly and the top page named only the module they live in, `--version` was accepted and named nowhere, and `log_events --help` advertised `start` as if it works when it declines by design and exits 2. All three are fixed at the source (`apps/trigger.py` 1.0.1 → 1.1.0, `apps/modules/log_events.py` 1.0.0 → 1.1.0), never on the page. **The oldest open row in the branch closed with it:** `error_logged` was advertised as a real event by that help page and two docstrings and is fired by nothing anywhere in the fleet — known since 2026-08-25, cured in `events/startup.py` on 2026-09-12 and in the last three sites tonight (`apps/handlers/watchers/log_watcher.py` 1.2.0 → 1.2.1). The fleet now names the string in exactly one place: the line recording that it never existed. Deleted rather than moved: the dated Running-now snapshot (medic state, registry totals, mute counts) and the test-count essay, both replaced by the commands that print them live. Every string leaving the README was grepped across `tests/` first — one test reads the file, the pin holding `--version` to the README header, and it passes because both moved together. Measured: Startup_Budget 100 with no column over cap (README 6,541, prompt 6,074, local 14,445, obs 1,303, passport 1,717, dashboard 1,210), `audit aipass @trigger` 100 on all 48 consulted standards with Readme and Readme_Quality 100, checklist 39/39 on the entry point and 34/34 on both changed handlers, the advisory docs lane 11/11 linked with no named-path, count-claim, dated-heading or invocation line, 1079 passed from both rootdirs, ruff check and format clean.
- **spawn: the README diet — the face is 6,498 chars, was 36,300, and every verb the README was the only witness to is now in `--help`** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, spawn FPLAN-0613; spawn owning, devpulse landing). `README.md` 1.0.0 → 2.0.0 is purpose, Quick Start (one fenced block), What It Does, Live Inventory (bare `drone @spawn` for the module self-map, `--help` for the surface, `--version`), How To Reach Me, a Commands section that is a pointer with zero invocation lines, an Architecture paragraph naming all nine modules and every handler by group, the `docs/` index and Integration. Depth was sliced out by line range into seven new pages, each far under 20,000 and every one linked: `birth.md` 10,580, `update_engine.md` 7,156, `tests_and_quality.md` 5,028, `registry_and_repair.md` 5,026, `cli_contract.md` 3,153, `known_issues.md` 2,841, `retire.md` 2,381; `docs/README.md` 390 is the index stub the template now ships. The mint pipeline and the birth contract got their own page rather than being squeezed beside the update engine, because a citizen being born and a citizen being updated are two subjects with one file between them. The directory tree left the README for the branch prompt (3,951 → 5,605 of 9,000), re-derived with `find` rather than copied off the old page — the README's tree had `apps/handlers/json/` as a bare line with no note that it is the fleet json shim, and named neither `update_ignore.py` nor `adoption_ops.py`, both of which have shipped for weeks. **The `--help` diff before deleting the command list found nine live surfaces the page had been covering for**, every one fixed at the source (`apps/spawn.py` 1.0.1 → 1.1.0) and never on the page: `create @existing` (the whole adoption lane — a directory that already carries a passport), `delete <@branch> --yes`, `sync-registry` read-only, `sync-registry --fix` and `sync-registry --check` (with `--json`), `regenerate-registry [class]` and `--all`, `repair --relocate-artifacts` (the flag that carries a branch's vector store with it), and in OPTIONS `--root`, `--only`, `--confirm`, `--yes`, `--json` and `--version` — `--version` was accepted by the entry point, pinned by a test, and named nowhere on the help page at all. The rendered page was checked after the edit, not just the source: `regenerate-registry \[class]` is escaped because Rich eats a bracketed token and seedgo's `rich_markup` standard does not catch it, and every other bracketed placeholder in USAGE and COMMANDS survives the render. Deleted rather than moved: the dated audit-score and trinity-score snapshots, the live command sweep and the two disagreeing test counts, all replaced by the command that produces each number live (`tests_and_quality.md` carries the command table, not the results). Every string leaving the README was grepped across `tests/` first — no test reads the file, so nothing moved out from under a pin. Measured: `audit context @spawn` Startup_Budget 100 with no column over cap (README 6,498, prompt 5,605, local 14,920, obs 4,133, passport 1,797, dashboard 1,727), the advisory docs lane 8/8 linked with no unresolved path, count claim, dated status heading or invocation line; `audit aipass @spawn` 100 on all 48 consulted standards with Readme and Readme_Quality 100; checklist 39/39 on the entry point; 982 passed from both rootdirs, unchanged from before the diet because this was a documentation pass with no behaviour change; `ruff check` and `ruff format` clean.
- **backup: the README diet — the face is 5,395 chars, was 25,700, and the `--help` diff found a live legacy verb plus five flags the page had never named** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, backup FPLAN-0615; backup owning, devpulse landing). `README.md` is purpose, Quick Start (one fenced block), What It Does, Live Inventory (bare `drone @backup` for the self-map, `--help` for the surface), a Commands section that is a pointer with zero invocation lines, an Architecture paragraph naming every module and each handler group, the `docs/` index and Integration Points. Depth was sliced out by line range into eleven pages, each far under 20,000 and every one linked: `ignores.md` 4,938, `store_cleanup.md` 3,085, `path_resolution.md` 2,843, `cli.md` 2,802, `known_issues.md` 2,204, `module_fence.md` 2,159, `store.md` 1,897, `restore.md` 1,740, `tests.md` 1,622, `run_ceiling.md` 1,567, `drive_sync.md` 1,458, with `docs/README.md` 1,135 as the index. The fence rules and the import-time resolve rule got separate pages rather than one "internals" file, because a caller being refused at the door and a path being resolved before a cwd exists are two subjects. The directory tree left the README for the branch prompt (3,758 → 5,296 of 9,000), re-derived with `find` rather than copied off the old page: the prompt's tree had named neither `handlers/cleanup/` nor `display.py`, `restore.py` and `share.py`, and called the whole Drive lane stubs though `drive_check`, `drive_sync`, `drive_stats`, `drive_clear` and `share` have shipped for weeks. **The `--help` diff before deleting the command list found a live entry form and five flags the page had been covering for**, all fixed at the source (`apps/backup.py` 1.0.0 → 1.1.0) and pinned red-first in the existing `test_cli_routing.py::TestPrintHelp`: the legacy `backup <project>` form that runs `snapshot` and takes `--versioned` or `--all` (live in `main()`, named nowhere, exercised by no test), and in a new OPTIONS block `--name`, `--quiet` (`all`), `--force` (`drive_sync` re-upload, and required by `drive_clear`), `--project` and `--note` (`drive_sync`) and `--public` (`share`) — `--quiet`, `--project` and `--note` appeared in no help page at any level. Verb sets otherwise agreed in both directions, and `--version`, the `VERSION` constant and the README header agree at 1.1.0. The rendered page was checked after the edit, not just the source. Deleted rather than moved: the dated measurement banner, the test-count and case-count snapshots and the store-inventory tallies, each replaced by the command that produces it live. Every string leaving the README was grepped across `tests/` first — no test reads the file. Measured: `audit context @backup` Startup_Budget 100 with no column over cap (README 5,395, prompt 5,296, local 13,642, obs 7,370, passport 1,656, dashboard 1,443), the advisory docs lane all 12 `docs/*.md` linked with no unresolved path, count claim, dated status heading or invocation line; `audit aipass @backup` 100 on all 48 consulted standards with Readme and Readme_Quality 100; checklist 39/39 on the entry point; 379 passed from the repo root, unchanged by the diet; `ruff check` and `ruff format` clean.
- **commons: the README diet — the face is 6,511 chars, was 22,849, and `--help` had never named seven live verbs** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, commons FPLAN-0617; commons owning, devpulse landing). `README.md` is purpose, Quick Start (the one fenced block, placed above Architecture), What It Is, How To Reach Me (bare `drone @commons` for the self-map, `--help` for the surface), a Commands section that is a pointer with zero invocation lines, an Architecture paragraph naming all twenty-two modules and every handler domain, the `docs/` index and Integration Points. Depth was sliced out by line range — the moved text is the text that was there, not a retyping — into thirteen pages plus an index, each far under 20,000 and every one linked: `known_issues.md` 4,016, `architecture.md` 3,522, `identity.md` 2,167, `introspection.md` 1,978, `boardrooms.md` 1,936, `exit_codes.md` 1,913, `artifacts_and_trading.md` 1,729, `curation.md` 1,426, `rooms_and_space.md` 1,187, `posts_and_comments.md` 909, `profiles_and_engagement.md` 884, `search.md` 807, `notifications_and_catchup.md` 692, with `docs/README.md` 1,834 as the index. `identity.md` was given its own page rather than a section inside the architecture one, because which branch a caller resolves to is a contract other branches rely on, not an implementation detail. The directory tree left the README for the branch prompt (2,583 → 4,555 of 9,000), re-derived with `find` rather than copied off the old page, and the prompt is now its only copy; it also took the gotchas the README used to carry as prose — the trailing `--help` that executes the verb, handled-is-not-succeeded, `error()` versus `warning()` deciding the exit code, the caller-identity order, the gitignored `.trinity/` marker, the registry fallback, `autospec` on module patches, author-only pins and the silently clamped capsule window. **The `--help` diff before deleting the command list found seven live verbs the page had never named** — `database`, `leaderboards`, `push-central`, `reactions`, `unpin`, `unreact` and `whoami` — all cured at the source (`apps/commons.py` 1.1.0 → 1.2.0), the last three under a new BRANCH SERVICES block because they are services rather than conversation lanes; the reverse direction was already clean, and page and dispatcher now agree 52 verbs to 52 both ways. The same pass found `--version` answering 1.0.0 while the file header claimed 1.1.0: there is now one `VERSION` constant, and `--version`, the header and the README header all read 1.2.0. The help page's identity note also stops saying identity is taken from the current directory and states the order the code uses — `AIPASS_CALLER_CWD` as drone sets it, then the real PWD. Deleted rather than moved: the dated Status and Known Issues snapshots, the module, handler, test and log-site tallies, and the exit-code table's line citations — each replaced by the command that prints it live, collected as a "how to measure this branch" table in `docs/known_issues.md`. Every string leaving the README was grepped across `tests/` first — no test reads the file. Measured: `audit context @commons` Startup_Budget 100 with no column over cap (README 6,511, prompt 4,555, local 15,231, obs 1,303, passport 1,925, dashboard 1,231), the advisory docs lane all 14 `docs/*.md` linked with no unresolved path, count claim, dated status heading or invocation line (0/0/0/0/0); `audit aipass @commons` 100 with Readme and Readme_Quality 100 and no type errors; checklist 39/39 on the entry point; 497 passed from the repo root and again from the branch directory, unchanged by the diet; `ruff check` and `ruff format` clean.
- **flow: the README diet — the face is 6,618 chars, was 46,285, and `--help` had been wrong in both directions** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, flow FPLAN-0612; flow owning, devpulse landing). Six new `docs/` pages, each under 13,200 and all linked from the face: `plan_lifecycle.md` (the commands in depth and the close pipeline), `plan_types.md` (types, auto-healing, ignored folders, orphaned locations), `repo_root.md` (location discovery and its six rules), `argument_gate.md` (unknown arguments and what absolute means), `dashboard_mirror.md`, `known_issues.md`. The directory tree lives only in the branch prompt, re-derived with find (3,256 → 4,846). Deleted, not moved: a dated Status snapshot and a Quality section of pure count claims, both replaced by the commands that produce them. The `--help` diff cut both ways: three real flags the README named and `--help` did not (`--dry-run`, `--exclude-type`, `--version`), a phantom verb `template` that `--help` advertised and the dispatcher refused, and a "short or full name" claim false for seven of eight modules — all cured in the entry point, not the prose: `module_verbs()` reads an optional `COMMAND_VERBS` from a module owning several verbs (`template_manager` declares its four), the help text states the measured truth, the three flags are in EXAMPLES (`flow.py` 1.1.0, `template_manager.py` 1.1.0). `--version` printed a literal 2.2.1 against a README header of 2.6.0 with no constant either could read; one `BRANCH_VERSION` now, both 2.7.0. Two tests in `test_flow.py` pinned the false help claim and were rewritten to pin the measured one. Measured: Startup_Budget 100 with no red, audit aipass 100, docs lane 7/7 linked with no advisory line, 995 passed both rootdirs.
- **skills: the README diet — the face is 5,578 chars, was 30,490, and bare `drone @skills` was printing the reference page instead of its self-map** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, skills FPLAN-0614; skills owning, devpulse landing). `README.md` is purpose, Quick Start (one fenced block), What It Does, Live Inventory (bare `drone @skills` and `--help`), a Commands section that is a pointer with zero invocation lines, an Architecture paragraph naming all six modules and every handler group, the `docs/` index and Integration Points. Depth was sliced out by line range into eight new pages, each far under 20,000 and every one linked: `system_status.md` 9,016 (the skill off Linux and the whole `machine_vitals()` contract), `telegram.md` 4,824 (the retirement and the tmux/systemd portability work behind it), `dead_cwd.md` 4,637, `known_issues.md` 2,257, `runner_notes.md` 2,184, `skill_contract.md` 2,035, `off_switch.md` 1,909, `json_handler.md` 1,672; `docs/README.md` 730 → 1,364 is the index, all nine `docs/*.md` linked. The directory tree left the README for the branch prompt (2,759 → 4,026 of 9,000) and was re-derived with `find`: the old one named five handlers where there are nine, omitted `module_paths.py` and the `json/` shim entirely, showed no `integrations/`, and listed `dev.local.md`, a scratchpad this branch does not carry. Deleted rather than moved: an empty `## Overview` heading, the dated command-sweep line, and every current-state count claim — the suite figures are the command that prints them now. **The `--help` diff before deleting the command list found four live surfaces the page had been covering for**, every one fixed at the source (`apps/skills.py` 1.0.2 → 1.1.0) and never on the page: `run <name> --help`, which prints a skill's own `SKILL.md` rather than dispatching an action named `--help`; `create <name> --help`; the `-h` and bare `help` aliases; and two command rows misaligned by one column. `--version` printed a literal `SKILLS v1.0.0` against a file header of 1.0.2 with no constant either could read — one `VERSION` now, and the test that pinned the literal pins the constant. **Bare `drone @skills` was not the self-map the fleet contract promises**: the entry point routed no-args to `--help`, so `print_introspection()` — which names all six wired modules — was unreachable through drone and only a direct call could see it; no-args now routes to it, and `--help` stays the reference. Every string leaving the README was grepped across `tests/` first — no test read the file. Measured: `audit context @skills` Startup_Budget with no column over cap (README 5,578, prompt 4,026, local 14,783, obs 2,134, passport 1,691, dashboard 1,180); `audit aipass @skills` 100 on every category with Readme and Readme_Quality 100; checklist 39/39 on the entry point; the advisory docs lane 9/9 linked with no named-path, count-claim, dated-heading or invocation line; 362 passed in the CI shape from both rootdirs, unchanged by the diet (skills' own run also collects `lib/telegram/tests`, where the retired cases skip: 404 passed / 1,114 skipped); `ruff check` and `ruff format` clean.
- **aipass: the README diet — the face is 7,268 chars, was 23,660, and the `--help` diff found a top-level verb the page had been the only witness to** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, aipass FPLAN-0616; aipass owning, devpulse landing). `README.md` is purpose, Quick Start (one fenced block), What It Does, How To Reach Me — bare `aipass` for the live inventory, `aipass --help` for the reference, and one line saying `drone @aipass` does not resolve by design, because this branch is the user's own front-door CLI and ships as a binary on PATH instead of behind the router — then a Commands section that is a pointer with zero invocation lines, an Architecture paragraph naming all fourteen modules and every handler group, the `docs/` index and Integration Points. Depth was sliced out by line range into eight new pages, each far under 20,000 and every one linked: `doctor.md` 4,104 (the seven groups, the flags, the provider wiring door, what `--fix` refuses to do), `baud_phone_face.md` 4,026, `scaffold_update.md` 3,854, `init_and_install.md` 3,274, `known_issues.md` 3,119, `trust_and_projects.md` 2,700, `help_chat.md` 2,223 (the keyword model, and why there is no model call), `shared_contract.md` 1,993; `docs/README.md` 579 → 2,043 is the index, all 13 `docs/*.md` linked. Two sections were split by subject rather than squeezed: the update ritual left `init` and `install` because a plan that may replace files is not a command form, and `shared/` left Integration Points because the part of this branch @spawn imports is a contract, not a dependency line. The directory tree left the README for the branch prompt (7,147 → 8,968 of 9,000), re-derived with `find` rather than copied: the prompt's tree named five modules where there are fourteen and four handler groups where there are seventeen, and omitted `templates/`, `tools/`, `aipass_json/` and `shared/` outright, while the README's own tree had never named `init/scaffold_manifest.py`. The prompt is now PROMPT_STYLE compliant as well — single `#` headings, no emphasis, no dividers. **The `--help` diff before deleting the command table found five live disagreements**, every one fixed at the source (`apps/aipass.py` 0.4.1 → 0.5.0) and pinned red-first in the existing `tests/test_aipass_main.py` (1.1.0 → 1.2.0, 45 → 48): `revoke <path>`, a top-level verb `trust.py` has routed since DPLAN-0244 and no help page at any level had ever named; the `doctor --json` row advertising "JSON output for structure scan" when `--json` alone falls through and JSON comes only from `doctor --fix --json`; the `init` row and the example line that both read as though bare `init` starts the guided setup, when it prints usage and `init run` walks the stages; `install` described as "clone + setup + init" in the bare list and "clone + setup.sh + hooks" in `--help`, when it is clone, `setup.sh`, hooks, the phone face and a welcome chat; and the `trust [path]` row misaligned by one column. The rendered page was re-checked at 80 columns after the edit, not just the source, and every exit code was measured with output redirected to a file rather than piped. In the other direction the table's per-flag rows were not gaps — those belong to each command's own `--help` — and the forms `init --help` still omits (`init agent`, the bare template form, `--style`) are now in `docs/init_and_install.md` and `docs/known_issues.md` instead of on the face. `--version` prints the repo's pyproject version resolved live; the README header never carried a number, and none was added, so there is no second copy to drift. Three of the five help-vs-behaviour claims the README carried were **false by tonight and were deleted rather than moved**: `--help` no longer omits `feedback` or `handoff`, bare `aipass` no longer omits `handoff`, and `init --help` does document the `init [target] [name]` scaffold form; the two that are still live moved with their line citations replaced by function names, because those citations had drifted by two hundred lines. Also deleted rather than moved: the dated `## Status` section, the `## Admin setup` summary that said nothing `docs/admin_setup.md` already said, and every current-state count claim — including two that disagreed inside the same file, the tree's "1072 test functions -> 1101 cases" against the Tests section's "1052 ... 1081"; the command that prints the number is the answer now. Every string leaving the README was grepped across `tests/` first: no test reads this branch's README — every README reference in `tests/` is a scaffold target in a tmp dir or help_chat's own `_SAMPLE_README` fixture. Measured: `audit context @aipass` 100% with no column over cap (README 7,268, prompt 8,968, local 14,366, obs 7,318, passport 5,461, dashboard 2,181); the advisory docs lane silent but for its index line — all 13 linked, no unresolved named path, no count claim, no dated status heading, no invocation line; `audit aipass @aipass` 100% on every category with Readme and Readme_Quality 100; checklist 39/39 on the entry point and 23/23 on the test file; 1,127 passed from the repo root, unchanged by the diet; `ruff check` and `ruff format` clean.
- **cli: the README diet — the face is 4,309 chars, was 15,493, and a live help page was advertising a verb the dispatcher refuses** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, cli FPLAN-0618; cli owning, devpulse landing). The branch every other branch prints through, so the pass is a diet and nothing about the render surface changed. Depth was sliced out by line range into six new pages, each far under 20,000 and every one linked: `docs/display_api.md` 3,416 (the symbol table, signatures and import paths), `docs/rich_markup.md` 3,366, `docs/exit_seam.md` 2,586, `docs/json_handler.md` 2,166, `docs/known_issues.md` 2,090, `docs/testing_output.md` 1,242, with `docs/README.md` 540 as the index. The directory tree and the gotchas left the README for the branch prompt (4,799 → 5,352 of 9,000), re-derived with `find` rather than copied off the old page — the old tree listed neither the archived json handler sitting beside the shim nor the README files in `apps/` and `tests/`, and annotated every test file with a case count that rots the next time a test is added. **The `--help` diff before deleting the command list found a refusal, two live verbs and a flag the pages had been covering for**, every one cured at the source and never on the page: `drone @cli templates --help` advertised `drone cli ops demo`, a verb the dispatcher refuses with exit 1, beside a `drone cli templates` form that also exits 1; `show`, a live alias of `display` with its own tests, was named on no top-level page; a bare `demo` runs and was named only on the drone-compliance line; and `--version` answered while the COMMANDS block never listed it (`apps/cli.py` 2.2.0, `apps/modules/templates.py` 0.3.1, `apps/modules/display.py` 0.4.1). The same diff found the `VERSION` constant reading 2.1.0 three lines below a file header claiming 2.2.0: the constant is the one source now, and `--version`, the header and the README header all read 2.2.0. Two help pages also pointed at `CODE_STANDARDS/cli.md`, a path that does not exist anywhere in the repo; both now point at `docs/display_api.md`. Deleted rather than moved: the dated Status block, the seedgo-score and test-count header lines, and the importer tallies, each replaced by the command that produces it live. Answering the question only this branch can: brackets printed through this surface are markup, so the rule the fleet gets is escape the value with `rich.markup.escape()` and keep `markup=False` for whole lines that carry no styling — measured five spellings through a capture console and written up in `docs/rich_markup.md`, with the note that the surface should publish the escape itself so branches stop importing Rich directly. Every string leaving the README was grepped across `tests/` first — no test reads the file. Measured: `audit context @cli` Startup_Budget 100 with no column over cap (README 4,309, prompt 5,352, local 12,897, obs 1,735, passport 1,963, dashboard 1,198), the advisory docs lane all seven `docs/*.md` linked with no unresolved path, count claim, dated status heading or invocation line (0/0/0/0/0); `audit aipass @cli` 100 on every category with Readme and Readme_Quality 100 and no type errors; checklist 39/39 on the entry point and 34/34 on both changed modules; 176 passed from the repo root and again from the branch directory, unchanged by the diet; `ruff check` and `ruff format` clean.
- **canary: the README diet — the face is 4,980 chars, was 13,370, and four routed forms were named on no help page** (`wc -m`; DPLAN-0347 / FPLAN-0593 Phase 4, canary FPLAN-0619; canary owning, devpulse landing). The smallest overshoot in the fleet and the last branch through, so the diet itself was short; the `--help` diff was the part that paid. Depth moved into four new pages, each far under 20,000 and every one linked: `docs/command_surface.md` 3,605 (the routed forms, the exit-code contract, subcommand help), `docs/note_module.md` 3,107 (the store's format, its parse refusal, why it avoids the fleet json service), `docs/testing.md` 2,567 (the suite, the conftest seam, what CI actually runs), `docs/branch_data.md` 2,703 (everything written to disk), with `docs/README.md` 908 as the index. The tree and the gotchas left the README for the branch prompt (3,673 → 4,732 of 9,000), re-derived with `find`: the old tree called `apps/modules/` "empty by design" when it holds the note module, omitted the `notes` handler and the cross-branch import guard entirely, and still described the json shim as sitting over the retired shared handler rather than the prax service. **The `--help` diff before deleting the command table found the gap on the other side: the table was right and the help pages were short.** Every documented row routed and every exit code matched, but the note module routes four forms no page named — a bare `drone @canary note`, `note help` as a bare word, `note -h`, and `--help`/`-h` anywhere in the arguments (so `note add --help` prints help and stores nothing) — and the entry point's own reference omitted the third help form `help` that it routes, plus the exit-code contract, which lived only in the README. Cured at the source, never on the page (`apps/canary.py` 2.1.0 → 2.2.0, `apps/modules/note.py` 1.0.0 → 1.1.0); `--version` and the README header both read 2.2.0 from the one constant the tests already pin. Deleted rather than moved, after re-verifying every claim: "APLAN-0019 remains open by doctrine" (closed 2026-09-13, one day after the README's own Last Updated, confirmed in the plan registry, the closed-plans file and the dashboard), a quoted `logs/README.md` line carrying a doubled word the real file does not contain, a `git status --ignored` example printing a branch-relative path where porcelain prints repo-root-relative, a sibling-branch count that had since moved by one, and every test-count and dated snapshot — each replaced by the command that produces it live. Kept because still true and now in `docs/command_surface.md`: a module that raises is reported as `Unknown command` with exit 1, unfixed by design. Every string leaving the README was grepped across `tests/` first — no test reads the README, the prompt or any `docs/` page; the only coupling is the store-path pin, which is untouched. Measured: `audit context @canary` Startup_Budget 100 with no column over cap (README 4,980, prompt 4,732, local 15,022, obs 7,625, passport 3,309, dashboard 1,201); `audit aipass @canary` 100 on every row with Readme and Readme_Quality 100, no type errors, and the advisory docs lane silent but for its positive index line, all five `docs/*.md` linked, with no unresolved path, count claim, dated status heading or invocation line (0/0/0/0/0); checklist 39/39 on the entry point and 34/34 on the note module; 83 passed from the repo root and again under the repo-root config form, unchanged by the diet; `ruff check` and `ruff format` clean. Phase 4 closes here.

- **startup: the README leaves the startup read fleet-wide; the inventory is `drone @self`** (DPLAN-0347; Patrick 2026-09-15 16:02 "readme 10k and off from start up"). Root `CLAUDE.md`: the greeting reads the three `.trinity/` files one per call, never the README — it is the face for strangers, capped at 10,000 chars (seedgo's number, ruled up from the room's 6,000); the live inventory is the bare `drone @<self>` and `--help`. Why: 17 of 18 branches read 13k–96k chars of README at hello (memory's 95.7k sits at the harness Read cap), so six branches floored at 33–43k tokens before the first word; the devpulse 31k was never the fleet's. Boardroom thread 16 (Commons `boardroom-context-budget`, five seats, 45 min) converged the layer contract: kernel = don't get lost, navmap = the fleet, branch prompt = breadcrumbs under a hook-enforced 9,000, `drone @self` = live inventory, README = capped face plus docs index, `docs/` = depth per module; every cap read from its owner's config, never copied. FPLAN-0593 builds it in five phases.
- **hooks: the injected layers are capped at render and every fail-open is loud** (DPLAN-0347 / FPLAN-0593 Phase 1, thread 16 rows 1-6; hooks owning, devpulse landing). `grounding_content.py` 1.2.0: `BRANCH_CHAR_BUDGET` 9,000 (the ruled prompt cap), `INTEGRATION_CHAR_BUDGET` 2,000, `IDENTITY_CHAR_BUDGET` 4,000 now enforced — `_truncate_block` cuts on a line boundary with a marker naming the source file and logs WARNING, instead of silently crossing Claude Code's 10,000-char persist line (measured live: a 17,999-char branch prompt cut with the marker). `cadence.py` 2.5.0: every fail-open path names itself at WARNING with loader and cause, including the no-session path (measured live: "cadence FAIL-OPEN loader=navmap: turn forced to 0, so it fires EVERY turn — no CLAUDE_CODE_SESSION_ID"); loader `alert` added (period 5). The four prompt loaders (`tier0_kernel`, `navmap`, `branch_loader` 1.1.0, `identity` 1.2.0) log cadence failure at WARNING. `persistent_alert.py` 1.1.0 announces on arrival then repeats on cadence 5 with a 300-char body — an active alert used to re-inject the full banner every turn. `context_gauge.py` 1.1.0 guards per context window via `cadence.window_opened_at`, not per session (it went silent after the first compact of a day). `edit_gate.py` 1.13.0: `passport.json` joins `_TRINITY_MEMORY_FILES` (shell refusal + tripwire) and is measured by `check_file_budget` at memory's 6,000 / 600 per string; the tripwire watches every `.trinity` in the project (stats only, depth 4) and names another branch's change as possibly its own live seat; renders memory 1.11.0's `unknown_field` and `field_over_cap` reasons. Measured (chars): hooks branch block 9,105 → 8,078, four loaders 21,468 → 20,441; live blocks aipass 7,267, devpulse 6,856, memory 1,763 — the cap binds nobody today, a guard not a cut. hooks' own prompt 8,987 → 7,960. +38 cases in existing files (test_cadence 1.1.0, test_branch_loader 1.1.0, test_identity 1.2.0, test_persistent_alert 1.1.0, test_context_gauge 1.1.0, test_edit_gate_bash 1.1.0, test_edit_gate_trinity 1.6.0); hooks suite 2047 passed, seedgo audit @hooks 0 violations. Left for Phase 5: degraded fail mode (kernel only), cadence-by-default key, per-turn ledger, tripwire restore. Open: `test_edit_gate.py::TestEditGateProjectBoundary::test_allow_host_seat_writing_down_into_nested_project` failed 2 of ~70 runs on hooks' machine with no captured text (hooks todo #49).
- **prax: the dashboard caps — a plan subject is a glance, and the file has a budget** (DPLAN-0347 / FPLAN-0593 Phase 2, thread 16; prax owning, devpulse landing, @flow mirroring on its push writer in the same commit). `handlers/dashboard/operations.py` 0.5.0 holds the numbers once — `SUBJECT_CAP` 120, `DASHBOARD_CHAR_BUDGET` 6,000, `TRUNCATION_MARKER`, `cap_subject()`: first line only, cut at 120 with the marker inside the cap, truncate never refuse (refusing drops the plan from the glance; refusing an essay at authorship is @flow's `create`). `refresh.py` 0.7.0 applies it to `open_recent[].subject` and `recently_closed[].subject`; `modules/dashboard.py` 0.4.0 re-exports both numbers and the cut for writers outside prax; `plugins/devpulse_dashboard/git_section.py` 1.2.0 drops the copy it introduced the same morning — a commit subject and a plan subject are the same glance, so they are the same number. `save_dashboard` measures the rendered JSON and logs one WARNING naming branch, size and budget above 6,000, then writes the file whole: warn, never refuse, never trim, because a refused save leaves the stale dashboard that reports zero new mail with three in the inbox. Why: five rows of a 430-char subject is ~2 KB of every greeting on every branch, forever — the 5/5 count caps never bounded the section, because the subjects are the section. A constants module beside the handlers was tried first and seedgo's `json_structure` refused it (every handler imports json_handler and logs an operation; a constants file has none), so the numbers sit in `operations.py` beside the budget they belong to. Measured (chars): devpulse's dashboard 5,560 → 4,113 (-26%), longest subject 319 → 120, none over; prax 2,153; nothing in the fleet over 6,000 today, a guard not a cut. The contract @flow mirrors is a file, `prax/docs/dashboard_caps.md`, never retyped. +12 cases in three existing files (test_operations 1.1.0, test_flow_section_contract 1.1.0, test_devpulse_dashboard_plugin 1.2.0); prax suite 1555 passed; seedgo 34/34 on every changed source.
- **flow: the second dashboard writer mirrors prax's caps, and the stray `flow_plans` key is gone** (DPLAN-0347 / FPLAN-0593 Phase 2, flow FPLAN-0597; flow owning, devpulse landing in the same commit as prax's rows so the cap cannot re-inflate). `push_branch_dashboard.py` 2.1.0 imports `SUBJECT_CAP`, `DASHBOARD_CHAR_BUDGET` and `cap_subject()` from `aipass.prax.apps.modules.dashboard` — no number retyped — and cuts at the two renderer seams (`_build_open_recent`, `_build_section_data`; one seam for closed entries deliberately, because `_filter_branch_plans` builds them on two paths and a per-path cap is a cap the next path forgets); its own write measures the rendered JSON and logs one prax WARNING over 6,000, never refuses, never trims. `update_local.py` 2.0.0 (733 → 82 lines) delegates to that one writer: the top-level `flow_plans` key had one writer and zero production readers (grep across `src/`, only its own tests), flapped live (absent after a prax refresh, back after the next plan op — caught on flow's own card), sat outside `sections` so preserve and budget never saw it, carried the unbounded active list the 2026-08-16 ruling removed from the section, and was wrong anyway (`closed[-5:]` after a plan_id sort is last-five-by-id, not recent closes; a substring `'flow' in location` match published throwaway test dirs as plans). Gone, not moved. Behaviour change, named: the section writer refuses to create a missing dashboard (a directory with no dashboard is not a branch) where the old code minted one — `drone @prax dashboard refresh @<branch>` is the door. Measured: devpulse's registry source holds 10 subjects, longest 383, all over 120 — with the old writer the very next push would have written all 383 straight back over prax's cut; after: 4,107 chars, longest 120, 0 over, no top-level key, flap ended. Tests 994 green both rootdirs (45 builder tests retired with the builders, +6 delegation and +10 cap pins that read prax's numbers at assert time so they go red on a copy); four mutations caught; seedgo audit @flow 100 — it caught two of flow's own rows first (a `/tmp` literal in a docstring, a stripped `log_operation`), fixed not bypassed. `test_push_branch_dashboard.py` 1.2.0, `test_update_local.py` 2.0.0. Found: `todo restore` refuses pre-schema backlog records (status field, task over 100) — flow reshaped its five by hand; the second seat to hit it today, Phase 5 row.
- **seedgo: `trinity_groups` derives the entry shape from memory's config, and `readme_check` gains the docs index lane** (DPLAN-0347 / FPLAN-0593 Phase 2, seedgo FPLAN-0598). `trinity_groups.py` 1.2.0 / `trinity_check.py` 1.3.0 read `memory.config.json entry_types.<type>.fields` (memory's Phase 1 map) — the hand-kept `_ENTRY_RULES` mirror is gone and a config without the map is an error row, never a silent pass ("cannot measure shapes: memory.config.json unreadable — the entry shape is never assumed"); byte-identity with memory's `trinity_push.entry_rules` and its renderer holds across 3 branches × 4 sections; fleet Trinity measured in-process on all 18 before and after: unmoved, all 100. Left mirrored on purpose: `_DRAFT_PERCENT` (memory publishes it only as the module constant `entry_limits.DRAFT_PERCENT`, so failing closed would have reded every branch's Meta lines over a number nobody disputes) — guarded by a pin that calls memory's real `draft_target()` and goes red the hour that constant moves; it retires when memory publishes `entry_limits.draft_percent` in config (Phase 5, one line on memory's side); same for the two renderer fallback literals. `readme_check.py` 1.2.0 / `readme_content.py` 1.2.0 gained an ADVISORY docs lane on `check_branch_info`, scoring nothing: every `docs/*.md` linked from the README, every named path exists, rot bait flagged (stale counts, dated status headings, command lists that duplicate `--help`); `BRANCH_INPUTS` gained `docs/*.md` so adding a doc busts the cache. Why unscored: the fleet has 30 docs files and 3 linked, 0 of 18 branches with a complete index — a scored check would have reded 18 of 18 on one commit (a checker moving in one commit put 17 of 18 red on 09-13). Measured: seedgo 2 of 2 docs unlinked, 1 named path absent, 36 stale count claims, 2 dated headings, 33 command invocations duplicating `--help`; hooks 3 of 3 unlinked, 1 absent path, 14 count claims; devpulse 1 of 2 unlinked, 1 dated heading. That is Phase 4's scoreboard. 33 new test functions in existing files, 33/33 mutants red. Seen, not cured (seedgo's own, Phase 4/5): `trinity_content.py` and `trinity.md` still spell the canonical shapes in prose that nothing measures; `trinity_groups.py` at 1,416 of the 1,500-line limit (seam: the four meta/render functions into `trinity_meta.py`); `proof aipass` reports `readme_currency` and `triplet` FAILED, pre-existing.

---

### Fixed

- **daemon: the update digest reads the real inbox, rounds reach `run.log`, and one test name drops the owner's name** (daemon owning, devpulse landing; daemon's rounds mail 2026-09-19 04:57). `daemon_wakeup.py` and `update/data_loader.py` read a dotless `ai_mail.local/inbox.json` that never existed, so every update digest reported 0 mail (backlog #3, APLAN-0015); both read `.ai_mail.local` now, two pins in `test_data_loader.py` red on the old path. Rounds outcomes only ever landed in `rotation.log` because prax names the log after the calling module; `run.py` now writes its own Fired/Failed lines, two pins in `test_rotation.py` red before. `test_the_wake_carries_patricks_ruling` → `test_the_wake_carries_the_owners_ruling` (DPLAN-0350, the identifier left for its owner). 707 passed from both rootdirs, audit 100.

- **api: an upload whose generated name collides no longer deletes the earlier upload** (api owning, its todo 13; devpulse landing). `_write_stream` opens the destination with `O_EXCL`, and on any `OSError` it discarded the destination — including the case where the create itself was refused because the name was already taken, which deleted the EARLIER file, the silent loss `O_EXCL` exists to prevent. Now a `created` flag is set only after the create succeeds and the discard runs only for a file this write made. Two cases in the existing class, red first: a collision (the name generator patched to the first upload's name) raises `UploadUnavailable`, the earlier bytes are intact and the folder holds one file; a body that dies mid-write after the create still leaves no partial. Four mutants killed (discard unconditional, never discard, flag set before the create, flag never set). The test file's shebang went with it. 56 passed in the file, 1714 from the branch, `audit aipass @api` 100. Two attribution swaps from DPLAN-0350 wave 3 ride in the same two files.

- **seedgo: the mock-repr advisory reads a path returned from a same-module helper — the miss behind Windows CI red 35426157867** (seedgo owning, devpulse landing). Memory's fence pin `test_a_write_into_another_root_is_refused_and_the_file_is_untouched` was exactly the shape `mock_repr_path` (1.0.0, landed the same day) exists for, and the advisory nominated nothing. Seedgo reconstructed the pre-cure lines in its own scratch and measured the AST: the haystack (`" ".join(str(call) for call in ….call_args_list)`) WAS read as repr text, across the two statements, and `.resolve()` kept a path a path — the miss was the needles. `home, foreign = self._fenced_world(tmp_path, monkeypatch)` bound both to a helper's return, which the pass classified as None; a path returned from a helper was a known blind spot named in the detector's own docstring, and it bit on the first new occurrence. `mock_repr_path.py` 1.1.0 reads same-module helper returns: a tuple return binds element by element, a helper whose returns disagree in kind stays opaque, and only `self.`/`cls.`/`ClassName.`/bare-name calls resolve — 238 helpers now read fleet-wide. Widened exactly as far as the count supports: 19 candidate rows (a repr haystack with a needle the pass could not read), hand-read, 2 true (memory's two needles on one line) and 17 false (words, verbs, labels, counts); the widening converts the 2 and none of the 17. Two alternatives measured and refused: a name-guessing arm (catches `home`, never `foreign`) and fixture reading (converts 0 of 19). The other direction checked too: 105 path needles into unreadable haystacks, 9 inside functions touching `call_args`, all 9 hand-read and reading the real args — 0 hidden reprs. Pre-cure memory line nominated, flow's three pre-cure lines still nominated, cured memory 0, live fleet 0. 3 new cases inside the existing file (red first on the old detector), 26/26 mutants killed through an import hook with the disk untouched. `windows_compat.md` names the sibling it did NOT build, with the count: `str()` of an `OSError` built with a filename renders that filename as a repr, so `str(path) in str(exc)` is red on Windows by the same mechanism — 20 such constructions in tests and 11 in apps, none red on the run. From the devpulse seat: 4309 passed / 53 skipped, `audit aipass @seedgo` 100. Devpulse cured the memory line itself first (`1ded6056`): join the rendered `call.args`, never `str(call)`.
- **drone: the PR lock answers "blocked" on a Windows PermissionError instead of escaping** (drone owning, devpulse landing; the last line of the fleet lock-pattern sweep, seedgo's `exclusive_create_race` advisory now 0 fleet-wide). `git/lock_handler.py` 1.1.2: `acquire_lock` does an `O_CREAT | O_EXCL` create and handled `FileExistsError` as "already held"; on Windows an exclusive create against a lock another process is mid-removing raises `PermissionError`, which escaped as an uncaught exception out of the door every `dev-pr` and `commit` crosses. No wait loop: the door has no budget and answers blocked at once by design, so the new arm answers the same way — it re-reads the lock and says `Lock blocked: already held by <holder>` if the file still reads, or names the denial (`create denied (<the denial>) - a lock mid-release on Windows, retry; if it repeats, the repo root is not writable`) if not, logged at WARNING so a root that is truly unwritable does not pass for a busy lock. 2 cases inside the existing `TestLockAcquire` with a fake `os.open` that denies only `.git_pr.lock`, matched by `Path(path).name` and counted, never `str(call)`; both red first with the escaping `Errno 13`; 5/5 mutants red (holder arm dropped, denial unnamed, condition inverted, a retry added, "retry" dropped from the message). `docs/git_interface.md` names the Windows arm. From the devpulse seat: 1372 passed at `-n 3`, `audit aipass @drone` 100. The Windows answer is manufactured in the tests; Windows CI is the real proof.
- **seedgo: an advisory sees a path asserted against a mock call's repr, and the Windows standard says what a Linux seat cannot observe** (`windows_compat_check` 1.3.0, new pure detector `mock_repr_path.py`; seedgo owning, devpulse landing). Windows Test 35416653326 went red on three flow tests that joined `str(c)` over `mock_logger.error.call_args_list` and asserted the lock path was in it: `str(call(...))` is a repr, repr doubles backslashes, and a POSIX path has none, so it passed here by accident. The detector is a per-function name-flow pass — repr text (str/repr/f-string of a mock call record or its containers, carried through join, slice, lower/strip and bound names) meeting path text (tmp_path, tmpdir, Path, tempfile, os.path and anything built from them) at `in`/`==`/`.count`/`.find`/`.startswith`. Excluded and named: the real message one level inside the args (flow's cure), a path side repr cannot change (`.name`, `.as_posix()`), a side already escaped, a platform guard. Corpus `tests/**/*.py`, info channel, no score. Fleet count: 3 before (exactly the CI lines), 0 after flow's cure, 0 in the other 570 test files; two broader arms were measured (0 and 14 hits, all 14 read by hand and none real) and not shipped. Ten cases in the existing test module, 18/18 mutants red in memory; 4306 passed / 53 skipped, `audit aipass @seedgo` 100. `windows_compat.md` gains "What a Linux seat cannot observe — pin it by shape": a state POSIX lacks (nothing is ever delete-pending) and a spelling POSIX never makes (no backslash), so a green Linux run proves nothing about either; and the one thing it can observe, flow's instrument — `pytest --basetemp` with a literal backslash puts one in every tmp path and turns the repr shape red on Linux. The exclusive-create advisory is down from 8 lines to 1 (drone `git/lock_handler.py:171`).

- **flow and ai_mail: seven exclusive-create sites survive the Windows delete-pending denial** (flow FPLAN-0625, `daemon.py` 1.9.0; owners curing, devpulse landing). The rows seedgo's advisory named after the api incident. **ai_mail** `dispatch/daemon.py:251,:273` `_write_pid_file`: a `PermissionError` from the pid file's exclusive create escaped `run_daemon` and the daemon never started; `:273` runs right after its own unlink, the likeliest site in the fleet to meet its own delete-pending file. New `_create_pid_file()` polls a denial every 0.05 s inside a 5 s budget, then raises `PermissionError` naming the wait, chained from the denial. The unlink between the two creates (`:271`, not flagged) gets the same treatment. `os.replace` was weighed and refused: last-writer-wins would let two daemons both start, and Windows denies a replace over a delete-pending target the same way. Five red-first cases, 6/6 mutants red; 1504 passed, `audit aipass @ai_mail` 100. **flow** `save_registry.py:59`, `mbank/process.py:58`, `aggregate_ops.py:45`, `append_closed_plan.py:43`: `except PermissionError` ahead of `except OSError`, retried as a held lock on the existing 10-try budget; when the budget runs out on a denial it raises naming path, attempts and seconds, chained. `runner/lock_ops.py:84` `try_create_lock` was single-shot and now retries on its own small budget (5 tries, ~1.55 s) because a delete-pending lock is the previous runner releasing, and "already running" would leave the just-closed plan unprocessed; past budget it raises, never `False`, because `False` is reported as already running. `post_close_runner.py` catches it, reports through `error()`, and `__main__` exits 1 instead of crashing. Sixteen cases inside existing classes, 15 red first; 1011 passed, `audit aipass @flow` 100, all five advisory lines gone. Both owners manufacture the Windows answer; Windows CI is the proof.

- **trigger: one repeat-digest thread per subject, not per signature** (trigger FPLAN-0624; trigger owning, devpulse landing). Between 09-17 08:00 and 08:25 @devpulse got 29 `[REPEAT]` mails under two subjects (`@memory / captured_memory_files` 27, `captured_entry_limits` 2). Measured in `logs/escalation.jsonl`: 29 distinct signatures, each `created` once and `updated` in place on every later fire. The upsert lookup was stable; the key was the wrong unit. `escalation:<signature>` hashes the normalized message, and @memory's CARRIED warnings name a different branch and field each time (vera, verify, writer and research are not fleet citizens, so the name collapse cannot see them, and the field names differ anyway). The signature stays the counting unit. The mail thread is now `escalation:<LEVEL>:<branch>:<module>`, exactly what the subject names. Every signature under one subject upserts the same open message; its body lists the others (newest first, 15 lines, then a count) and the subject gains `(N signatures)`. Closing the message re-arms it. `apps/handlers/escalation.py` 1.3.0 → 1.4.0. Cases added inside existing classes, including a 24-signature replay against ai_mail's match rule (red at 24 messages, green at 1, re-armed on close). 8/8 mutants red, 1087 passed from both rootdirs, `audit aipass @trigger` 100.

- **memory: a dict-shaped memory file is "undrainable", said out loud — not "ready for rollover" forever** (`detector` 0.5.0, `extractor` 0.7.1, `orchestrator` 1.1.0, `modules/rollover` 0.10.0; memory owning, devpulse landing). VERA.local sat at 250/15 key_learnings and WRITER.local at 63/15 for weeks: both are schema 2.0.0 files whose `key_learnings` is a dict, and the June migration walked `AIPASS_REGISTRY.json` only, which never listed Vera Studio. The detector counted `(list, dict)` as a trigger, the extractor drains lists only and returned "No entries exceed v2 limits" — logged at INFO, with a false reason — so `check` printed "ready for rollover" and every PreCompact fired a fleet run that could move nothing. Now a dict is not a trigger: `undrainable_in()` names the container and the schema, `check_all_branches` returns an `undrainable` list and logs one WARNING per file per walk, `check`/`run`/`status` print those files as a third state and never as ready. Eight red-first cases inside existing classes, 9/9 mutants red; 1877 passed / 2 skipped, `audit aipass @memory` 100. Found under it and NOT cured, a ruling: since 09-17 07:47 a `drone @memory` call from a Vera Studio cwd persisted that project's registry into `known_registries.json` (gitignored, which also holds three pytest tmp paths and a scratchpad path), and rollover has been writing four Vera Studio branches' memory files ever since — the pin `test_the_rollover_lane_never_reaches_into_another_repository` is red on that machine and green in CI. `rollover run --branch` stays fleet-wide for files and now says so on every run.

- **commons: a fresh clone no longer talks to a phantom database, and `search` accepts a plan id** (commons owning, devpulse landing; night rounds 09-18). The branch-root walk accepted only `.trinity/`, which is gitignored, so a clone fell back to `~/.aipass/commons.db` — an empty database that reported success. The walk now also accepts `.aipass/` (tracked), the fallback is deleted, and `get_db()` raises `CommonsRootNotFound` naming both markers, the directory walked from and the `AIPASS_ROOT` override — deferred to use time so the package still imports. Second, `drone @commons search "FPLAN-0593"` exited 2 with `no such column: 0593`: the raw query went into `MATCH` and FTS5 read the hyphen as an operator, refusing every plan id, hyphenated name or quoted phrase. Each token is now quoted into an FTS5 literal phrase in one helper shared by post and comment search. Ten red-first cases across `test_lifecycle.py` and `test_search.py`; `known_issues.md`, `architecture.md` and the branch prompt reconciled. 507 passed, `audit aipass @commons` 100; the search exits 0 from this seat.

- **api: the token store lock died on Windows and a revoke was lost** (CI run 35192484222, Windows Test red on a docs-only commit; api owning, devpulse landing). `_store_lock` takes its lock with an exclusive create and retried only on `FileExistsError`. Windows answers a create against a lock another writer is still removing (delete pending) with `PermissionError`, so the revoke thread died unhandled and `test_a_revoke_racing_a_touch_across_threads_still_wins` was false: a revoked credential stayed live. Cure (`tokens.py`): `PermissionError` is polled like a held lock inside the same deadline, then raises `OSError` naming the wait, `from` the denial, so a real permissions problem still surfaces. `_drop_lock` now says out loud (warning) when it cannot remove the lock; it used to log the refusal at debug as "already gone", which was false. Four cases inside the existing class, three red first; 9/9 mutants red; 1712 passed, `audit aipass @api` 100. The Windows answer is manufactured in the tests, not observed, and CI is the proof. Logged, not built, api todos 13 and 14: an upload name collision discards the file that was already there, and a stale-lock break is check-then-act.

- **seedgo: an advisory sees exclusive-create locks that only handle "exists"** (`windows_compat_check` 1.2.0, new pure detector `exclusive_create_race.py`; seedgo owning, devpulse landing). The api defect above is a shape: an exclusive create (`os.open` with `O_EXCL`, or `open(p, "x")`) whose `try` handles `FileExistsError` and lets `PermissionError` escape, or catches it in a retry loop only to give up on the first one. The rule is an info channel with no score — advisory first, the house rule since a checker that moved in one commit put 17 of 18 branches red — and the branch's `windows_compat` bypass rules silence a line. Measured: 18 exclusive creates fleet-wide, 8 lines; the pre-fix api lock is caught at the create and the cured shape clears. Every create was hand-read with its handlers and callers, and the grep that said six files "handle it" held for none: those `PermissionError` mentions guard `os.kill`, `/proc` reads or `os.replace`, never the create. REAL, escaping: flow `runner/lock_ops.py:84` (the detached runner crashes instead of saying "already running") and ai_mail `dispatch/daemon.py:251,:273` (`_write_pid_file`; the daemon never starts). SOFT, give-up: flow `save_registry.py:59`, `mbank/process.py:58`, `aggregate_ops.py:45`, `append_closed_plan.py:43`. Latent: drone `git/lock_handler.py:171`. Twelve cases in the existing test module, 12/12 mutants red; 4296 passed / 53 skipped, `audit aipass @seedgo` 100. Owners of the real and soft rows are dispatched separately.

- **devpulse: watchdog delivery moves off the Monitor tool — nothing happening, nothing happens** (DPLAN-0348 / FPLAN-0622). Claude Code 2.1.271 replaced Monitor's no-timeout `persistent` option with a deadline of at most 30 minutes per watch and a wake to re-arm; no setting restores it. The continuous `watchdog baseline` wire therefore died every half hour and woke the seat to say so: 33 empty wakes in one 15-hour absence, enough to force an auto-compact. The sign-in is now the one-shot that already existed, `drone @devpulse watchdog baseline --once` under Bash `run_in_background`: it replays missed completions, sits silent on one `stat()` a second, and its exit is the single wake. Measured 09-16: armed 20:33, silent 31 minutes (past the Monitor cap), one exit at 21:03 carrying the @hooks completion, `delivered=1 ticks=1875`, zero model turns while idle. `wire.py` refusals and the `watchdog --help` examples named `Monitor(..., persistent=true)`, a remedy that no longer exists; they name the one-shot now, and `test_continuous_arm_under_run_in_background_is_refused` pins that the refusal says `baseline --once` and never `persistent`. `tools/statusline.sh`: green accepts a `background` wrapper as well as `monitor` (the continuous wire refuses background Bash before it registers, so a registered `background` wire is always a one-shot), and no wire armed paints dim `watchdog:idle` instead of red `OUT`, because idle is the designed resting state and red on every quiet hour trains the eye to skip red. Named limit: work out with no wire armed also reads `idle`. `docs/watchdog.md` rewritten for the new floor, with the unsettled parts said (`/clear`, `/resume`, terminal close) and the one killer named: that night's test wire was reaped by the harness after 77 idle minutes ("the system is running low on memory", on a box with 13 GB available), which arrives as one `killed` notification, is not to be restarted unprompted, and is switched off only by starting Claude Code with `CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP=1`. Its start-edge rule is corrected: a direct wake is `kind="wake"` and dropped by kind, but the daemon's start edge is `kind="dispatch"` and is dropped for carrying no `sender`. Branch prompt rule rewritten to match. 596 passed / 2 skipped; `audit aipass @devpulse` 100. Installed statuslines are copies: re-run `cp tools/statusline.sh ~/.claude/statusline.sh`.

- **hooks: a harness wake is not a turn, and a compaction grounds once, not twice** (DPLAN-0348 / FPLAN-0622; hooks owning, devpulse landing). Claude Code 2.1.271 caps every Monitor at 30 minutes and wakes the seat with a `<task-notification>` prompt, and UserPromptSubmit hooks fired on those wakes as if someone had typed. On 09-16 an idle seat took 33 wakes in 15 hours, 7 of them paying the full ~20,400-char stack. After the 09:03 compaction the backstop delivered 21,688 chars in three parts, then the turn-0 fire-all sent 20,310 more at 09:30. **Cure 1** (`cadence.py` 2.7.0): `is_automated(hook_data)` reads the payload's `source` first (`system` = automated). The 2.1.273 schema declares that field, but both payload builders compile it out (`...!1` in 2.1.273 and 2.1.274), and six live payloads carried no `source`: `-p`, typed, slash command, background-Bash notification, interactive wake. So the fallback is how the left-stripped prompt opens: `<task-notification>` or `[SYSTEM NOTIFICATION`, never a substring. A missing prompt reads as human. On an automated turn the counter neither advances nor writes (token included), and `should_fire` answers False for every loader before the turn-0 clause. Mail still announces on arrival. An alert's arrival never asks cadence, and `should_fire("alert")` only decides repeats, which would otherwise have repeated on every wake. Documented change: a notification no longer cancels queued re-ground parts. **Cure 2** (`post_compact_regrounding.py` 2.2.0): the final part stamps `aipass-regroup-done-<session>.json` with `{window, token}`. A partial or degraded re-ground stamps nothing. At turn 0, `tier0`/`navmap`/`identity`/`branch` stand down once when the window matches and the prompt is within `regroup_fresh_bytes` (150,000). Every sibling process of the spending prompt agrees, and a later turn 0 fires. The distance comes from the ledgers: the double ground sat 126,258 bytes after the last part, and the other two post-compact prompts came after 308,557 and 587,488 bytes of autonomous work, so they still ground. `DEFAULTS` carries it because `cadence_config.json` is gitignored. DPLAN-0276's dead zone stays closed: compaction then an immediate prompt, a partial delivery, and a second compaction all fire all four. Ledger rows (`injection_ledger.py` / `ledger_store.py` 1.1.0) carry `automated`, and `drone @hooks ledger` marks them. 16 new cases inside existing classes; 12 were red first, and the other 4 are constraint pins, green before and after. 14 mutants red. 2099 passed / 2 skipped from the branch, from the repo root, and with the operator config absent; `audit aipass @hooks` 100. One existing line changed, type-only: `test_cadence.py` `in degraded_reason()` became `in (degraded_reason() or "")`, a pre-existing pyright error my edit gate refused to pass.

- **hooks: `git_gate` failed OPEN on Windows for edits to the provider settings file and `.claude/hooks/`, and the degraded banner read the per-user `~/.aipass` as a stamped tree** (Windows CI run 35077780481 on c6d72d3a, 6 failures; hooks FPLAN-0621, devpulse dispatch). Phase 5 replaced a hardcoded POSIX home path in `test_git_gate.py` with one built from `Path.home()`. On the Windows runner that produced a backslash path, and the protected-path regexes, written with forward slashes and never fed a normalised path, returned allow. The old constant had made the test pass on Windows while testing nothing there: the hole predates the phase, and the test was right. `git_gate.py` 1.2.0 reads `\` as `/` before matching, the same idiom its executable-name check already used, and matches case-insensitively, because Windows and default macOS filesystems open `.Claude\Settings.JSON` as the same file. The other three failures had a different cause, not a path spelling. The Windows temp dir sits under home, so `grounding_content._find_project_dir()` walked up into the per-user `~/.aipass` (trust registry, commons.db) and counted it as a stamped tree, and tests expecting silence got the `[GROUNDING DEGRADED]` banner. `grounding_content.py` 1.3.1 skips that directory, compared with `samefile` so an 8.3 short name cannot slip past. Live reach measured nil: the engine injects nothing in an unstamped tree under home. All three were reproduced red on Linux by running the suite with `--basetemp` under home, then green. New pins spell the Windows paths out so every host exercises them. The home-layout pin asserts on the walk, not on stdout, because a tmp_path under the real home would make a stdout assertion depend on the host. A control keeps a real stamped tree under home loud. 5 mutants red. Swept for the same hole: no other gate matches a path against a forward-slash pattern. `auto_fix` and `subagent_gate` test forward-slash substrings but fail toward more checking. Also measured and left open in `docs/known_issues.md`: `git.exe <write verb>` and `rm.exe -rf` pass `git_gate` and `rm_gate`, which recognise the executable by name. Measured: the six by node id pass, 2083 passed / 2 skipped from the branch, the repo root, xdist, and the home layout; `audit aipass @hooks` 100.
- **memory: `rollover_config.py` back to 100% — the display literals move behind a handler and the three config verbs are named for what they are** (DPLAN-0347 / FPLAN-0593 Phase 5; memory owning, devpulse landing). Run 35069130247 went red on `seedgo-audit` alone — `FAILED: 1 branch(es) below 100%` / `memory: 99%` — with every test, lint and coverage job green and the new startup ratchet passing (README 7,887/10,000, prompt 6,763/9,000). The threshold has hard-failed under 100% since 2026-06-05, so the decision to park the finding at 99% was itself the red. **The diagnosis was wrong twice before it was right, and both corrections came from measurement rather than argument.** First reading: "the failing check is size alone." It was two checks — `check_file_size` (657 against a 600 hard fail) *and* `check_thin_orchestration` — read as one because a filtered grep of the checklist showed a single FAIL line. @memory found why it hid: a standard ticks green at score ≥75 while the audit counts the failed check, so the file read **34/34 on the checklist and 80% in the audit at the same moment**. Second reading: "take the bypass, it is precedented." @memory measured what nobody had and refused it — `is_bypassed(path, "modules")` takes the standard name with no per-check key, so one entry would drop the whole modules standard *including size*, on the one file in the branch that had just proved it regrows (656 → 657 on a four-line edit), and the scoped reason proposed for it would have been readable to a human and invisible to the checker. **The cure, in two halves.** The display half: `apps/handlers/cli/config_content.py` (new, 197 lines) holds the literals as `fmt_count`, `defaults_lines`, `overrides_lines`, `branch_limits_lines`, `help_lines` — data in, markup lines out, never prints, no `aipass.cli` import — and the module prints what it returns, so handlers-may-not-print is satisfied on its own terms rather than dodged. 657 → 568, size clean. The naming half: `config_get` / `config_set` / `config_set_default` → `handle_config_get` / `handle_config_set` / `handle_config_set_default`. Not cosmetic and not the rename that was refused earlier — that one proposed `show_`/`update_` prefixes, and `show_` claims display for a verb that writes. `ORCHESTRATION_PREFIXES` carries `handle_` commented "Sub-command routing", and `_handle_config` dispatches to exactly these three. Read end to end before ruling, `handle_config_get` is resolve → banner → fork on branch → fork on JSON vs screen → delegate → log: every line a decision or a delegation with no extractable body, so the checker skipping it is the checker being right. **The standards finding this leaves for @seedgo, which is the real one:** `check_thin_orchestration` uses function LENGTH as a proxy for "implements logic", and a verb answering both the machine surface (`--json`) and the screen — both required by seedgo's own standards — carries two forks a single-surface verb does not. `THIN_WRAPPER_MAX_LINES = 40` is fair for one surface and wrong for two. @memory also corrected its own reference count mid-flight: of "16 references", 4 were `log_operation` event names, left unchanged deliberately because renaming them would split existing log history in four files for no check's benefit. Measured: branch audit 100% overall with modules 100% and 0 violations / 0 failed checks / 0 type errors across 57 files; 1,870 passed / 2 skipped at `-n auto` loadscope, `-n 3`, and serial; all five rendered outputs byte-identical to HEAD through a recording Rich console at width 110 (help 3,334/51, defaults 428/6, overrides 304/4, no-overrides 63/2, branch limits 1,042/11); no outside callers fleet-wide.

- **ai_mail: the macOS cwd test no longer takes its expected value from the host it runs on** (FPLAN-0593 Phase 5; ai_mail owning, devpulse landing). Windows run 35058244702 went red on `test_wake.py::test_get_pid_cwd_darwin` — `assert None == 'C:\\Users\\runneradmin\\…\\pytest-project'` — while Linux and macOS both passed it, so it was red on every Windows head of the diet and it alone stood between PR #770 and a green merge train. Cause: the test fakes darwin and then built the fake `lsof -Fn` output out of `tmp_path`, which on a Windows runner is a drive-letter path; `_get_pid_cwd_darwin` accepts only a name line whose prefix is `n` followed by a forward slash, so the parse fell through the loop and returned None. A comment recorded that `tmp_path` had been chosen so the literal would not hardcode a POSIX directory the Windows run cannot name — that change is what broke it. Fake `lsof` output is data the parser reads, not a directory the host has to own. Cure in the test file alone: a synthetic POSIX literal and no `tmp_path` in the signature, chosen over a skip guard because this test is the only coverage the macOS parser gets on the other two runners. The leading slash is now load-bearing and pinned by its own case, red-first against a manufactured Windows path and measured both ways from the devpulse seat: a drive-letter name line reads None, a POSIX one reads the path back. 1,499 passed in the CI shape, one more than before. **Seen and reported, not cured:** `dispatch/daemon.py` carries AST-identical twins of all four functions — `_get_pid_cwd`, `_get_pid_cwd_darwin`, `_read_session_type`, `_read_session_type_darwin` — diverging only in the log tag, and its darwin lanes have no coverage at all, so the same trap sits there unexercised. A shared helper taking the tag as an argument is the cure; a naive merge would make daemon-side log lines say `[wake]`, which is how a daemon poll is read in prax.
...
- **memory: todos v2 — a sticky-note pad, not a log** (DPLAN-0345 / FPLAN-0590 rows 1-5; memory owning, devpulse landing; Patrick 2026-09-14 23:49: "max 10 max 100 characters, roll off to a .todo folder in backup, each agent gets their own backlog", and 2026-09-15 00:21 "we just use the existing triggers shape"). Why: the third todo bloat since S225 — devpulse's `todos[].status` held 64,657 chars of nightly wake-back logs because @memory's entry_limits capped only `task`, and every reset since June cleared the content, not the cause. The canonical todo is now `{number, date, task, priority?}` — `status` is gone from `ENTRY_RULES` (trinity_push.py 1.2.0); `task` caps at 100 (was 150) and the pad count is `rollover.defaults.local.todos.count: 10` (config_loader 1.4.0 — the key hooks' advisory already read with a hardcoded fallback; no `max_entries` on entry_limits, every reader would have ignored it). The roll is a NEW file-only handler, `rollover/todo_roll.py` 1.0.0: over 10 is legal on disk, and at the next rollover the oldest by number roll to `.backup/todo/<dir>/backlog.json` in the push lane's order — append, replace atomically, read back, compare every entry, only then prune (the classic rollover prunes first, wrong for open work). Backlog entries are nested `{rolled, reason, entry: <byte-identical todo>}`, never vectorized (a restored-then-rerolled todo would land as the CPLAN-0002 duplicate species); `restore` re-issues `max(pad ∪ backlog)+1` so numbers never collide and a restored todo is not the first to roll again; the todos_meta tab derives `next #N` at render time (tab_renderer 1.4.0). The push rule IS the migration: a non-canonical or overflow todo goes to the backlog, verified, never reshaped — one fleet pass empties every pad (dry-run 2026-09-15 02:01: 195 todos across 18 branches, 107,383 chars, devpulse 23 / 69,680, seedgo 72 / 16,127) and agents pick their ten by hand in Patrick's style: "Check on seedgo's errors in logs", "fix drone help". `RESHAPE_ONLY_SECTIONS` retired (zero consumers). New verbs `drone @memory todo` (pad + backlog counts), `todo backlog [@branch]`, `todo restore <number>` (modules/todo.py 1.0.0); `rollover check|run [--branch @x]` (rollover.py 0.7.0, cli/branch_flag.py 1.1.0, rollover/todo_report.py 1.1.0 keeps the literal "ready for rollover" hooks matches on). `LOCAL.template.json` `_usage` and todos prose rewritten; `config get --json` gains an additive read-only `todos` key. The shape contract for the mirrors is a file, `memory/docs/todos_v2_shape_contract.md`, never retyped. 13 existing test files updated, 69 mutants red; scratch proof roll 12→10, restore refused on a full pad, delete + restore re-numbers, re-roll. Triggers unchanged by ruling: PreCompact through hooks' existing lane and `rollover run`; own branch only, never the detached fleet walk; no SessionStart hook (compass #272 holds).

- **seedgo: `module_eviction` — a 15th rule in the pytest_quality pack, from the night's flaky memory contract test** (seedgo owning, devpulse landing; advisory, shadow status, gates nothing). What it reads: every function in a `tests/` file; it flags `sys.modules.pop(K)`, `del sys.modules[K]` and `delattr(M, attr)` where M is provably a module — the exact shape that made `memory/tests/test_contracts.py` exit 0 on macOS and Linux 3.13 (a helper evicted the rollover module, re-imported it bound to a mock cli, and teardown never put it back). Acquitted when monkeypatch recorded the same target earlier in the function, `patch.dict(sys.modules)` wraps it, an explicit restore follows (a re-import is not a restore), a fixture has statements after its last yield, or a same-file autouse fixture records the key. Surfaced by `drone @seedgo audit pytest_quality [@branch]` and `drone @seedgo standard module_eviction`; not in `audit tests`. Precision measured: 27 rows hand-read across prax, backup and memory with a runtime plugin comparing `aipass.*` module objects before each test and after teardown — 23 leaks proven at runtime, 3 right-shaped but not visible, 1 false positive left (prax `test_jsonl_writer.py:24`, restored by a conftest autouse the rule does not read), 5 false positives cured by the autouse acquittal. Fleet: memory 29 rows (98), prax 30 (98), backup 2 (99), the other 15 branches 100; one new real leak found in memory's `test_rollover.py:243` (bare pop of the cli submodules, module objects replaced after teardown — memory's to cure). `module_eviction_check.py` 1.0.0, `module_eviction_content.py` 1.0.0, `module_eviction.md`; `test_pytest_quality_pack.py` 1.1.0 (+23 functions / 25 cases), 32/32 mutants red; seedgo.py 2.0.2, README + branch prompt rule counts 11 → 15.
- **seedgo: the incremental audit cache watches a checker's external inputs** (seedgo owning, devpulse landing; the audit-cache inputs follow-up). `branch_audit.py` 2.0.0 → 2.1.0: `_external_input_files(checkers)` collects each checker's `external_inputs()` (opt-in by defining it, like `check_branch_info`), and `_collect_watch_files` keys them by resolved absolute path and compares content, not presence; `trinity_check.py` 1.1.0 → 1.2.0 declares `memory.config.json` plus the LOCAL and OBSERVATIONS gold templates that exist, sharing `_config_path`/`_templates_dir` with the loaders so what is watched cannot drift from what is read. Closes the stale cached Trinity rows (CANARY, aipass, drone) after a config or template edit. `test_incremental_audit.py` 1.1.0 (+6), `test_trinity_check.py` 1.2.0 (+4), 8/8 mutants red. Also decided by seedgo: no "gold newer than seed" row — spawn's byte-for-byte birth-receipt test is the owner's contract and mtime means nothing across clones; and the fleet audit's one-off spawn Trinity 99 this morning was seedgo's own mutant run mutating the live tree for ~7 s during the audit (trinity_check.log "memory.config.json not found" at 09:17:24), not a transient read — it stays a lost point by design, and mutants now run on a copy (seedgo KL 370).

### Changed

- **devpulse: the README is an awareness map, not an encyclopedia** (DPLAN-0346; Patrick 2026-09-15). README 19.2 KB → 4.8 KB: what exists, one line on what it does, how to reach it. Watchdog internals moved to `docs/watchdog.md`; stale counts and resolved-issue history dropped (the seedgo audit and the suite are the live numbers). The branch prompt drops the memory-limit and dispatch-command sections the navmap already carries (7.5 → 6.6 KB); the passport's compass trait is one line, which also shrinks the identity hook. Seedgo devpulse 100, Readme and Readme_Quality 100.
- **hooks: the identity injection is on the cadence beat; prax: the dashboard's last commit is one glance; devpulse: commit subjects are subjects** (DPLAN-0346 rows; Patrick 2026-09-15 13:02 "every 5 not every turn" and 13:14 "go"; hooks and prax owning, devpulse landing, both mailed). `handlers/prompt/identity.py` 1.1.0 gates on `cadence.should_fire("identity")` exactly like the kernel and navmap — loader `identity` period 5 offset 0 in `cadence.py` 2.4.1 DEFAULTS (the local `cadence_config.json` mirrors it) — so turn 0 and every fifth turn carry the four grounding parts together and the four turns between carry nothing; it was the only every-turn injection, ~500 tokens a turn. `test_identity.py` 1.1.0 holds the gate open for the render tests (they went red under the live pytest-session counter) and pins skip, fire and fire-anyway-on-error; hooks suite 2010 passed. Dashboard: `git log -1 --format=%s` was already subject-only, but the last eight commits were 1.3–4 KB one-line subjects with no body, so `DASHBOARD.local.json` carried a 2.8 KB subject on every greeting — `devpulse_dashboard/git_section.py` 1.1.0 keeps the subject's first line capped at `SUBJECT_CAP` 120 (7.9 KB → 5.2 KB live), `test_devpulse_dashboard_plugin.py` 1.1.0 +2. The committer convention changes with it, in the devpulse branch prompt: subject under ~80 chars, blank line, the WHY essay in the body — the record keeps the essay, the glance gets the subject. Measured this session: a fresh hello sits at ~34k message tokens, the lowest on record since the memory files reached their caps (S208 61k, S470 50.5k).
- **hooks: the PreCompact rollover names the compacting branch** (FPLAN-0590 row 7; hooks owning, devpulse landing). `lifecycle/rollover.py` 2.2.0 resolves the branch from the payload cwd (`context_window.find_branch_dir`, the same resolve `pre_compact_prep` uses) and runs `drone @memory rollover check|run --branch @<dir>`; before, both calls went out bare from the repo root, drone re-stamped `AIPASS_CALLER_CWD`, no branch resolved and the todo pad was never checked (memory's finding, measured). No branch → the bare call as before plus one info line; `READY_PHRASE` is a module constant. `security/edit_gate.py` 1.11.0: the todo-count advisory reads memory's `get_todos_count(branch)` (fallback 10 kept; the old read took a per_branch block with a literal while memory falls back to the configured default) and names the roll — "legal on disk; the oldest N roll off at the next rollover (PreCompact, or drone @memory rollover run --branch @x) to .backup/todo/x/backlog.json; drone @memory todo backlog @x reads it"; `_NON_ROLLING_SECTIONS` removed (todos was its only member) and todos get their own INFO note naming the backlog, not the "recall with drone @memory search" wording that is false for a file-only roll. +8 pins in existing files, 13 mutants red; live proof `check --branch @hooks` → "@hooks todos: pad 10/10 - within count". Follow-up the same night (edit_gate.py 1.12.0, hooks FPLAN-0592): canary measured that the advisory FIRED (edit_gate.log, engine.jsonl `stdout_len 227`) but never reached the agent — PreToolUse plain stdout on exit 0 shows in the transcript, not to the model — so `_check_trinity_change` now returns `{"hookSpecificOutput": {"hookEventName": "PreToolUse", "additionalContext": <advisory>}}` (no permissionDecision, no hooks.json change); measured before/after on hooks' own pad: the Edit result now carries "PreToolUse:Edit hook additional context: todos over limit (11/10) …". `test_edit_gate_trinity.py` 1.5.0 pins the exact shape; 5/5 mutants red; README paragraph: the count is advised never refused, at most once per cadence window per session. Edge left open: `--branch @<unregistered seat>` exits 2, so a seat outside memory's scope would log WARNING after the fleet roll — every registered seat resolves.
- **seedgo: the trinity standard mirrors the v2 todo shape** (FPLAN-0590 row 6, same commit as memory's rows 3-4 so no trinity row goes red between; seedgo owning, devpulse landing). `trinity_groups.py` 1.1.0: `_ENTRY_RULES` todos = number/date/task required, priority optional, no status; `_resolve_todos_count` mirrors memory's resolver (per file key, per_branch local block without todos falls back to the default); `_todos_tab` mirrors `tab_renderer._todos_tab` glyph for glyph with the count from config, no literal 10; `expected_meta_line` takes `todo_ctx`, the `next #N` slot is read by shape (any `str(int)` or `?`), the no-context rendering `<branch>`/`#?` (what spawn birth writes) accepted whole — a strict N would flap red fleet-wide since adding a todo does not re-render. Group 8 "Todos hygiene" RETIRED: it required `status` as a str, so its purpose (no done-trophies) is structurally impossible; measured before retiring — done-markers in task text: 1 hit in 191 fleet todos, not a trophy; its 5 points go to Entry shapes (25→30, `trinity_check.py` 1.1.0, weights still sum to 100). `trinity_content.py` 1.1.0, `trinity.md`, README counts (3393 functions / 4143 cases). +19 pins in the existing `test_trinity_check.py`, 15 mutants red; the byte-identity guard against memory's live `compose_meta` runs 4 todo_ctx cases and 4 config edits. Memory's `TestThePushedFileIsCanonical` passes strictly. Fleet Trinity pre-push 95→89 average by design: the new mirror flags status-bearing todos under Entry shapes on 17 branches until the push moves them; the memory row's Meta lines & _usage is already clean.

### Fixed

- **devpulse: the bare self-map and `--help` print to stdout** (found 2026-09-15 measuring self-map sizes fleet-wide for the boardroom thread 16 / DPLAN-0347: devpulse read as 0 chars while every other branch printed 200–1,600). `apps/devpulse.py` aliased `console = err_console` since a 2026-03 compliance sweep with no reason recorded, so the entry point's inventory and usage went to stderr — the same class as the feedback module's fix (`test_help_goes_to_stdout_so_a_redirect_captures_it`). Now imports cli's stdout `console`; `err_console` stays for refusals. Live: bare 667 / help 967 chars on stdout, 0 on stderr; 86 console tests green.
- **memory: `todo restore` re-renders the todos tab, and three more `sys.modules` leaks in memory's tests are undone at teardown** (memory owning, devpulse landing; the rows seedgo's new `module_eviction` rule and the `-n 3` bar surfaced). Restore wrote the pad with a stale `todos_meta` (canary's tab said `next #15` with #15 on the pad), so an agent copying the tab by hand would have collided — `rollover/todo_roll.py` 1.1.1 → 1.2.0 `_rendered_todos_meta()` composes the line through the existing `tracking/tab_renderer` (the push and refresh renderer, no second one) inside the same pad write that is read back and verified; if the tab cannot be rendered the restore refuses and writes neither file. Proven on canary: `todo restore 2` → `#2 -> #16`, tab `next #17`, backlog `high_water: 16`. Leaks: `tests/test_rollover.py` 1.1.2 (the cold-submodule-cache test bare-popped the cli submodules — help_flags, json_flag and branch_flag were new module objects after teardown, seedgo's runtime evidence), `tests/test_config_verbs.py` 1.4.1 (the `verbs` fixture's `_evict()` now records every module-cache entry and parent attribute through monkeypatch, still re-imports against the real cli), `tests/test_rollover_pipeline.py` 1.2.1 (`_import_rollover_module` had the same bare pop and turned the contract test red under `-n 3` when pipeline rollover tests ran before contracts — pre-existing, found by the bar); each pinned with a teardown-restore test, 8/8 mutants red. `module_eviction` on memory: 26 rows of 2052 (was 29), none in the three files. Left open, memory's list: 26 bare-pop rows in other memory test files (test_archive, test_auto_process, test_watch_module, test_symbolic_extras, test_unified_schema, …) may leak the same way under another worker order — a sweep is a separate row. Verified from the devpulse seat: memory suite 1707 passed / 2 skipped from the repo root in CI shape, ruff check + format clean, the four leak-order files before `test_contracts.py` 454 passed.
- **memory: the backlog path in the push note and the roll receipt is posix on every OS** — the `windows-setup` CI job was red on 02610e3b, 561678fd and 3c426914 (runs 34960702085 / 34990875051, one failure each, `memory/tests/test_trinity_push.py::TestTodosMoveToTheBacklog::test_the_note_names_the_backlog_and_stays_canonical`: `assert 'todo/guinea/backlog.json' in '1 todo(s) #9 moved to C:\\Users\\…\\.backup\\todo\\guinea\\backlog.json …'`). Both display helpers render the path repo-relative with `as_posix()` when it sits inside the repo root and fell back to `str(path)` when it does not — which is every pytest `tmp_path`, and on Windows `str()` spells backslashes. trinity_push.py 1.3.0 → 1.3.1 `_backlog_display` and todo_roll.py 1.1.0 → 1.1.1 `_display` fall back to `Path(path).as_posix()` — the same species in both, one line each (devpulse edit, memory mailed). Emulated: `PureWindowsPath(r'C:\…\.backup\todo\guinea\backlog.json').as_posix()` carries `todo/guinea/backlog.json`. Linux: test_trinity_push + test_rollover_pipeline 212 passed, ruff clean; the Windows job on the next push is the proof. Read this morning from the job log via `gh api actions/jobs/<id>/logs` (6.6 MB) — the "admin-gated 403" of the night was the unauthenticated gh after the reboot, not a permission.
- **memory: `todo restore` lands on top, and todo numbers never walk backwards** (FPLAN-0590 follow-up; memory owning, devpulse landing; three findings from canary's proof, finished under the RESUME dispatch after the 04:20 power loss). Restore appended the restored todo at the tail (`list(todos) + [restored]`), so the sanctioned verb wrote what seedgo's newest-first Trinity check fails (canary scored 99) and the restored todo sat where the oldest belongs — `rollover/todo_roll.py` 1.0.0 → 1.1.0 inserts it at index 0. Number reuse after a hand delete is closed with a floor: `next_number` = `max(pad numbers, backlog entry numbers, backlog document_metadata.high_water, N-1 from the branch's own rendered "next #N") + 1`; `high_water` is raised, never lowered, on every memory backlog write (roll, push, restore) and compared on read-back; a backlog without the key (all 18 written on 2026-09-15) is no floor, never an error; the renderer (tab_renderer 1.5.0) and the push (trinity_push 1.3.0) share it, so the `next #N` agents copy never goes backwards after a deletion. Residual, stated in the contract (s5), README and the docstring: a todo added by hand and deleted by hand with no memory write or tab render between leaves no trace, so its number can be issued once more — curing that needs the edit hook to observe writes, not memory. Also cured in the same pass: the macOS CI red — and the Linux 3.13 red at 3c426914, job 104454882390 — on `tests/test_contracts.py::TestUnknownArgumentExitsNonZero::test_a_known_verb_given_a_bogus_subargument_exits_non_zero` was test pollution, not the product (a real `rollover <bogus>` exits 2): `tests/test_rollover.py` evicted the rollover module with a bare `sys.modules.pop`, re-imported it bound to a MagicMock cli, and teardown restored the cli but not the module, so the next in-process `memory.main()` on that xdist worker routed the bogus subarg through a mock `error()` that marks nothing → `resolve_exit` 0; it predates the landing (reproduced at af83f7c7) and which worker inherits it is loadscope's size-ordered queue, which is why it flickered by platform and Python; 1.1.0 → 1.1.1 evicts through monkeypatch and pins the teardown (`TestMockedReimportIsUndoneAtTeardown`), 2/2 mutants red, the two-file order (`test_rollover.py` then `test_contracts.py`) passes single-process and `-n 3`. `rollover check --branch @x` now labels its file list "(fleet-wide)" and says in `--help` that `--branch` scopes only the todo pad line (rollover.py 0.7.1) — the classic walk is unchanged. Five test files +16 pins, 27 mutants red; contract doc s2/s3/s5 + README. Verified from the devpulse seat: memory suite 1703 passed / 2 skipped from the repo root in CI shape (`-n auto --dist loadscope`), ruff check + format clean (138 files), the leak order 57 passed; on canary, deleting #12 by hand then `todo restore 3` wrote `#3 -> #15` at index 0 — pad strictly descending [15, 14, 11 … 4], backlog `high_water: 15`, seedgo Trinity 100 (was 99).
- **memory's gold trinity templates spell the branch placeholder as spawn does** (`{{BRANCH}}` → lower; memory owning, devpulse landing; the drift devpulse mailed @spawn on 2026-09-15). Gold carried `{{BRANCHNAME}}` (renders UPPER) while spawn's citizen seeds carried spawn's own correction, so a real `templates spawn-templates` write would have reverted spawn — and the todos-v2 landing (02610e3b) changed gold's `_usage` + todos prose without pushing the seed, which is the one red on CI run 34959498046 (`spawn/tests/test_birth_receipt.py::test_trinity_seed_matches_gold_template[local.json]`, all four Pythons). `LOCAL.template.json` ×4 and `OBSERVATIONS.template.json` ×3 now spell `{{BRANCH}}`; `monitor/detector.py` 0.4.1 and `templates/trinity_push.py` 1.2.1 resolve it to `branch.lower()` (the push would otherwise have written a literal `{{BRANCH}}` tag into every frame); `spawn_pusher.py` 0.1.1 docstrings; `test_detector.py` 1.2.1 (three recreate expectations lowercase), `test_trinity_push.py` 1.1.1 (+1 pin: no `{{BRANCHNAME}}` in either gold, tags resolve with no `{{` left); 4/4 mutants red. `drone @memory templates spawn-templates` run for real: 1/1 sets, `citizen/local.json` updated, both seeds byte-identical to gold, second dry-run "already up to date". spawn's `tests/test_birth_receipt.py` 1.0.1 drops the `{{BRANCH}}`→`{{BRANCHNAME}}` normalisation the drift had needed: the seed is a byte copy of gold now, a drift of one character fails there (devpulse edit in spawn's tree, owner mailed). Memory suite 1687 passed / 2 skipped from the root; ruff clean.
- **hooks: `tests/test_engine.py` wrote `~/.aipass/trusted_projects.json` under a fake HOME** (seedgo's runtime leak probe; hooks owning, devpulse landing). `TestErrorResilience::test_corrupt_hooks_json` reached `bootstrap()` because `REGISTRY_PATH` binds `Path.home()` at import. `tests/conftest.py` 2.1.0: autouse `isolated_trust_registry` points it at a per-test sandbox (the `isolated_cadence_state` shape) so no future test that walks into a hooks.json can leak the same way; `test_engine.py` 1.1.0 pins that bootstrap lands in the sandbox, asserting the path is not the live file BEFORE calling. Found on the way: `test_corrupt_hooks_json` and `test_empty_hooks_json` passed for the wrong reason (the temp project was never enrolled, so the trust check returned None before the parse they are named for) — both now enrol first and assert the "bad config" line. Probe re-run: 2009 items, 0 home writes, 0 network; 2 mutants red.
...
- **aipass: `aipass baud install` also lands `baud-cli` — the headless binary arrives from the same release as the phone face, is verified against the same `SHA256SUMS.txt`, and the api is pointed at it (FPLAN-0589 row 3 of four; aipass owning, devpulse landing).** New `handlers/baud/binary.py`; `modules/baud.py` 1.1.0, `modules/install.py` 1.2.0. Fetch `baud-cli-<tag>-linux-x86_64` through the face's door (public download first, then the API asset route with the seat's token), verify by the asset's own line in the sums file, land at `~/.aipass/baud/bin/baud-cli` by staging, a second hash, 0755 and rename (`baud-cli.prev` kept), the `.baud-cli.json` marker (tag, sha256, installed_at, source) written last, then api's `set_baud_bin` through api's module door (`aipass.api.apps.modules.host_api`) — the face calls moved to the same door and aipass's FPLAN-0587 encapsulation bypass row is retired (27 → 26). Every file is verified and the binary's location checked before anything is placed, so a refused binary leaves no new face beside it; a binary with no marker is refused. `--from <tar>` gains `--binary <file>` (sums beside, or `--sums`); `--dest DIR` puts the face in DIR and the binary in `DIR/../bin`; a platform with no matching asset (anything but linux x86_64 today) says so in one line, installs the face only and exits 0. `aipass baud status` shows both rows; `aipass install` step 4 is "Phone face + baud-cli". Proven from the devpulse seat: `--from --binary` with the locally built binary, then the network route against v0.2.1 (`github-api`, sha bd60c8a4…), both undone with `set-config … default`. Also cured, a leak of its own: `test_install.py` ran the real step 4 unpatched, and once baud's releases carried the bundle every suite run on a seat with a gh token fetched the latest release into the real `~/.aipass/baud` and re-pointed the live api (the commit gate at 11:30 the same day did) — an autouse guard now stubs the step for every test and fails at teardown on any `urlopen` call (probe: 1 request before, 0 after). 1101 green from the repo root, ruff, pyright and audit clean; the 100-case draft stays parked in `docs.local/` behind the test-write gate (32 new cases, 7/7 mutants red, run in memory).
- **api: the baud binary is a setting — `baud_bin` in the host_api config, `host-api set-config --baud-bin <path>|default`, and the fleet lanes resolve `baud-cli` per request in a pinned order (FPLAN-0589 row 2 of four, DPLAN-0343; api owning, devpulse landing; Patrick 2026-09-14 11:59: "binary shape it is").** Why: a server or container with no desktop has no GTK, so the desktop `baud` cannot answer the fleet, census, roster and room lanes there — baud v0.2.1 ships `baud-cli`, the same six headless verbs built without the window, and the api now finds it. `handlers/host/config.py`: `baud_bin()`, `set_baud_bin(path | None)`, `BaudBinRefused(ValueError)` — four legs validated before any write (absolute, exists, regular file, executable), `None` clears, clearing an unset key writes nothing, the other keys survive; a refused `--baud-bin` stores no `--face-dir` from the same call. `handlers/host/fleet.py` 1.1.0: `locate_binary()` → `BinaryLocation(path, source, looked)` and `snapshot_binary()` resolves on every request (no restart, unlike the face): configured (refused by name if it is gone or lost its bit) → `~/.aipass/baud/bin/baud-cli` → checkout `target/release/baud-cli` → checkout `target/release/baud` → `baud-cli` on PATH → `baud` on PATH; an automatic step counts only an executable regular file and names the one it passed over; the not-found text lists every place in order and ends "Install it: aipass baud install." The six subprocess call sites are unchanged. `host-api config` shows `binary: <path> (configured|installed|checkout|path|missing)` and `executable: yes|no`. `modules/host_api.py` 1.2.0 hands `config`/`set-config` to the new `host_config_cli.py` sub-router (the module had reached 726 lines against seedgo's 600) and carries the module door for @aipass: `load_config`, `face_dir`, `set_face_dir`, `baud_bin`, `set_baud_bin`, `FaceDirRefused`, `BaudBinRefused`. +24 pins in the existing `test_host_api.py` / `test_host_fleet.py` (1684 → 1708 from both rootdirs), the resolution suite rewritten with every place isolated; 27/27 mutants red; ruff, pyright and audit clean. Windows CI cure rides along: the two `TestIntrospection` pins that painted 9594a1a8 red searched a path inside a joined repr of mock calls (repr doubles the backslash) — a `_printed(mock)` helper now joins the call arguments as plain strings, and all 20 such joins in the file are swept.
- **drone: `drone @git <verb> --repo <path>` — git write into an external repo (projects/baud, Vera-Studio, any repo that is not AIPass) from the admin seat only (DPLAN-0344; drone owning, devpulse landing; Patrick 2026-09-13 23:13: external repos "don't get my attention nearly enough", git write there goes through the admin badge, project managers excluded).** `apps/handlers/git/repo_door.py` 1.0.0 + `git_module.py` 1.3.0: `--repo` on status, diff, log, commit, push (new, door-only: the checked-out branch to origin under its own name, never forced, refuses a detached HEAD) and tag. Order is the contract: WHO first (`is_verified_admin_caller()`, the 5-leg admin grant through ai_mail's verified-caller rail, asked on every use, never cached; drone stamps the caller identity fresh the way the router stamps a child), then the flag's shape, the verb, the repo. AIPass itself is refused two ways (its registry at the target, or the caller's own root); a folder inside a repo, a missing path, a non-repo, a bare or doubled `--repo` and a verb outside the six are refused by name. No lock taken (git's own index.lock serialises a write; AIPass's PR lock would stall its train for another repo). One ledger line per exit, refusals included, in `.ai_central/git_repo_door.jsonl` beside drone's deletion log, same rotation bound. The gh passthroughs (issue/run/workflow) keep the CLI's own `--repo OWNER/NAME` and never reach the door. 13 pins in the existing `test_git_access.py`, 13/13 mutants red; drone suite 1359 green; hooks' git_gate unchanged (its lookbehind skips `@git`, drone's shell-outs are subprocesses the hook never sees). First live use the same night: baud 774dace + aaec11e committed, pushed, tagged v0.2.0 through it, release run green. Also cured on the side: `run view --log` / `--log-failed` printed nothing on gh 2.45 against GitHub's job-level log archives — an empty exit-0 answer now falls back to the jobs API (job name and a tab as the first column, `--log-failed` = the full log of each failed job, an API refusal fails by name), 7 pins in `test_git_module.py`.
- **aipass: `aipass baud install` — the phone face arrives from a baud release, not a source checkout (FPLAN-0587 row 2 of three, DPLAN-0343 tier 2; aipass owning, devpulse landing; Patrick 2026-09-13: "ok let move on ur direction").** `apps/modules/baud.py` + `apps/handlers/baud/{fetch,verify,unpack,installer,point}.py`: `aipass baud install [--tag <vX.Y.Z>|latest] [--from <tarball> [--sums <SHA256SUMS.txt>]] [--dest <dir>] [--dry-run]` and `aipass baud status`. Fetch tries the public release download first, then, with a token (`GITHUB_TOKEN`, else the gh CLI's login), the API asset route — the token rides `add_unredirected_header` on api.github.com only and is never printed; `latest` resolves the tag first so tarball and sums come from one release; no token and 404 says the repo is private or the asset is missing and names `--from`. Verify parses `SHA256SUMS.txt` (sha256sum format; `--from` without `--sums` reads the one beside the tarball), no sums = no install, a mismatch deletes a download and never a `--from` file. Unpack validates every member before a byte is copied — links, devices, absolute names, `..`, backslashes, drive letters, any directory but `assets/` refused; `phone.html` at the root and `assets/` required — into a staging dir beside the destination, swaps it in (`phone.prev` restored on failure), and writes the `.baud-phone.json` marker last; a non-empty destination without the marker is refused. Default destination `~/.aipass/baud/phone/`. Then api's `set_face_dir(dest)` in-process (api validates, aipass never writes api's config file) and the restart line. `aipass install` 1.1.0 gains step 4 of 5 "Phone face", best-effort — offline, private repo or no token prints one honest line plus the retry command and the install continues; `--no-baud` skips it, `--dry-run` walks it. stdlib urllib only. +4 pins in the existing `test_install.py` (1097 → 1101 from the repo root); the 68-case `test_baud_install.py` draft was refused by the test-write gate and stays parked in aipass's `docs.local/` by Patrick's ruling ("we still have a lot of work to do on test so id rather not stack that issue"). Live from the devpulse seat: a tarball packed from `dist-phone` with the release recipe installed to `~/.aipass/baud/phone` and the host api served it after a restart (log "served from … (configured)", served sha == installed sha, `/v1/*` still 401 without a token), then `--face-dir default` put the checkout back. The network fetch, proven 2026-09-14 00:22 once baud v0.2.0 was released with the phone tarball (the first release carrying one): `--tag v0.2.0 --dry-run` walked the four steps, then a real `--dest` install fetched through the API asset route with the seat's gh token (`source: github-api` in the marker), verified against the release's `SHA256SUMS.txt` and unpacked 9 files. Docker stranger test the same night (fresh ubuntu 24.04, `aipass install --here`): step 4 failed honestly without a token, `--from` the local tarball installed, and Patrick signed in from his phone against the container.

- **api: the phone face's directory is a setting — `face_dir` in the host_api config, `host-api set-config --face-dir <dir>|default`, `face_location()` configured-else-checkout (FPLAN-0587 row 1 of three; api owning, devpulse landing).** `handlers/host/config.py` 1.1.0: `face_dir()` and `set_face_dir(path | None)` validate absolute → directory → `phone.html` BEFORE any write and refuse with `FaceDirRefused(ValueError)` storing nothing (the bind rule's doctrine: refused at write time, where whoever typed it is watching); `None` clears, clearing when unset writes no file. `handlers/host/face.py` 1.1.0: `FaceLocation(root, source)`; a configured dir that lost `phone.html` is reported AS the configured dir with its fixes (`aipass baud install`, or `set-config --face-dir`), never swapped for the checkout build — serving a bundle nobody chose would be a lie; a relative stored root never serves. `server.py` resolves the face ONCE at `create_app()` and every face route serves that snapshot (`statics.face_file_route` now takes the resolved path — re-resolving per request would have served a new `phone.html` against the old `/assets` mount), so a running server needs a restart to see a new dir, stated as a known limit in code, README and the CLI. `host-api set-config` is one command with one outcome: a refused face stores no bind and a refused bind stores no face; `host-api config` shows the face dir, its source and whether `phone.html` is present. The help's SECURITY line is now READ from `LOOPBACK_ONLY` ("Non-loopback binds are open since the 2026-08-14 security review"), curing the stale "Phase 1 is loopback-only" line @baud's audit caught. `is_face_available()` removed (unused once the server took the snapshot). +35 in the existing `test_host_face.py` / `test_host_api.py` (1649 → 1684 from both rootdirs; the locating tests run under an isolated config store), red-first 34/34, 21/21 mutants red, ruff + pyright clean, audit 100. `server.py` sits at 1498 of 1500 lines: the next host route splits the proxy routes into a router first.

- **skills: `machine_vitals()` cpu section grows `cores`, `logical`, `physical`, `mhz` and `mhz_max` — the per-core read behind BAUD's cpu page (FPLAN-0586 row 1 of two; skills owning, devpulse landing; Patrick 2026-09-13: "when i open it id expect to see the cores").** `system_status` 2.1.0 → 2.2.0: ONE `cpu_times(percpu=True)` sample per read, the baseline holds the per-CPU list, and the headline `percent` is the sum of busy over the sum of total across the same deltas the cores use — summed, not averaged (3 busy of 5 ticked reads 60 where the mean of the bars would say 37.5), so the headline and the bars agree by construction. `cores` is a list of one-decimal percents per ONLINE logical CPU in index order, or `None` while warming — never a list with holes; any CPU with no tick makes the whole section `warming`. `logical`/`physical` are the CONFIGURED counts, `mhz`/`mhz_max` the package clock from `cpu_freq()`; all three are instant readings published even while warming, and `None` where the host gives none (a `cpu_freq()` that raises `NotImplementedError`/`OSError`, psutil 7.2.2's two Linux paths, costs only the frequency with `detail` naming it; the `/proc/cpuinfo` fallback's `max 0.0` also reads `None`). `VITALS_SCHEMA` stays 1 (additive); the api relays the dict verbatim, so `GET /v1/machine` carries the keys with no api change. Cost 0.85 ms per read. Tests in the existing `test_machine_vitals.py`, 32 → 41 functions / 45 → 57 cases behind a four-CPU stand-in (CPU 2 spends half its idle in iowait); 17/17 mutants red; 1515 green from both rootdirs, ruff clean, audit 100. Live on this laptop (i5-4308U): `cores [1.6, 3.2, 1.6, 4.8], logical 4, physical 2, mhz 1125.0 / 3300.0`.

- **api: `GET /v1/lock` — whether the laptop's screen is locked, for BAUD's header lock chip (FPLAN-0585 row 2 of three; api owning, devpulse landing; Patrick 2026-09-13: "did I lock the laptop? ... are we able to detect its state").** `handlers/host/lock.py` under `read` scope, the machine proxy's pattern: the skill's door imported inside the call, a `ReadCache("lock_state", 1.0)` single flight, the answer relayed verbatim — all six keys, the sentence included. `ok: False` (the skill could not tell: `no_session`, `no_reader`, `read_failed`) is a 200 reading served for its window, never an error and never drawn as unlocked. 503 `lock_door_failed` only when the door raised or answered outside its shape — a boolean `ok`, a `locked` key, and the two agreeing (a bool when ok is True, None when False), so `{ok: False, locked: False}`, the one answer a chip could misdraw, is refused rather than relayed. Any parameter is 400 `lock_parameters_refused`. `POST /v1/verbs/lock` is untouched. +25 cases in the existing `test_host_fleet.py` (1624 → 1649 from both rootdirs), 15/15 mutants red, ruff clean, audit 100; live after a unit restart: three 200s with the unlocked body inside one second, one read. `server.py` now sits at 1495 of its 1500-line cap: the next host route splits the proxy routes into a router. Known limit on record in FPLAN-0585: the skill's reads carry no subprocess timeout, so single flight bounds the reads of a hung logind to one but not the wait behind it.

- **skills: `lock_state()` — the screen lock's position, published beside `lock_screen()` in `lib/screen_lock` (FPLAN-0585 row 1 of three; skills owning, devpulse landing).** 1.0.0 → 1.1.0: `{ok, locked, method, session, reason, detail}`, read from logind's `LockedHint` through `loginctl show-session` — the mechanism the lock itself writes through, so read and write agree by construction — with `org.gnome.ScreenSaver.GetActive` behind it; `_resolve_session()` tells `no_session` (no graphical logind session) from `no_reader` (a session but nothing to read it with) and `read_failed`, precedence no_session > read_failed > no_reader, and the read never raises (catches widened to `OSError`/`SubprocessError`). A new `state` action: `drone @skills run screen_lock state` prints the sentence. Tests 22 → 42 in the existing `test_screen_lock.py` behind a strict stub that raises on any argv outside the read tree; 22/22 mutants red; 1503 green from the repo root, ruff clean, audit 100. Live on this laptop: `ok True, locked False, loginctl, session 3`.

- **seedgo: `calendar_bound` — the 47th checker (48 scored entries): a test that asserts a date literal against production that computes with the clock, and does not own that clock, is convicted before it can merge (seedgo owning; devpulse landing; Patrick's standing rule on red, 2026-09-08: the defect class goes into the standard first, then owners cure).** Why nothing flagged the daemon bomb that painted main red on 2026-09-13: no rule in either pack read the clock — `pytest_quality` judges what a test proves and a literal equality is a strong oracle, the host rules ask which host a test needs and never which date, and the execution lanes run the suite on today's date, the blind spot itself. The rule stands on three legs: the unit asserts a date literal (ISO string, or `datetime`/`date` built from int literals) in an `assert` comparison or the `assertEqual` family; following calls by name from the unit into production (6 hops, patched calls cut from the walk) reaches a function that *computes* with `datetime.now`/`utcnow`/`today`, `date.today` or `time.time` — arithmetic, ordering, equality, or handed to a call; a stamp (`isoformat`, `strftime`, a `None` check, logged, stored) is not a derivation; and the unit does not own the clock — no patch or monkeypatch naming `datetime`/`date`/`time`/`now`/`today`/`clock`/`freeze` in the unit, its class helpers or a same-file helper it calls, no injected `now=`/`clock=`/`timestamp=`, no clock-named fixture, no `freeze_time`/`time_machine`, no autouse fixture in the file or a conftest; a patch on `sleep`/`monotonic`/`perf_counter` owns nothing. A date the production spells in its own string constant is acquitted. Measured to the RPLAN-0004 rule over 18 branches and 603 test files in 9.3 s: before, daemon audited 100 on all 47 with the bomb in the tree; the shape as first posed convicted 58 (1 real, 57 round trips where a date goes in through a helper and comes out unchanged while the module also stamps `now`), a textual narrowing was rejected because the incident's date sits in its own helper default; the landed rule catches the pre-cure `test_run_module.py` line 404 four calls in (`_tick` → `run_tick` → `_tick_body` → `runstate.is_catch_up_fire`) with 0 false positives — 9 acquittals named on the context line (devpulse `test_watchdog_schedule` ×6 on a clock fixture, daemon and ai_mail injected instants, one production-spelled ruling date); after, 18/18 branches at 100, no fleet rows besides the incident, which b66f30cb already cures. Scored and CI-gating like `host_portability`: `.github/scripts/seedgo_audit.py` `EXPECTED_STANDARDS` 47 → 48. +10 pins in the existing `test_aipass_standards.py` (the contract, the incident convicted, freeze in a same-file helper, injected `now=`, stamp is not a derivation, patched seam cut, production-spelled acquitted, a sleep patch owns nothing, file-share score, a lib skill's own test dir in the corpus); 12/12 mutants red; 4040 passed / 53 skipped from the repo root, ruff check + format clean, pyright 0 on the new files, audit 100 on 48. Known limits, stated in `calendar_bound.md`: expected values in parametrize tables and calls through arbitrary objects are not read. Not built, for Patrick: a backstop for what a static reader cannot see — a nightly one-leg run of the suite on main (~8 runner-minutes a day; would have shown red around 03:40 UTC Sunday), or a PR leg under a shifted clock (libfaketime `+30d`; would have gone red at the v2.8.7 merge).

### Changed

- **skills: Telegram is retired — the skill stays in the tree, switched off, its tests skipped, and the one door that still delivered around the switch is gated (skills owning, devpulse landing; Patrick 2026-09-14 14:29: "Tg should be skipped ignored by all. We keep the work in place but disable it and skip it in tests. Do not waste time fixing test for tg").** BAUD's phone face is the notification and control surface now. `lib/telegram/tests/conftest.py` 1.1.0: a `pytest_collection_modifyitems` hook marks every item under that directory `skip` with the ruling as the reason (a skip, not `collect_ignore`, so the modules still show; the path filter is load-bearing — without it the hook skipped all 1518 skills cases, measured); the skills suite goes 1515 passed → 404 passed / 1114 skipped, 164 s → 45 s, and seedgo's runtime-probe row in `test_log_streamer` (a write to `~/.aipass/telegram_bots`) is skipped, not cured. The off-switch record (OFF since 2026-08-18) carries the ruling as its reason; `switch_handler.off_reason()` is new and `runner.py` 1.2.0 prints it in the one-line refusal: `Error: Skill 'telegram' is switched OFF and will not run (Telegram is retired — …). Turn it back on with: drone @skills on telegram`. Found on the way: `@daemon`'s `schedule/telegram_notifier.py` imports `send_telegram_notification` in-process on every job fire and never passed the runner gate, so it was still delivering with the skill OFF — 2 sends on 09-14 07:47, 5 to 17 a day since 09-07 (`skills_json/notifier_log.json`); `notifier.py` 1.2.0 now asks the switch itself and returns False before config or network when OFF or unreadable, daemon's fail-soft path untouched. +3 pins in the existing `test_switch.py` (`TestTheNotifierDoor`: off sends nothing, unreadable sends nothing, on reaches `load_bot_config` and `urlopen`), the two runner-gate cases assert the exact refusal line; 6/6 mutants red. Nothing deleted, moved or renamed. Not touched, on Patrick: the user template unit `~/.config/systemd/user/telegram-bot@.service` is disabled, not masked, so a new instance name could still be started by hand.

### Fixed

- **seedgo: `posix_literal` arm 4 (REPR-HAYSTACK) now reads a path searched inside a join over a mock's call records, and a needle that names a path only one binding hop back or through a call named for one (seedgo owning; devpulse landing; Patrick's standing rule: the red goes into the standard first).** Why the two api pins that painted 9594a1a8 red on Windows (run 34882199024) were never scored: both halves of the arm missed at once — the haystack's `str()` sat inside `" ".join(...)` over a comprehension variable and the old reading looked only at the outermost expression, while neither needle spelled a path (`str(bundle)`, `str(host_face.face_root())`); each widening alone still catches neither row, measured. `posix_literal_check.py` 1.2.0: the rendering is found anywhere in the haystack expression; a comprehension variable iterating a call record counts as one, a variable over `call.args` does not (that is the cure shape and stays clean); a needle bound one hop back to a path-ish value, or a call whose own name carries a path word; a list record's cure now names an index (the old text sent owners to `.call_args_list.args[n]`, which raises). Counts over 18 branches: at 9594a1a8 the two red rows score (api 100 → 99), 0 rows anywhere else; at HEAD 0, api's cure is in; the widened reading sees 112 in/not-in comparisons fleet-wide against 34 before, none with a path-ish needle outside the two. +5 pins in the existing `test_pytest_quality_pack.py`, 8/8 mutants red. Advisory pack, no gate change.
- **seedgo: `dead_code` rule 5 reads a name inside a parenthesised multi-line import — drone's false positive of 2026-09-13 (seedgo owning; devpulse landing).** `dead_code_check.py` 1.1.0, comments stripped; drone's `repo_door` rejoined into its import block now reads referenced; 0 rows moved across 18 branches. +2 pins in the existing `test_checkers_batch4.py`, 2/2 mutants red. seedgo's README test count re-counted (62 files, 3374 functions, 4100 cases) — the calendar_bound pins had pushed it past the 10% drift line.
- **hooks: compass ambient recall went silent after five surfacings and stayed silent for the rest of the session (FPLAN-0588; hooks owning, devpulse landing; Patrick 2026-09-14 00:15: "does it still surface info to u?").** Measured from the devpulse seat: the recall hook found three candidates on every prompt and refused every one with "Session budget exhausted (5/5)" — 93 prompts in a row on 09-13 — because `compass_recall.py` passed no config to memory's governance engine (default 5 per session) and the `max_per_session: 10` knob in hooks.json only reached the hook engine's own per-session budget, never governance; both budgets were keyed to the Claude session id, which survives compaction. `compass_recall.py` 1.1.0 reads its own hooks.json block and passes `max_per_session` → `max_surfaces_per_session`, `threshold`, `min_messages_between`, `cooldown_seconds` (non-numeric or missing → engine defaults), and logs each budget, spacing or cooldown refusal with the id and reason. `cadence.py` 2.4.0 stamps when a context window opened (PreCompact's existing `reset_counter`, also startup and `/clear`; resume does not) and on the next prompt both budgets reopen — surfaces, spacing and last-surface time zeroed, `surfaced_ids` kept so nothing repeats; `engine.py` 1.5.0 reopens its fire count on the same stamp. Replayed on the live state: 5/5 refused before, `[IMPRESSIVE] #103` surfaced after, a spent window reopened by PreCompact. +6 pins in the existing `test_compass_recall.py` against the real engine, 12/12 mutants red; hooks suite 1998 green; seedgo 100. Live within the minute of landing: the first surfacing in eleven hours arrived on the next prompt of the devpulse session.
...
- **api: `GET /v1/machine` — the machine's vitals on the host API, a proxy to @skills' published `machine_vitals()` and nothing more (DPLAN-0341, FPLAN-0561 row 2 of three; api owning, devpulse landing).** The lock verb's pattern, applied to a read: `handlers/host/machine.py` imports the skill's door inside the call (a server that never serves the route loads no `system_status`), asks it through a `ReadCache("machine_vitals", 1.0)` single-flight so any number of phones cost at most one read per second, and relays the dict and every sentence in it verbatim — *rewriting another branch's diagnosis is how one gets lost.* Absence is a 200 carrying the skill's own `available: False` section, values `None`, never a zero. Every `ok: False` refusal is a 503 by the rule the snapshot and kill seams already hold — a closed door answers an honest 503 that names itself, never a 200 dressed as a reading: `error.code` `machine_refused`, the skill's code in `error.reason`, its `detail` as `error.message` verbatim, which covers `psutil_missing`, `switched_off` and any code the server has never seen. A door that raises, or answers without a boolean `ok`, is 503 `machine_door_failed` naming the exception, never a 500 traceback. A refusal or a door failure is never cached (raising is the only way to keep one out of `ReadCache`, which stores whatever the producer returns). The route takes no parameters and refuses any with 400 `machine_parameters_refused`, as `/v1/roster` does — a `?section=fan` the route ignored would read as a filter that worked, so the phone's transport must not add a cache-buster. No cache-control header, matching `/v1/fleet`. Live on this box against the restarted unit: three calls, the first two inside one second sharing one `sampled_at` (the first read after a restart relays the skill's `warming` as-is), the third a second later carrying fan `Right Side` 3554 rpm at 46% of 1299–6199 — read while both api suites were running, the wheel doing the job Patrick named. Tests extended in `test_host_fleet.py`, no new file: +17 cases, stand-ins only, the skill's door patched with autospec — manufactured macOS (temp and fan `platform` with the skill's sentence, body equal to the stand-in so any adapter goes red), refusals parametrized incl. a code from the future, a raising door, a malformed answer, a parameter, the cache window pinned on a clock the case owns (0.9 s stored, 1.1 s fresh), a fresh interpreter importing `machine.py` loading no `system_status`; 10 mutants red. 1605 → 1622 from both rootdirs, ruff clean, audit 100.
...
- **api: `store_usage_data` is read-modify-write and survives the service's own document shape (api owning, from its 09-13 05:00 rounds; devpulse landing).** `tracking.py` 1.0.0 → 1.1.0: loads the usage doc, sets only its own keys (`data`, `last_updated`, `setdefault` on `module_name`/`created`) and writes every other key back as read - prax's FPLAN-0542 rule (devpulse 262dd23f). Worse than reported: the live `api_json/usage_tracker_data.json` is the service's document with no `data` key, so the old read died on `current_session` and every generation tracked since 05-02 would have been refused. Red first - 2 new cases in the existing `tests/test_tracking.py` for the two faults - then 7 of 7 mutants red; 15 green from the repo root. One pin moved: a fresh file's `module_name` is `usage_tracker`, the service's name for it (nothing reads the old `api_usage`).

- **hooks: the shipped hooks config (`.aipass/project_hooks.json`) wires the `.trinity` shell tripwire - `trinity_tripwire_snapshot` on PreToolUse and `trinity_tripwire` on PostToolUse, matcher `Bash` (DPLAN-0342 row 3 B; devpulse landing).** A fresh clone gets it on enrol. This repo's live `.aipass/hooks.json` is hash-enrolled and any byte change takes every hook dark until a human re-enrols with `aipass trust <repo>`, so the live wire waits for Patrick's hand; Row A (the refusal) needs no wire and is live already.

- **memory: every `.trinity` meta line now carries a draft target beside its cap - `task ≤150 chars · draft to 120`, `value ≤200 · 160`, `summary` and `note ≤300 · 240` (DPLAN-0342, Patrick's ruling 2026-09-13 01:43; memory owning, devpulse landing).** `entry_limits.py` 1.8.0 → 1.9.0 adds `DRAFT_PERCENT = 80` and `draft_target(max_chars) = max_chars * 80 // 100`; `tab_renderer.py` 1.2.0 → 1.3.0 renders it in all three tab shapes. Derived at render time, never stored: a `per_branch` cap of 500 renders 400 with no second number to go stale. Cap, `enforce` and the config schema are unchanged, so the hooks gate reads the same record. Why: DPLAN-0342 mined 2280 fleet memory edits (9.9% rejected, median overage 7% of the cap, `sessions.summary` worst at +15%) and a field replica that saw only the meta line wrote to 294/300 - text goes to whatever number the line shows, so the line now shows the number to write to. Tests edited in place (`test_tab_renderer.py` pins all four tab endings, `test_trinity_standard.py` pins `per_branch` 500 → 400 and a floor table incl. 77 → 61); 6 mutants red; memory suite 1583 passed; audit 100. The fleet re-render (`drone @memory push --confirm`, 22 branches, 936 entries carried, 2 archived) was fired by devpulse at 02:10 so every agent's meta line shows the target now; `sessions.summary` stays 300 by the owner's call - the 08-31 fleet median already sits at 0.94 of that cap and a bigger cap moves the bounces with it.

- **seedgo: the trinity checker's `expected_meta_line` mirrors the new `· draft to N` tab suffix (DPLAN-0342, Patrick's ruling 2026-09-13; memory owning the renderer, seedgo owning the mirror).** All three tab shapes carry `draft to {draft}` before the closing bracket, `draft = max_chars * 80 // 100` from the same resolved cap (per_branch included), floored - 77 drafts to 61. `trinity.md`'s example shows it and says what the number is: a length to write to, derived and never stored, not a second cap. memory's `test_a_pushed_branch_satisfies_the_trinity_checker` goes green with no edit on its side. +7 cases, 4 mutants red (round instead of floor, the default cap instead of the resolved one, the suffix dropped from two shapes); 4030 passed from the repo root, ruff clean. Until each branch is re-rendered by a push or rollover, its `Meta lines & _usage` row reads red locally - measured 17 of 18 branches; `.trinity/` is not CI-scored.

### Fixed

- **daemon: the slot-seeding test owns its clock - main and PR #767 were red on every host from 2026-09-13 03:00 UTC on one test (daemon owning, FPLAN-0584; devpulse landing; Patrick 12:31 "main green is the task, full go").** `tests/test_run_module.py::TestSlotSeeding::test_a_slotted_job_is_seeded_before_it_can_fire` asserted the weekly Sunday 03:00 slot as the literal `2026-09-06T03:00:00` while the seeder read `datetime.now()`; it was true from 09-06 to 09-12, merged green in v2.8.7, and the calendar turned it red (`'2026-09-13T03:00:00' == '2026-09-06T03:00:00'`, runs 34769664864 / 34769664829 / 34731222219 attempt 2). Cure: `freeze_runstate_clock(monkeypatch, at)` rebinds the `datetime` that `runstate` reads (seeder and due check both), and the one test is six parametrized cases whose expected slots are written by hand from a calendar, never computed with the arithmetic under test - the Sunday main went red, a later Sunday after the hour, the same Sunday a minute before, a Wednesday, the slot instant itself, a week behind. Sweep: a scratch pytest plugin shifted the process clock by +1d, +30d7h, +365d13h and +3650d19h across the whole daemon suite (control: at −7d the uncured test went green, so the shift reaches the seeder); no other calendar-coupled test exists (8 `test_memory_health` reds at the shifts are mtime artefacts and pass when `fromtimestamp` moves with the clock). 697+1F → 703 passed from the repo root and the branch, ruff clean, audit 100. Same change: the daemon README and `run --help` command-job example now match the live `prax/.daemon/schedule.json` (Sunday slot, `drone rm --stale 10d ..`, timeout 120), the two README nits devpulse had mailed.

- **ci: the Windows Test bound is 45 minutes, not 30 - two green runs on main were killed at 30:00 on 2026-09-13 and read as a red X (devpulse; CPLAN-0002).** A healthy Windows run is 25-27 minutes now (21k tests; since the 08-31 test seam and the 09-03/04 DPLAN-0325 sweep every test drives the real prax json service into a tmp sandbox, ~9 fsync'd staged writes each, and NTFS pays 5-20 ms per op where ext4 pays ~1 - Linux per-test cost is flat, Windows tripled; 84 test files with identical counts went 2.6 → 5.8 min). The bound was written when a run was ~9 minutes; the dependabot merge and the v2.8.7 merge six minutes later both hit it, the second queued behind the first under cancel-in-progress false. The comment in `windows-test.yml` now states today's number and why. The cause itself (skip fsync under `AIPASS_TEST_LOG_DIR`, or write nothing there - 511 of 557 test files write that exhaust and never read it) is prax's, captured in CPLAN-0002, not built.

- **hooks: an over-cap `.trinity` rejection now shows where the cap falls, and the navmap tells the 300-char fields to count before the edit (DPLAN-0342, FPLAN-0562; hooks owning, devpulse landing; Patrick ruled 2026-09-13 01:43).** DPLAN-0342 measured a 9.9% fleet rejection rate on memory edits with a median overage of 7% of the cap, and the refusal named only `318/300 chars (+18)`, so the retry was a guess. `edit_gate` now prints a second line under each measurement: `kept: "<the first cap characters>" | over: "<everything past the cap>"`, both halves JSON-quoted so a boundary that is a space, a newline or a quote stays visible, sliced by code point as the cap counts. The text is resolved from the proposed document through @memory's own `entry_types` (list index or dict key, then the entry's field) and drawn only when its length agrees with @memory's six-key violation record; a record that disagrees draws no cut and logs why, and that contract carries no text and does not grow. Warn mode logs the same cut. The block is unchanged: the edit is refused whole, never truncated or trimmed. `.aipass/tier1_navmap.md`: `sessions.summary` and `observations.note` count the draft in tens in reasoning with a running total and trim before the edit (counting in tens measured exact, 31/31); `todos.task` and `key_learnings.value` draft to ~80%; the navmap stays inside its ~8,000-char budget (7,991). Tests in the existing `test_edit_gate_trinity.py`, no new file: `TestRejectionNamesTheCutPoint`, 7 cases — list and dict entries, the cut on its own line, a whitespace-and-quote boundary, code points, a disagreeing record, warn mode; 8 mutants red.

- **hooks: `git check-ignore` is read-allowed (devpulse bf05f334).** It reports whether paths are ignored and writes nothing — `-q`, `-v`, `--stdin`, `-z`, `-n`, `--no-index` only shape output — so refusing it contradicted the gate's own redirect text, which promises read-only verbs run raw. Pinned in `TestGitGateReadAllowed`; 1 mutant red. Hooks suite 1953 → 1961 from both rootdirs, ruff clean, audit code rows 100.

- **hooks: a shell write to a `.trinity` memory file is refused at the gate, and one the reader cannot see is reported the moment it lands (DPLAN-0342 row 3, FPLAN-0563; hooks owning, devpulse landing; Patrick 2026-09-13 08:32: "still over cap writes getting through").** @memory's caps are measured on the Edit/Write lane, so a shell write landed unmeasured — @baud 21 of 21 sessions, @api 12 sessions + 16 learnings, @hooks 9 entries. `edit_gate` 1.9.0 → 1.10.0: the Bash lane reads its targets once and runs a second rule after the project fence — a claimed write to `.trinity/local.json` or `observations.json` is refused for every seat in every project, the admin seat included, and the refusal names the target and verb and says where memory is written (the Edit or Write tool, or a `drone @memory` verb). Known over-refusal, stated in the refusal and the README: an interpreter that only reads a memory path is refused too, so read it with the Read tool, `cat` or `jq`. The caps refusal's advisory now says a Bash write "is refused", no longer "not measured". Measured over 33 shell shapes: redirect, tee, `sed -i`, cp/mv/install/rsync/ln, `dd of=`, truncate, `python -c`, a python heredoc, the venv python, an absolute path in program text, `bash -c`, perl, node and cd-then-redirect are all claimed. The open shapes are published in `bash_writes.NOT_CAUGHT` (1.4.2) and the README: a bare file name in interpreter source after a `cd` (the `cd` is honoured; a bare word is not read as a path), a path joined in program text, a path in a shell variable, and `sponge`/`ed`. The tripwire covers those: `edit_gate.tripwire_snapshot` (PreToolUse, Bash) records the seat's memory `(mtime_ns, size)` under the call's `tool_use_id`, and `edit_gate.tripwire` (PostToolUse, Bash) compares. On a change not made by a `drone @memory`/`@spawn` verb it measures the whole file against @memory's live caps and tells the agent `MEMORY WRITTEN FROM A SHELL: …` with every over-cap entry and its cut point (or that the file measures clean, or no longer parses). It never blocks, and keeps at most 8 pending calls per session. **Not live until wired:** two `.aipass/hooks.json` entries, then `aipass trust`; no provider wire. Its limits: a write followed by a tool error (`PostToolUseFailure`, not wired in this engine) goes unreported, and a concurrent writer during a long call reads as the shell's. Live on this box: the running gate refused a redirect into a scratch `.trinity/local.json` with the new text, and the tripwire, driven in-process on a scratch seat with the live caps, printed `sessions [0]: 315/300 chars (+15)` and its cut. Tests in the existing `test_edit_gate_bash.py` (+19) and `test_edit_gate_trinity.py` (+12), no new file; 15 mutants red (the `.trinity` check dropped, the name set widened, the admin seat exempted, the advisory unchanged, the rule unwired, and ten on the tripwire). Hooks suite 1961 → 1992 from both rootdirs, ruff clean, audit 100.

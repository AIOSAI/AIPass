...
class TestNothingMovesUntilPatrickBlesses:
    ...
    def test_no_external_source_is_consulted_when_nothing_is_declared(self):
    ...
    def test_a_local_citizen_still_resolves_unchanged(self):
...
class TestPrecedence:
    def test_aipass_local_always_wins(self):
        """An external named 'memory' must never shadow our own @memory.
        ...
        assert undisturbed is not None and branch is not None, "our own @memory stopped resolving"
        assert branch["path"] == undisturbed["path"], branch["path"]
        assert "impostor" not in branch["path"], branch["path"]

    def test_declaration_order_breaks_ties_among_externals(self):
    ...
    def test_a_local_external_collision_is_logged_on_both_sides(self):
        with patch.object(fleet, "external_branches", return_value=[_external("memory", "impostor")]):
...
class TestGatewayFailureIsContained:
    def test_local_resolution_survives_a_broken_gateway(self):
        with patch.object(fleet, "external_branches", side_effect=RuntimeError("gateway down")):
            assert registry_handler.get_branch_by_name("nosuchbranch") is None

    def test_a_broken_gateway_is_loud(self):
        with patch.object(fleet, "external_branches", side_effect=RuntimeError("gateway down")):
            with patch.object(registry_handler, "logger") as log:

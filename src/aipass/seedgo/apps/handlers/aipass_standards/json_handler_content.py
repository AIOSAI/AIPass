...
def get_json_handler_standards() -> str:
    ...
    lines = [
        ...
        "",
        "  [bold]1. Handler capability[/bold] — ONE accept path, no alternatives:",
        "    The file IS the canonical shim: its sha256 equals the bytes pinned",
        "    in DPLAN-0325 section 3. Judged by identity, so a shim cannot drift",
        "    by a single character without saying so.",
        "",
        "    [dim]Three transitional shapes were accepted while the fleet sweep[/dim]",
        "    [dim]ran — the service import with no branch tokens, the shared[/dim]",
        "    [dim]JsonHandler wiring, and a standalone triplet surface. The sweep[/dim]",
        "    [dim]finished; all 18 handlers hash to the one shim, and the checker[/dim]",
        "    [dim]has had a single accept path since. Listed here as history, not[/dim]",
        "    [dim]as a way in.[/dim]",
        "",

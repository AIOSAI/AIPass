...
def get_modules_standards() -> str:
    ...
    lines = [
        ...
        "  • Do not route debug-grade detail to [dim]logger.info()[/dim] - it lands in",
        "    system_logs/ on every run. Use [dim]logger.debug()[/dim] instead: silent",
        "    under the default INFO, enabled with [dim]AIPASS_LOG_LEVEL=DEBUG[/dim] or a",
        "    per-tier log_level in [dim]prax_json/prax_logger_config.json[/dim]",

...
def get_silent_catch_standards() -> str:
    ...
    lines = [
        ...
        "  bypass rules",
        "",
        "[bold cyan]EXEMPTION -- CLASSIFYING, NOT SWALLOWING:[/bold cyan]",
        "  An except handler that turns a NAMED exception into a returned value",
        "  is not a silent catch: the exception's information IS the return, and",
        "  the caller decides. Reported as passing.",
        "     [dim]except FileNotFoundError:  return 'absent'          -- allowed[/dim]",
        "     [dim]except AssertionError as e: return ('FAILED', str(e)) -- allowed[/dim]",
        "  [yellow]![/yellow]  TWO clauses, both measured, and it is deliberately narrow:",
        "     [dim]1. a SPECIFIC exception type -- bare except and except Exception[/dim]",
        "     [dim]say only that something failed, which classifies nothing;[/dim]",
        "     [dim]2. a body of EXACTLY one return, whose value is a constant or[/dim]",
        "     [dim]mentions the bound exception. Extra statements, a pass, a[/dim]",
        "     [dim]continue, or a computed return that drops the exception all keep[/dim]",
        "     [dim]their finding. Measured fleet-wide before landing: 19 handlers[/dim]",
        "     [dim]qualify (16 test, 3 production) -- a shape, not an amnesty.[/dim]",
        '  [yellow]![/yellow]  "Constant" includes the EMPTY ANSWER of any return type:',
        "     [green]return []   return {}   return ()   return set()[/green]   allowed",
        "     [red]return [1, 2]   return set(c)   return build()[/red]      flagged",
        '     [dim]A function contracted to return a list says "found nothing" as[/dim]',
        "     [dim]`return []`, and set() has no literal spelling at all -- reading[/dim]",
        "     [dim]only ast.Constant made the verdict depend on the return TYPE. An[/dim]",
        "     [dim]empty container is an absence; a filled one is a fabricated[/dim]",
        "     [dim]answer, and set(c) is a computed value wearing the same name.[/dim]",
        "     [dim]Found by dogfooding seedgo's own checker (2026-08-31).[/dim]",
        "  [yellow]![/yellow]  A COUNTER is not this shape: `except X: missing += 1` may be",
        "     [dim]asserted on later or read by nobody, and the checker cannot see[/dim]",
        "     [dim]which -- so the finding stands and the reader rules on it.[/dim]",
        "     [dim]Raised by @spawn (2026-08-31): a logger call in a test helper[/dim]",
        "     [dim]would route production logging out of a suite, which is what the[/dim]",
        "     [dim]hygiene lane exists to stop -- two standards pulling opposite ways.[/dim]",
        "",

...
def _find_real_caller():
    ...
    Windows CI gate 2026-08-31 (@spawn's finding, relayed by @devpulse):
    inspect.stack() needs a READABLE CWD, and it needs one before any of this
    function's own code runs. It builds a FrameInfo per frame, which calls
    getsourcefile() -> getmodule(); getsourcefile returns EARLY for any file
    that exists on disk, so getmodule is only reached for the frozen importlib
    frames an import puts on the stack — and there, getmodule's module-scan loop
    calls os.path.realpath OUTSIDE any try. On POSIX the walk raises earlier, in
    getabsfile, where inspect catches FileNotFoundError and returns None; on
    Windows ntpath.abspath SUCCEEDS, so control reaches the unprotected
    realpath. That asymmetry is why this was invisible on Linux for as long as
    it existed. A frame's co_filename is already a string in memory; reading it
    touches nothing.
    """
    # Path.resolve() reaches the same realpath, so this is guarded too — __file__
    # is already absolute; the resolve only normalises it.
    try:

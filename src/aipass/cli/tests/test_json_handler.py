...
# Description: Tests that cli's shim is wired to the fleet json service
# Version: 2.0.0
# Created: 2026-09-03
# Modified: 2026-09-03
# =============================================

"""Tests for cli's JSON handler shim.
...
def test_every_public_name_is_a_bound_method_of_the_service(name):
    ...
    from frame 2 - so every entry cli logged would be attributed to the
...
def test_the_exceptions_are_the_services_own():
    """A caller catching cli's InvalidDocument catches the service's."""
...
def test_the_shim_is_bound_to_this_branch():
    """for_module derived cli's root from the shim's own __file__."""
    assert json_handler.get_json_path.__self__.branch_root.name == "cli"

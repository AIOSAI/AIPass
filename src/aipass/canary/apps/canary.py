...
"""
CANARY Branch - Main Orchestrator
...
os.environ.setdefault("AIPASS_BRANCH_NAME", "canary")
...
def _module_import_path(stem: str) -> str:
    ...
    """
    try:
        importlib.import_module(f"aipass.canary.apps.modules.{stem}")
        return f"aipass.canary.apps.modules.{stem}"
    except ImportError as e:
        # Not silent: the fallback path is a real branch in behaviour, so the
        # reason it was taken has to be readable afterwards. A canary that
        # swallows this is the exact failure species it exists to catch.
        logger.info("[CANARY] Package import failed for %s (%s) — using local layout", stem, e)
...
def discover_modules() -> List[Any]:
    ...
    for file_path in MODULES_DIR.glob("*.py"):
        ...
        try:
            ...
            logger.error("[CANARY] Failed to load module %s: %s", module_name, e)
...
def print_introspection() -> None:
    ...
    console.print()
    console.print("[bold cyan]CANARY - Permanent Test Citizen[/bold cyan]")
    console.print()
    console.print("[dim]Spawned, dispatched, resumed and broken so the working fleet never is[/dim]")
    console.print()
    ...
    if not modules:
        console.print("  [dim]none — canary carries no production modules by design[/dim]")

    console.print()
    console.print("[dim]Run 'drone @canary --help' for usage information[/dim]")
    console.print()


def print_help() -> None:
    """Rich-formatted top-level help for CANARY."""
    console.print()
    console.print("[bold cyan]CANARY - Permanent Test Citizen[/bold cyan]")
    console.print()
    console.print("[dim]Spawned, dispatched, resumed and broken so the working fleet never is[/dim]")
    ...
    console.print("  [green]drone @canary <command>[/green] [dim]\\[options][/dim]")
    console.print()
    ...
    console.print("[dim]Nothing here is evidence about the production fleet.[/dim]")
    console.print()
...
def route_command(command: str, args: List[str], modules: List[Any]) -> bool:
    ...
    for module in modules:
        try:
            ...
            logger.error("[CANARY] Module %s error: %s", module.__name__, e)
    return False
...
def main() -> int:
    ...
    # on `remaining` rather than the raw argv, so `canary <cmd> --help` shows
    # that subcommand's help instead of running it.
...
if __name__ == "__main__":
    try:
        ...
        logger.error("[CANARY] Unhandled error in main: %s", exc)
        sys.exit(1)

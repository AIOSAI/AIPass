...
Re-derived from the tree, not copied from a page. Modules come and go: today five.
...
│   └── kv.py              # key/value pairs kept in a file (set|get|delete|list)
├── handlers/
...
│   ├── kv/store.py        # reads and rewrites the kv store, owns every kv refusal

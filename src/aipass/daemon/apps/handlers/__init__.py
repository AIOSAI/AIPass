...
def _find_real_caller():
    ...
    the Windows CI gate 2026-08-31: inspect.stack() needs a READABLE CWD, and
    it needs one before any of this function's own code runs. It builds a
    FrameInfo per frame, which calls getsourcefile() -> getmodule() ->
    os.path.realpath; ntpath.realpath calls the working-directory reader
    unconditionally on its first lines, before it even checks whether the path
    is absolute, and that call site in getmodule is not inside a try. On POSIX
    the equivalent raise happens earlier, inside getabsfile(), where inspect
    catches it - which is why this was invisible on Linux for as long as it
    existed. A frame's co_filename is already a string in memory; reading it
    touches nothing.
    """
...
def _guard_branch_access():
    ...
    if caller_file is None:
        ...
        # either way - a second copy of the cwd dependency above, in service of
        # a branch that could not change the answer.
        return

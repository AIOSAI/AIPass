...
└── tests/                      # Test suite — 528 test functions in 19 files (pytest expands to 550)
```
...
All numbers below re-measured 2026-09-07 (FPLAN-0492 ruling 6), not carried.

- **528 test functions** across 19 test files; parametrization expands these to **550 cases**
  (`.venv/bin/python -m pytest src/aipass/daemon -c pyproject.toml --rootdir=. -q` →
  `550 passed in 99.70s`, 0 failed, 0 skipped)
- 10/10 modules covered — every module under `apps/modules/` is imported by at least one live test file
- **54 of 58 public functions tested** (seedgo's count; was 47/51 before ruling 6 added
  the catch-up, MISSED and slot surfaces)

...
# CODE STANDARDS:
#   - Pytest conventions
#   - Temp dir isolation via tmp_path - NEVER the live daemon_json/ directory
# =============================================
...
Measured on @daemon's unfixed handler (2 writers + 2 readers, 13,103 reads):
74.6% empty, 17.9% unparseable, 92.5% unusable.
"""
...
def json_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point the handler at a throwaway directory, never the live daemon_json/."""
    target = tmp_path / "daemon_json"
    target.mkdir()
...
def _valid_data_doc() -> Dict[str, Any]:
    ...
    return {"created": "2026-08-16", "last_updated": "2026-08-16", "operations_total": 7}


def _atomic_write(target: Path, data: Any) -> None:
    """Call the handler's atomic write helper, resolved at call time.

    Resolved dynamically so this suite can be written red-first: before the fix
    lands the helper does not exist and every test naming it fails loudly with
    that sentence, rather than the file failing to import.
    """
...
class TestAtomicWriteHelper:
    ...
    def test_failed_write_cleans_its_temp_file(self, tmp_path: Path) -> None:
        """A failed write does not litter the directory the handler globs."""
        target = tmp_path / "precious.json"
    ...
    def test_raises_rather_than_silently_failing(self, tmp_path: Path) -> None:
        ...
        with patch.object(jh.os, "replace", side_effect=OSError("nope")):
            with pytest.raises(OSError):
                _atomic_write(target, {"a": 1})
...
class TestEveryWriteSiteIsRouted:
    ...
    def test_no_truncating_open_survives_in_source(self) -> None:
        ...
        offenders = _TRUNCATING_OPEN.findall(source)
    ...
    def test_source_guard_catches_offenders(self) -> None:
        ...
        fix removed, plus the one form it must tolerate.
        """
        assert _TRUNCATING_OPEN.search('with open(json_path, "w", encoding="utf-8") as f:')
        assert _TRUNCATING_OPEN.search("with open(path, 'w') as f:")
        assert _TRUNCATING_OPEN.search('open(target, "a", encoding="utf-8")')
        assert _TRUNCATING_OPEN.search('open(target, "w+")')
        # The fix itself must NOT trip the guard
        assert not _TRUNCATING_OPEN.search('with os.fdopen(descriptor, "w", encoding="utf-8") as stream:')
        # Reads are none of the guard's business
        assert not _TRUNCATING_OPEN.search('with open(json_path, "r", encoding="utf-8") as f:')


# Matches open(..., "w"/"a"/"x"/"w+") while tolerating os.fdopen(fd, "w") - the
# staged-descriptor write inside the atomic helper.
_TRUNCATING_OPEN = re.compile(r"(?<!fd)open\(\s*[^)]*?['\"][wax]\+?['\"]")
...
class TestConcurrentReadersSeeWholeDocuments:
    ...
    def test_no_torn_reads_under_concurrent_writes(self, json_dir: Path) -> None:
        ...
        Unfixed, this same probe scored 92.5% unusable reads on @daemon's
        handler (74.6% empty, 17.9% unparseable) across 13,103 reads.
        """
        module = "raced"
...
class TestExistingBehaviourUnchanged:
    ...
    def test_round_trip_through_save_and_load(self, json_dir: Path) -> None:
        ...
        jh.save_json("roundtrip", "data", document)
        loaded = jh.load_json("roundtrip", "data")

        assert loaded is not None
        assert loaded["operations_total"] == 7

    def test_invalid_structure_still_raises(self, json_dir: Path) -> None:
        """Structure validation still rejects bad documents before writing."""
        with pytest.raises(ValueError):
            jh.save_json("bad", "config", {"missing": "fields"})
    ...
    def test_no_temp_files_left_in_json_dir_after_normal_use(self, json_dir: Path) -> None:
        ...
        jh.increment_counter("tidy", "runs", 3)
        jh.update_data_metrics("tidy", last_result="ok")

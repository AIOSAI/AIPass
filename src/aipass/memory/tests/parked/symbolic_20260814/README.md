...
`(disabled)` suffix, the house convention for code that is present but must not run: it is
not importable by dotted path and pytest never collects it.
...
```
handlers/         chroma_client(disabled).py  deduplicator(disabled).py
                  extractor(disabled).py  hook(disabled).py  retriever(disabled).py
                  storage(disabled).py  __init__(disabled).py  <- the original package init
modules/          symbolic(disabled).py                        <- the original 1602-line CLI module
handlers_vector/  embedder(disabled).py                        <- added 2026-08-14 (see below)
```

Strip `(disabled)` from a filename to get the name it had, and must have again, in the
live tree. That is the whole of the renaming.
...
Every step drops the `(disabled)` suffix — the file's real name is what is left.

1. Move `tests/parked/symbolic_20260814/handlers/*(disabled).py` to
   `apps/handlers/symbolic/`, renaming each to its bare name (the original
   `__init__.py` is in there and replaces the raising stub)
2. `tests/parked/symbolic_20260814/modules/symbolic(disabled).py` -> `apps/modules/symbolic.py`
3. `tests/parked/symbolic_20260814/handlers_vector/embedder(disabled).py` ->
   `apps/handlers/vector/embedder.py`

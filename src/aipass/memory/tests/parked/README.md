...
|---|---|---|---|
| `symbolic_20260814/` | The owner, 2026-08-14 — park the symbolic fragments tier, revivable; the active piece is Compass | 9 implementation files, 0 test units. Pinned by `tests/test_symbolic_parked.py`, which fails if any of them goes missing. | **2026-08-14, open.** Revives when the AUDN deduplicator records what a `Delete` verdict removed and why — the Agent Memory Atlas finding that parked it — and a live lane needs fragments back. Compass is the curated-truth piece until then. |

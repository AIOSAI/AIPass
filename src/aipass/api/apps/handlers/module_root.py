...
MEASURED IN THIS BRANCH, 2026-08-31: with the handlers guard cured, 49 of
...
and never walks up to a Path.cwd() fallback, so only the resolve guard
lives here.
"""
...
def _record_unresolved(path: Path, exc: OSError) -> None:
    ...
    Runs at module import time on every caller. The logger line is the primary
    diagnostic and may itself raise in the world that gets here - that raise
    belongs to module_file's wrapper, which owns the promise that no
    diagnostic becomes an import crash.
    ...
    # import takes it, and identical warnings describe one condition -
...
def module_file(file: str) -> Path:
    ...
    try:
        ...
        # ratified fleet-wide): the world that reaches this line is exactly the
        # world where the logger may be down too - prax's logger construction
        # reads the cwd. A crashing diagnostic here would BECOME the import

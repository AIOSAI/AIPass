...
# Description: Tests for {{BRANCH}}'s entry point routing, help and introspection
# Version: 1.0.0
# Created: {{DATE}}
# Modified: {{DATE}}
# =============================================

"""Tests for {{BRANCH}}'s CLI entry point.
...
class _StubModule:
    ...
    __name__ = "aipass.{{BRANCH}}.apps.modules.stub"
...
def _run(monkeypatch, argv):
    ...
    monkeypatch.setattr(sys, "argv", ["{{BRANCH}}", *argv])
...
def test_version_flag_prints_version(monkeypatch, capsys, flag):
    ...
    out = capsys.readouterr().out
    assert "{{BRANCHNAME}}" in out
    assert branch_entry.__version__ in out
...
def test_subcommand_help_does_not_execute_the_command(monkeypatch, stub_module):
    """`{{BRANCH}} probe --help` asks the module for help; it never runs bare."""
...
def test_branch_name_is_set_at_import_time(monkeypatch):
    ...
    reloaded = importlib.reload(sys.modules["aipass.{{BRANCH}}.apps.{{BRANCH}}"])

    assert os.environ["AIPASS_BRANCH_NAME"] == "{{BRANCH}}"
    assert reloaded.__version__ == branch_entry.__version__
...
def test_module_import_path_logs_when_it_falls_back(monkeypatch, mock_logger):
    ...
    monkeypatch.setattr(branch_entry.importlib, "import_module", _always_missing)

    result = branch_entry._module_import_path("ghost")

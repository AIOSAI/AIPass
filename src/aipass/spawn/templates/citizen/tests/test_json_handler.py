...
# Description: Tests for {{BRANCH}}'s JSON handler shim and its shared contracts
# Version: 1.0.0
# Created: {{DATE}}
# Modified: {{DATE}}
# =============================================

"""Tests for {{BRANCH}}'s JSON handler.

Two things are under test and they are different things:
  1. The shim wiring - that this branch's singleton points at {{BRANCH}}_json/
     and re-exports the shared API.
  2. The contracts the branch relies on - defaults, validation, raises-on-invalid,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("data", {"created": "2026-01-01", "last_updated": "2026-01-01"}, True),
        ("data", {"created": "2026-01-01"}, False),
        ("log", [], True),

...
#   - tmp_path + monkeypatch for file isolation — never the live devpulse_json/
# =============================================

"""
Durability tests for the devpulse JSON handler.
...
import aipass.devpulse.apps.handlers.json.json_handler as json_handler_mod
...
def json_dir(tmp_path, monkeypatch):
    ...
    target = tmp_path / "devpulse_json"
    target.mkdir()
    monkeypatch.setattr(json_handler_mod, "JSON_DIR", target)
    return target
...
def test_exhausted_retry_leaves_the_original_intact_and_cleans_the_temp(json_dir, monkeypatch):
    ...
    json_handler_mod.save_json("durability", "data", original)
    ...
    with pytest.raises(PermissionError):
        json_handler_mod.save_json("durability", "data", _valid_data(filler="doomed"))
